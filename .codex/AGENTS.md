...
> **2026-08-28 当前产品基线（覆盖下方全部历史阶段说明）**：产品代码基线为 `bace1b4`。`V8.4.6` 已完成教师申请、课程创建与审核、学生选课、多教师共享草稿、不可变发布、三种完成结果、三角色工作台、完整演示数据和全链路验收；原 `V8.1.5 / UI-015` 登录前作品导览候选已正式整合。正式活动为 `90 / 18 / 19 = 127`，全部实验本体继续冻结。当前仍由用户与主开发单线协作，不建立工作组或额外 worktree；下一周期尚未定版，只能从 `doc/02-更新规划.md` 的候选清单经负责人批准后启动。优先方向改为管理驾驶舱、课程版本演进、可解释学习证据和教学语义编排；新实验、微信小程序、服务器发布、竞赛材料、生产级安全与全框架重写继续暂缓。
>
> **2026-08-22 历史控制点**：文档控制基线曾为 `V8.0.37 / DOC-002`，产品运行字节曾以 V8.0.34 为未验候选。以下长篇阶段记录只用于追溯，不得覆盖上方 2026-08-28 控制点。

## 当前基线

星序 Astra 是多星系全栈学习平台：

- 工科试验室：五大学科、90 个可视化实验。
- 代码空间：`codevis/` 独立子站与多语言运行时。
- 未来星系：六个跨学科探索方向、19 个活动身份。
- Python/FastAPI 业务后端：认证、学校/班级/课程、作业、内容、学习分析、任务与审计。
- Node/C++ 静态服务：只承担审核后的浏览器资源托管。

以下内容从 V6—V8.0 按时间保留，用于解释历史分支、退回项和旧验证结论；当前实现和开发顺序始终以上方控制点、01 与 02 为准。
...
2026-07-31 用户随后以第二轮 10 条浏览器批注恢复 `FE-021 / V7.9.69` 单任务。主开发在根工作区完成学生 rail 232px↔76px 收缩、身份摘要贴底、stage inline-size 单 / 双栏、班课内存选择恢复、学生授权顶栏 / 搜索裁剪、五学科概览删项与 Englab Hero 复活；Router 保持冻结 1116 行，资源代际为 `20260731v7969StudentUiP0`。随后 `DATA-004 / V7.9.70` 只通过当前 9003 正式 HTTP API 建立全课程与部分课程两套相互隔离的师生 / 班级：前者 17 课 / 124 open units，后者 14 课 / 42 units（30 open / 6 locked / 6 hidden）；账号秘密只在用户回执中提供，不进入仓库。

2026-07-31 `FE-022 / V7.9.71` 修复 DATA-004 对照账号暴露的 ModuleSelector 旧授权 DOM：`astra:student-catalogue-ready` 每次变化都会先移除五学科旧 sidebar / ledger / gallery / sources，再按当前 exact-open activities 重建；身份替换或撤权先走既有 module close，重复 Router support / 主启动保持单例。入口资源代际为 `20260731v7971CatalogueCountP0`。9003 已按部分学生数学 1 / 物理 1 / 生物不可达，再切回全开放学生生物 19 完成真实复验。
...
- `doc/01-开发者手册.md`：当前实现、核心业务、运行和边界的主入口；详细技术由 `doc/01-子文档/11—19` 承担。
- `doc/02-更新规划.md`：已完成周期、下一周期候选、优先级、风险和长期方向的唯一规划主文档。
- `doc/03-发布历史.md`：发布判断和大版本索引；详细历史由 `doc/03-子文档/31—35` 承担，跨版本原文由 39 保存。
...
1. 当前没有活动 Goal、派单、工作组或非主 worktree；`main`、`houduan` 和全部 `codex/*` 历史分支保持原样，不因工作组解散而删除。
2. 七个未提交专业现场已由 PM-025 转为具名 stash，精确 SHA 只在 `doc/03-子文档/35-V8版本发布历史.md` 的 V8.0.36 记录；`u358@3734a2f` 的一条主线未集成提交继续由分支保存。未经用户明确决定，不得删除 stash/分支、批量 apply、reset、clean、rebase 或强制覆盖。
3. `doc/01-开发者手册.md`、`doc/02-更新规划.md`、`doc/03-发布历史.md` 及其编号子文档由主开发集中写入；禁止复制或新建同职责主文档，不得以整理为名删除原始技术或发布事实。
4. 恢复后的产品验证必须在全新环境和精确 revision 上执行并如实记录；主开发自测不能冒充尚未发生的独立 QA。只有用户以后明确要求独立工作组时才建立 QA 组。
5. `qianduan`、`codex/team-be`、`codex/team-fe` 和其他旧分支/标签只作历史与候选审计锚点，不是开发入口。
6. 默认不创建 worktree。若用户明确要求并行，先从最新 clean 主线建立最小隔离树；若要恢复旧 stash，必须先在相应原分支的新隔离树中审查，禁止直接应用到根工作区。
7. `主开发` 及其任务分支使用 `Vx.y.z GROUP type(scope): 中文说明（TASK-ID）`，责任组、任务编号与标题必须一致；`review` 当前冻结，只有用户重新启动 review 周期后才恢复 `reN：中文说明`。
8. 若平台强制审批、权限不足或安全规则阻断某一步，立即停止受影响动作，不绕过、不重复申请、不扩大范围；保留精确错误、路径、日志和现场状态，并向用户报告。
9. 当前候选优先级为：管理驾驶舱 → 课程版本时光机与发布影响预演 → 可解释学习证据链 → 教学语义卡与知识星图编排 → 可用性复验。新实验、深层安全、真实 MySQL、生产部署、极限性能、微服务/框架重写、通用 OJ 与竞赛材料均后置，除非用户另行立项。
...
9. 未来星系的当前开发入口是 `doc/02-更新规划.md` 第 6—7 节，课程身份、内容和视觉边界继续以 `doc/02-子文档/25-V7.7未来星系18课程内容与路由冻结矩阵.md`、`26-V7.7未来星系页面视觉规格.md`、`27-V7.7三星系六门代表课程教学与证据冻结契约.md` 为准。18 门课程全部保留；CONTENT-003 / V7.9.51 已完成来源校准，不改变课程数量、身份、教学闭环或代表课选择。目标只注册一个 `page-frontier` 并由共享 Router 独占 hash，canonical route 为 `#frontier/<direction>/<course>`，legacy 只 `replaceState` 一次。只有精确 open 课程可以加载自己的内容模块、唯一 owner 和 evidence；publication 未决 / 失败与 hidden / missing / unknown 不得把课程身份写入目录、导航或页脚，locked 仅显示授权允许的安全元数据。课程事件只能在 evidence owner 明确接收后返回 true；两门 `domain_evidence` 代表课的 explained 必须由课程门禁发出第 27 号专用 schema，不能使用共享通用解释控件绕过。证据数值必须同时满足 finite、range 与精确小数位，不能用容差接受超精度值。Future 根元素不得以 `overflow-x:hidden|clip` 掩盖真实溢出，所有主动作保持 `#8CE7FF`。六个真实 owner 目录是 `pages/cosmos`、`pages/engineering`、`pages/datascience`、`pages/infotech`、`pages/materials`、`pages/humanities`。FE-012 的 V7.7.33 产品观察 finding=0，但不得把平台能力导致的 BLOCKED / NOT-RUN 冒充 A03a PASS；FE-017 的 R8 独立复审同样不得被静态门禁、R7 PASS 或主开发自测替代。26 号视觉规格与 PM-017 三图已完成两次产品确认；旧 f017 现场已归档为可恢复 stash，未经用户明确授权不得直接应用到根工作区。
...
- 单任务恢复增量：`V7.9.67` 只记录暂停与停组；`V7.9.68` 唯一分配给 `V7.9.68 FE feat(student-flow): 收束入班与课程发现体验（FE-020）`；`V7.9.69` 唯一分配给 `V7.9.69 FE fix(student-ui): 收束学生工作台与授权发现（FE-021）`；`V7.9.71` 唯一分配给 `V7.9.71 FE fix(catalogue): 修复身份切换后的实验目录统计（FE-022）`。三项只由主开发在根工作区写入，不恢复专业工作组或消费冻结候选。
- DATA 增量：`V7.9.6` 只属于 PM-009 的数据组实态登记，`V7.9.7` 唯一分配给 DATA-001 本地演示初始化器，`V7.9.12` 唯一分配给 DATA-002 单课程教师演示补样，`V7.9.19` 唯一分配给 DATA-003 producer profile 对齐，`V7.9.70` 唯一分配给 `V7.9.70 DATA feat(demo): 建立双范围班级账号（DATA-004）`；任何组不得自行猜测或复用。
