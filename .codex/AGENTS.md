...
2. 项目对接模式已经启用。CONTENT、FE、BE、QA 已按标准标题恢复；QA 只读，写入组只在收到主开发的精确任务 / 版本 / 文件包后写各自物理目录。CONTENT-004 / V7.8.3、DATA-003 / V7.9.19 与 BE-013 / V7.8.4 均已完成并暂停。QA-013 / V7.8.6 已 RETURN FE-013 并释放 Browser / 9001；qe13 与唯一 TEMP 只读保留。FE-012 的 `fz30` 已收到独立 Zoom 写入包；FE-013 / V7.8.7 的竞态返修包已登记，但 `ft87 / codex/fe-v7.8-teacher-attachment-race` 在 V7.9.29 实态回填与完整 `[ASSIGN]` 前仍不存在且写入为 0。两条 FE 线只因文件集合完全不相交而并行，仍各有一个写入者；旧 fe13 不恢复。q28a、c04e、da03、be13、fe13、qe13、fz30 与外部证据各守任务边界。原长期任务处于 systemError 时，只允许主开发登记的边界明确临时执行者接续同一任务编号，不创建竞争任务标题；不得复用旧 d6b6 / f27a / b56e。
