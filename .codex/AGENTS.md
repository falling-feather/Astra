...
V6.6.63 后端阶段已经完成；V7.4 完成项目对接、注册 / 生命周期、认证与角色资源、组织治理、统一三星系总览、单一全局治理和 9001 一键入口；V7.5 完成代码空间、未来星系、课程编排、教师进度、OJ 诚实降级和全仓 review。V7.6 由 `主开发` 作为集成分支，`main@V7.5.11` 保持公开稳定。V7.6.5 / BE-006 的权威学习证据与完成规则已通过 QA-014 A02。用户现已授权恢复 V8.0 开发：`PM-015` 在 V7.6.15—16 冻结约四次更迭 / 8 小时调度并恢复 CONTENT、FE、QA 三个最小长期组；`CONTENT-003 / V7.7.0` 冻结 18 门课程一对一内容矩阵；`FE-012 / V7.7.1—2` 已集成共享学习证据首批实现，V7.7.6 / V7.7.8 / V7.7.10 依次关闭 locked / authority / 44px、移动浮层遮挡、真实预测确认与折叠焦点问题。`QA-014 A03a / V7.7.11` 已证明这些主修复通过，但在精确 390×844 冷启动发现力学内容仍以约 896px 内宽被根级裁剪，故 `V7.7.12 / FE-012` 只返修该问题。`PM-009 / V7.7.13—14` 又以源码证明教师缺少逐学生证据发现、管理员缺少课程状态受约束变更两项展示阻断，解除既有 `BE-007` 暂缓、唯一预留 V7.8.0，并登记后端标准长期任务与 `0dce / codex/be-v7.8-three-role-min-api @ de7fbcb` 独立工作树。`FE-017 / V7.7.3` 只提出待用户确认的完整视觉规格，尚未授权未来星系产品代码。当前交付目标仍是课程作业 / 设计大赛的本地展示版：优先课程互动、学生—教师—管理员协同、SQLite 数据对账和 9001 一键启动；真实 MySQL、公网 R6、隔离 runner、压力测试、真实课堂试点与 AI 能力后置。任务、责任、版本与门禁只认 `doc/02-项目规划.md`。

当前增量：`FE-012 / V7.7.17` 已把折叠 dock 的 Escape 仲裁移到 document capture 阶段，并在 Zoom、实验指南和导出菜单持有 Escape 时让位；真实双 owner 生命周期合同与独立复审均通过。`QA-014 A03a / V7.7.18` 现唯一负责在精确集成提交上复验一次 Escape 只折叠 dock、路由 / active module 保持和焦点归还，并回归既有双视口、发布态、证据与资源门禁。`BE-007 / V7.8.0` 已由最终候选 `d1fefcdc` 集成；UI-003 / V7.9.4 仍在独立候选阶段。所有更新仍以 `doc/02-项目规划.md` 为准。

DATA-001 的 QA 结论前置已满足；唯一数据组长期任务登记在 detached `C:/Users/niu-h/.codex/worktrees/992d/工科实验室 @ c07bf60`，写入首步必须从主开发 V7.9.6 控制提交切换 `codex/data-v7.9-local-preview-demo`。`V7.9.7` 只允许通过既有正式 API 实现本地演示初始化器，不得新增模型、迁移、endpoint、service、router，不得直接写数据库或修改前端 / UI / teacher / Future。
...
2. 项目对接模式已经启用。CONTENT、FE、BE、QA 已按标准标题恢复并登记独立 worktree；CONTENT 与 QA 只读，写入组只在收到主开发的精确任务 / 版本 / 文件包后写各自物理目录。BE-007 已因教师逐学生证据发现和管理员课程状态变更两个源码缺口在 `0dce` 激活，FE-012 继续独占 `d6b6`；同一任务只设一个主责组。
...
9. 未来星系层级重构以 `doc/02-项目规划.md` 第 1.4 节、`doc/02-子文档/25-V7.7未来星系18课程内容与路由冻结矩阵.md`、`doc/02-子文档/26-V7.7未来星系页面视觉规格.md`、`FE-017`、`QA-015` 为唯一规划入口。18 门课程全部保留，CONTENT-003 已完成，FE-012 首批共享接口、A03a 原范围返修及 V7.7.8 移动端浮层避让已形成集成基线；V7.7.9 因 open 预测证据缺失和折叠 dock 不可见 Tab 停靠退回，V7.7.10 仅修这两个失败项。26 号视觉规格仍待用户确认，之后必须生成并再次确认完整概念图。只有 A03a 通过和两次视觉确认完成、FE 写入分支从最新集成点建立并收到新版本令牌后，FE-017 才能写入产品代码。
...
- `主开发` 及其任务分支使用 `Vx.y.z GROUP type(scope): 中文说明（TASK-ID）`；版本由 02 唯一分配。`V7.6.6`、`V7.6.7`、`V7.6.11`、`V7.6.12` 均为已取消的空预留，不得复用；`V7.6.15—16` 只属于 `PM-015`，`V7.7.0` 只属于 `CONTENT-003`，`V7.7.1—2`、`V7.7.6`、`V7.7.8`、`V7.7.10` 与 `V7.7.12` 只属于 `FE-012` 的分批接入和精确返修，`V7.7.3` 只属于 `FE-017` 的视觉规格提案而非产品实现，`V7.7.4` 只属于 `TOOLS-001`，`V7.7.5`、`V7.7.7`、`V7.7.9` 与 `V7.7.11` 只属于 `QA-014 A03a`，`V7.7.13—14` 只属于 `PM-009` 的三角色接口缺口证明、BE-007 激活与团队实态登记；`V7.8.0` 已唯一预留给 `BE-007`，不得由其他组使用。
- 增量版本对账：`V7.7.15` 属于 `CONTENT-001`，`V7.7.16` 属于 `QA-014 A03a` 的 RETURN 记录，`V7.7.17` 已由 `FE-012` 的折叠 Escape 路由逸出返修使用，`V7.7.18` 唯一分配给 `QA-014 A03a` 精确复验；`V7.8.0` 已由 BE-007 使用，`V7.9.4` 唯一分配给 UI-003，`V7.9.5` 属于 PM-009 契约对账。
