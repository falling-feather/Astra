...
> **2026-08-22 当前控制点（覆盖下方历史阶段说明）**：主线基线为 `V8.0.36`，产品运行字节仍以 V8.0.34 为未验候选。项目处于“用户 ↔ 主开发”单线暂停态；侧栏中的后端组、架构组、前端组、内容组、QA 组五个长期对接任务均已归档，只保留“星序 Astra｜主开发｜总控”，协作代理树也仅剩 `/root`，不自动恢复。Git 只登记根工作树；七个脏现场已转为具名 stash，`u358@3734a2f` 独有提交由原分支保存，全部 66 条本地分支均保留。恢复开发默认仍由主开发单线推进，优先师生前端、课程内容/动画、演示素材和竞赛包装；只有用户明确要求并行时才新建最小工作组、对接任务或 worktree。恢复锚点与路线只认 `doc/02-更新规划.md`。
...
2. 七个未提交专业现场已由 PM-025 转为具名 stash，精确 SHA 只在 `doc/02-更新规划.md` 第 9 节和 `doc/03-发布历史.md` V8.0.36 记录；`u358@3734a2f` 的一条主线未集成提交继续由分支保存。未经用户明确决定，不得删除 stash/分支、批量 apply、reset、clean、rebase 或强制覆盖。
3. `doc/01-开发者手册.md`、`doc/02-更新规划.md` 与 `doc/03-发布历史.md` 由主开发集中写入；禁止复制或新建同职责主文档，原始长文档不得为追求通用规范而大规模简化。
