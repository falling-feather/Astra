...
当前增量：`QA-014 A03a / V7.7.16` 已确认 V7.7.12 的移动端内宽、Canvas / DPR、Zoom、44px、发布态与学习证据均通过，但因次级 FAB 获焦后 Escape 同时触发 dock 折叠和模块关闭而 P1 RETURN；`V7.7.17` 只允许原 `FE-012` 修复这一个事件消费边界。`BE-007 / V7.8.0` 已由最终候选 `d1fefcdc` 集成；UI-003 / V7.9.4 仍在独立候选阶段。所有更新仍以 `doc/02-项目规划.md` 为准。
...
- 增量版本对账：`V7.7.15` 属于 `CONTENT-001`，`V7.7.16` 属于 `QA-014 A03a` 的 RETURN 记录，`V7.7.17` 唯一分配给 `FE-012` 的折叠 Escape 路由逸出返修；`V7.8.0` 已由 BE-007 使用，`V7.9.4` 唯一分配给 UI-003，`V7.9.5` 属于 PM-009 契约对账。
