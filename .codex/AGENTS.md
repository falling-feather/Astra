...
V6.6.63 后端阶段已经完成；V7.4 完成项目对接、注册 / 生命周期、认证与角色资源、组织治理、统一三星系总览、单一全局治理和 9001 一键入口；V7.5 完成代码空间、未来星系、课程编排、教师进度、OJ 诚实降级和全仓 review。V7.6 由 `主开发` 作为集成分支，`main@V7.5.11` 保持公开稳定。V7.6.4 已通过 QA-014 A01；V7.6.5 / BE-006 的权威学习证据与完成规则已通过 A02；V7.6.13 / PM-013 是历史暂停控制点，V7.6.14 / PM-014 登记未来星系重构问题。用户现已授权恢复 V8.0 开发：`PM-015` 已在 V7.6.15—16 冻结约四次更迭 / 8 小时调度并恢复 CONTENT、FE、QA 三个最小长期组；`CONTENT-003 / V7.7.0` 冻结 18 门课程一对一内容矩阵、canonical / 兼容路由、教学闭环、事实依据与 P0—P2 门禁；`FE-012 / V7.7.1—2` 已冻结并集成共享学习证据首批实现，`V7.7.6` 已集成 A03a 原范围返修。`V7.7.8 / FE-012` 已关闭 V7.7.7 的移动端浮层遮挡；`V7.7.9 / QA-014 A03a` 独立复验确认该遮挡、44px、locked 深链、两视口和权限通过，但因“记录预测”未形成权威 `predicted/completed`（P1）及折叠 dock 不可见按钮仍进入 Tab 顺序（P2）再次退回原 FE-012。`V7.7.10 / FE-012` 只用于关闭这两个精确失败项，完整 FE-012 与未来星系接线仍未完成。`FE-017 / V7.7.3` 只提出待用户确认的完整视觉规格，尚未授权未来星系产品代码；`TOOLS-001 / V7.7.4` 把本地 SQLite QA 账本对齐至当前 0051 迁移头。当前交付目标仍是课程作业 / 设计大赛的本地展示版：优先课程互动、学生—教师—管理员协同、SQLite 数据对账和 9001 一键启动；真实 MySQL、公网 R6、隔离 runner、压力测试、真实课堂试点与 AI 能力后置。任务、责任、版本与门禁只认 `doc/02-项目规划.md`。
...
2. 项目对接模式已经启用。CONTENT、FE、QA 已按标准标题恢复并登记独立 worktree；CONTENT 与 QA 只读，FE 在收到主开发的新版本令牌和写入包前也只读。BE 在主开发证明现有接口缺口前保持停用。同一任务只设一个主责组。
...
- `主开发` 及其任务分支使用 `Vx.y.z GROUP type(scope): 中文说明（TASK-ID）`；版本由 02 唯一分配。`V7.6.6`、`V7.6.7`、`V7.6.11`、`V7.6.12` 均为已取消的空预留，不得复用；`V7.6.15—16` 只属于 `PM-015`，`V7.7.0` 只属于 `CONTENT-003`，`V7.7.1—2` 只属于 `FE-012` 的择取契约与首个实现批次，`V7.7.3` 只属于 `FE-017` 的视觉规格提案而非产品实现，`V7.7.4` 只属于 `TOOLS-001` 的 SQLite QA 账本迁移头修正，`V7.7.5` 只属于 `QA-014 A03a` 的首轮独立验收退回，`V7.7.6` 只属于 `FE-012` 的 A03a 原范围返修，`V7.7.7` 只属于 `QA-014 A03a` 的返修复验退回，`V7.7.8` 只属于 `FE-012` 的 390×844 固定浮层遮挡返修，`V7.7.9` 只属于 `QA-014 A03a` 对 V7.7.8 精确集成提交的独立复验退回，`V7.7.10` 只属于 `FE-012` 的 open 预测证据与折叠 dock Tab 返修。
