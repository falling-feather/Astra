...
UX-001 只读审计确认当前星序学校 / 班级 / 课程范围选择会在 `role-home-client.js::choose()` 重绘后替换 active select 并丢失焦点。V7.9.63 只登记第 2.18 节单一 UX 写入包，V7.9.64 只回填新 worktree，V7.9.65 唯一修复该焦点连续性；产品范围固定为 role-home owner，不碰尚未集成的 UI-003 公共缓存代际、FE-013、FE-017、课程 owner 或后端。登录 tab 语义、Physics reduced-motion 与最终双视口继续保留原任务门禁，不因本切片提前关闭 UX-001。
...
2. 项目对接模式已经启用。CONTENT、FE、BE、QA 已按标准标题恢复；QA 只读，写入组只在收到主开发的精确任务 / 版本 / 文件包后写各自物理目录。CONTENT-004 / V7.8.3、CONTENT-003 / V7.9.51、DATA-003 / V7.9.19 与 BE-013 / V7.8.4 均已完成并暂停；c003 候选 `0ce127a5` clean、只读保留。V7.8.9 / fr89 在 V7.9.62 控制提交后只向原 FE owner 恢复同一目录写入，精确 allowlist 扩展为 owner、能力账本、两个合同和三份专业文档共 7 文件；若平台阻断再次出现立即原地暂缓。FE-017 的 R8 组合候选在 f017 为 36 tracked modified + 22 untracked、0 staged，A/B/C lane 写入关闭。静态 / 合同门禁已通过，但独立数值复审被平台安全误判中止；标准 FE、所有 lane 与 Browser 均暂停，不得复制差异、换审查者或用 R7 PASS 冒充 R8 / QA PASS。V7.9.48 仍唯一预留给 FE-017 最终产品提交；V7.9.51 已由 CONTENT-003 使用，V7.9.52—55、V7.9.57、V7.9.59 与 V7.9.61—62 只属于 PM 控制面。UI-003 的旧 7a78 / u356 均只读保留；V7.9.58 永久留空。`u358 / codex/ui-v7.9-admin-resource-generation @ 3734a2f` clean，V7.9.60 专业提交已通过第二轮独立复审但因正式服务启动被平台策略拦截而暂缓集成；禁止重试启动、换工具 / 目录、改提交、merge / push / rebase / 清理。Router 与 owner 源文件、admin.css、共享 auth-ui 保持冻结。ft87 继续关闭。FE-012 的 fz30 已提交 `30f7f189` 并关闭写入；QA-014 / V7.7.31—33 均已 BLOCKED / NOT-RUN，qz31 / qz32 / qz33 及其现场只读冻结，V7.7.33 产品 finding=0，同类运行包因工具能力暂缓。qt88 已完成回执并关闭运行，Browser / 9001 已释放。q28a、c04e、da03、be13、fe13、qe13、qz31、qz32、qz33、qt88、ft87、fr89、fz30 与外部证据各守任务边界；不得复用旧 d6b6 / f27a / b56e。
...
9. UX-001 的首个独立写入片只处理星序范围选择焦点连续性。V7.9.63 提交前不得创建或使用新 UX 工作树；V7.9.64 回填实际路径 / 分支 / 基线前不得写入。实现只允许第 2.18 节五文件，任何平台阻断按第 8 项原地保全。
