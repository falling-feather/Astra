...
2. 项目对接模式已经启用。CONTENT、FE、QA 已按标准标题恢复并登记独立 worktree；CONTENT 与 QA 只读，FE 在收到主开发的新版本令牌和写入包前也只读。BE-007 已因教师逐学生证据发现和管理员课程状态变更两个源码缺口解除暂缓，但后端组必须在 02 回填标准任务、独立 worktree、分支、基线与允许文件后才可写。同一任务只设一个主责组。
