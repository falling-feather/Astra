...
V6.6.63 后端阶段已经完成；V7.4 完成项目对接、注册 / 生命周期、认证与角色资源、组织治理、统一三星系总览、单一全局治理和 9001 一键入口；V7.5 完成代码空间、未来星系、课程编排、教师进度、OJ 诚实降级和全仓 review。V7.6 由 `主开发` 作为集成分支，`main@V7.5.11` 保持公开稳定。V7.6.5 / BE-006 的权威学习证据与完成规则已通过 QA-014 A02；`CONTENT-003 / V7.7.0` 已冻结未来星系六方向 / 18 门课程矩阵，`CONTENT-001 / V7.7.15` 已冻结三星系六门代表课程，`BE-007 / V7.8.0` 最终候选 `d1fefcdc` 已集成教师证据发现和管理员课程状态最小接口。`FE-012 / V7.7.19` 已集成 hidden 深链告警返修，但 `QA-014 / V7.7.20` 的真实 9001 / SQLite / 外部 Edge 复验确认学生课程单元列表会省略 hidden，现有前端因而仍把 hidden 与不存在项同归 `course_unit_missing` 并输出应用 warning，A03a 继续 RETURN。`BE-011 / V7.8.2` 专业组提交 `35fa219` 已集成 student-only 精确单元访问处置，随后仍由原 FE-012 接线复验；`UI-003 / V7.9.4` 已集成管理员六领域治理与两个按权威角色加载的 owner，仍待真实浏览器和最终联调；`DATA-001 / V7.9.7` 已集成 API-only 本地三角色演示初始化器。FE-013 已获得 UI 共享注册表基线，仍等待 A03a 收束后登记写入；FE-017 仍等待 26 号视觉规格与完整概念图两次用户确认。当前交付目标仍是课程作业 / 设计大赛的本地展示版：优先课程互动、学生—教师—管理员协同、SQLite 数据对账和 9001 一键启动；真实 MySQL、公网 R6、隔离 runner、压力测试、真实课堂试点与 AI 能力后置。任务、责任、版本与门禁只认 `doc/02-项目规划.md`。

当前增量：`FE-012 / V7.7.19` 最终候选 `e27987d` 经四轮独立复审 P0—P3=0 后已集成，但 `QA-014 A03a / V7.7.20` 在真实 9001、外部 Edge、全新 SQLite 上最终以 P0=0、P1=0、P2=2、P3=1 退回；hidden 深链稳定输出含目标键的应用 warning，普通未知深链也在 warning 中回显 slug，Zoom 打开态跨桌面/移动 resize 会短暂沿用旧位图尺寸。`BE-011 / V7.8.2` 专业组提交 `35fa219` 已通过独立复审并集成不泄露单元内容的精确访问处置；`V7.7.22` 已唯一分配给原 FE-012，负责消费该接口、收束固定安全诊断和 Zoom 打开态重测，写入只允许在 `d6b6` 从 V7.9.10 控制点切换的新分支进行。`UI-003 / V7.9.4` 专业组提交 `70c6380` 经第七轮独立复审 P0—P3=0 后已集成；UI 写入关闭，真实浏览器验收并入后续精确 QA。`DATA-001 / V7.9.7` 专业组提交 `93d9205` 经第五轮独立复审 P0—P3=0 后已集成；只读对照 28 号演示契约后建立 `DATA-002 / V7.9.12`，只为 control-flow 追加 pending 作业样例，使教师证据、纠正、反馈和 runner 降级可在同一课程演示。所有更新仍以 `doc/02-项目规划.md` 为准。

DATA-001 已完成并集成；专业组保留在 `C:/Users/niu-h/.codex/worktrees/992d/工科实验室 / codex/data-v7.9-local-preview-demo @ 93d9205`，工作树 clean。`DATA-002 / V7.9.12` 复用该唯一长期组，从 V7.9.11 控制点建立 `codex/data-v7.9-teacher-demo-sample`，只为 `control-flow.loop-boundary` 增加 pending 作业样例及专项断言；仍不得新增模型、迁移、endpoint、service、router、直接数据库写入或修改前端 / UI / teacher / Future。
...
2. 项目对接模式已经启用。CONTENT、FE、BE、QA 已按标准标题恢复并登记独立 worktree；CONTENT 与 QA 只读，写入组只在收到主开发的精确任务 / 版本 / 文件包后写各自物理目录。BE-007 与 BE-011 已完成；QA-014 / V7.7.20 已在精确主线 SHA 上给出 A03a RETURN，下一写入归原 FE-012 / V7.7.22，在 `d6b6` 接入精确处置、固定诊断与 Zoom 打开态 resize。DATA-002 / V7.9.12 可在隔离 `992d` 并行补单课程演示样例；UI-003 / V7.9.4 已集成并关闭写入，同一任务只设一个主责组。
...
- `主开发` 及其任务分支使用 `Vx.y.z GROUP type(scope): 中文说明（TASK-ID）`；版本由 02 唯一分配。`V7.6.6`、`V7.6.7`、`V7.6.11`、`V7.6.12` 均为已取消的空预留，不得复用；`V7.6.15—16` 只属于 `PM-015`，`V7.7.0` 只属于 `CONTENT-003`，`V7.7.1—2`、`V7.7.6`、`V7.7.8`、`V7.7.10`、`V7.7.12`、`V7.7.17`、`V7.7.19` 与 `V7.7.22` 只属于 `FE-012`，`V7.7.3` 只属于 `FE-017` 的视觉规格提案而非产品实现，`V7.7.4` 只属于 `TOOLS-001`，`V7.7.5`、`V7.7.7`、`V7.7.9`、`V7.7.11`、`V7.7.16`、`V7.7.18`、`V7.7.20` 与 `V7.7.21` 只属于 `QA-014 A03a`；`V7.7.21` 仅补录最终回执，不是新一轮运行验收；`V7.8.0` 已由 `BE-007` 使用，`V7.8.2` 已由 `BE-011` 使用，均不得复用。
- 增量版本对账：`V7.7.13—14`、`V7.9.2—3`、`V7.9.5—6`、`V7.9.8—11` 属于 PM 控制面；`V7.9.9` 只固化用户离线协作阻断规则，`V7.9.10` 只登记 V7.7.22 的 FE 写入包，`V7.9.11` 只冻结 DATA-002 单课程补样；`V7.7.15` 属于 `CONTENT-001`，`V7.9.4` 唯一分配给 `UI-003`。
