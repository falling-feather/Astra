...
V6.6.63 后端阶段已经完成；V7.4 完成项目对接、注册 / 生命周期、认证与角色资源、组织治理、统一三星系总览、单一全局治理和 9001 一键入口；V7.5 完成代码空间、未来星系、课程编排、教师进度、OJ 诚实降级和全仓 review。V7.6 由 `主开发` 作为活动集成分支，`main@V7.5.11` 保持公开稳定。V7.6.4 已通过 QA-014 A01；V7.6.5 / BE-006 的权威学习证据与完成规则已通过 A02，前端 FE-012 仍是隔离工作树中的未提交候选。当前交付目标重定为课程作业 / 设计大赛的本地展示版：优先代表课程互动、学生—教师—管理员协同、SQLite 数据对账和 9001 一键启动；真实 MySQL、公网 R6、隔离 runner、压力测试和真实课堂试点后置。任务与恢复条件只认 `doc/02-项目规划.md`。
...
1. 当前任务是“星序 Astra｜主开发｜总控”，主开发集中分配后续小版本并独占 `主开发` 根工作区写入；`main` 只接收已收束的稳定版本，`houduan` 保留下一轮后端开发起点。
...
5. `qianduan` 旧 worktree 无唯一提交，由 `ARCH-002 / V7.6.9` 清理；同步前的 V7.2 未提交实验完整保存在本地标签 `archive/qianduan-wip-before-main-sync-20260727`，该标签只用于恢复和审计，不是开发入口。
...
- `主开发` 及其任务分支使用 `Vx.y.z GROUP type(scope): 中文说明（TASK-ID）`；版本由 02 唯一分配。`V7.6.6`、`V7.6.7` 是已取消的空预留，不得复用。
