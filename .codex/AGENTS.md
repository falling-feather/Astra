...
> **2026-08-22 当前控制点（覆盖下方历史阶段说明）**：文档控制基线为 `V8.0.37 / DOC-002`，产品运行字节仍以 V8.0.34 为未验候选。01 的详细技术内容已迁入 11—19，02 已重写为 V8.1—V8.6 的展示优先路线，03 已按 V4—V8 迁入 31—35，跨版本原文和旧 02 快照保存在 39。项目处于“用户 ↔ 主开发”单线态；侧栏中的后端组、架构组、前端组、内容组、QA 组五个长期对接任务均已归档，只保留“星序 Astra｜主开发｜总控”，协作代理树也仅剩 `/root`，不自动恢复。Git 只登记根工作树；七个脏现场已转为具名 stash，`u358@3734a2f` 独有提交由原分支保存，全部 66 条本地分支均保留。开发默认仍由主开发单线推进，优先演示导航、师生平台、双旗舰课、内容补面和竞赛包装；只有用户明确要求并行时才新建最小工作组、对接任务或 worktree。未来路线只认 `doc/02-更新规划.md`，历史恢复锚点只认 03 的 V8 分卷。
...
- `doc/02-更新规划.md`：V8.1—V8.6 中版本、小版本、T0/T1/T2、风险和长期方向的唯一规划主文档。
...
9. 当前恢复优先级固定为：最新候选浏览器终验 → 演示主线/导航 → 师生平台 → 双旗舰课内容与动画 → 次级内容补面 → 演示包装 → 最终交付检查。深层安全、额外加密、真实 MySQL、生产部署、极限性能、微服务/框架重写、通用 OJ 与全量统一内核均后置，除非直接阻断可见演示。
