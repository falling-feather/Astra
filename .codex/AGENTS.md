...
V6.6.63 后端阶段已经完成；V7.4 完成项目对接、注册 / 生命周期、认证与角色资源、组织治理、统一三星系总览、单一全局治理和 9001 一键入口；V7.5 完成代码空间、未来星系、课程编排、教师进度、OJ 诚实降级和全仓 review。V7.6 由 `主开发` 作为集成分支，`main@V7.5.11` 保持公开稳定。V7.6.4 已通过 QA-014 A01；V7.6.5 / BE-006 的权威学习证据与完成规则已通过 A02；V7.6.10 / PM-012 确认“可扩展多星系互动教学平台 + 三角色协同 + 后置 AI 路线”；V7.6.13 / PM-013 是用户要求的暂停控制点，V7.6.14 / PM-014 只登记未来星系“主页面 → 方向目录 → 独立课程”重构问题。前端、后端、QA 长期组均已解散，FE-012 未提交候选只保存在 detached 隔离工作树；V7.6.11 / V7.6.12 失活但保留，不能视为继续开发授权。当前交付目标仍是课程作业 / 设计大赛的本地展示版：优先代表课程互动、学生—教师—管理员协同、SQLite 数据对账和 9001 一键启动；真实 MySQL、公网 R6、隔离 runner、压力测试、真实课堂试点与 AI 能力后置。任务、暂停状态与恢复条件只认 `doc/02-项目规划.md`。
...
1. 当前“星序 Astra｜主开发｜总控”处于暂停状态；除用户明确要求的控制面收束外，不得在 `主开发` 根工作区继续产品开发、测试集成或发布。`main` 只接收已收束的稳定版本，`houduan` 保留下一轮后端开发起点。
2. 项目对接模式已经停用，原前端、后端、QA 长期任务均已归档且没有写入权限。恢复协作必须先取得用户明确指令，再在 `doc/02-项目规划.md` 登记唯一责任组、独立 worktree、允许路径和版本令牌；同一任务只设一个主责组。
...
6. 后端 `dafe` 工作树已安全移除；前端 FE-012 的唯一未提交候选保存在 detached `C:/Users/niu-h/.codex/worktrees/a02c/工科实验室`，恢复决策前不得删除、覆盖、提交或当作稳定实现。`codex/team-be` 与 `codex/team-fe` 仅作历史 / 候选审计锚点，保留分支不表示工作组仍然存在。
...
9. 未来星系层级重构目前只记录在 `doc/02-项目规划.md` 第 1.4 节；用户未明确说“开始修改”前，不得修改未来星系源码、路由、清单、资源、测试或 detached 候选工作树，也不得创建实现任务或恢复工作组。
...
- `主开发` 及其任务分支使用 `Vx.y.z GROUP type(scope): 中文说明（TASK-ID）`；版本由 02 唯一分配。`V7.6.6`、`V7.6.7` 是已取消的空预留，不得复用；`V7.6.13` 是当前暂停控制点，`V7.6.11 / V7.6.12` 在恢复登记前不得使用。
