...
当前增量：`FE-012 / V7.7.22` 专业组提交 `5db1112ea06256b9a4d15d595adf1383e1d15f2b`（父 `a2878e0`）已在三轮独立复审 P0—P3=0 后集成。现有 `/units` 精确命中 open / locked 时零探针，零匹配才消费 BE-011；hidden / locked 静默归一，missing / ordinary unknown 只发不含 slug 的固定诊断，known / unknown pending 均有同步取消 owner，并以 controller identity + generation 阻断 reset 后同 slug 重开的 ABA 迟到覆盖；Zoom 打开态按当前父容器 / DPR 立即重建。该结论仍是代码与动态合同基线，不代替真实 9001 / SQLite / 外部 Edge 的 QA-014 A03a。`UI-003 / V7.9.4` 专业组提交 `70c6380` 经第七轮独立复审 P0—P3=0 后已集成；UI 写入关闭，真实浏览器验收并入后续精确 QA。`DATA-002 / V7.9.12` 专业组提交 `a920642` 经独立复审 P0—P3=0 后已集成，只为 control-flow 追加 pending 作业样例，使教师证据、纠正、反馈和 runner 降级可在同一课程演示；数据组写入关闭。所有更新仍以 `doc/02-项目规划.md` 为准。
...
2. 项目对接模式已经启用。CONTENT、FE、BE、QA 已按标准标题恢复并登记独立 worktree；CONTENT 与 QA 只读，写入组只在收到主开发的精确任务 / 版本 / 文件包后写各自物理目录。BE-007、BE-011、DATA-002 与 FE-012 / V7.7.22 已完成当前增量；FE、BE、DATA、UI 写入均已关闭，下一动作是 QA-014 在新的精确主线 SHA 上复验 A03a。同一任务只设一个主责组，失败仍退回原 FE-012。
