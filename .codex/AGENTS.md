...
2026-07-31 用户随后以第二轮 10 条浏览器批注恢复 `FE-021 / V7.9.69` 单任务。主开发在根工作区完成学生 rail 232px↔76px 收缩、身份摘要贴底、stage inline-size 单 / 双栏、班课内存选择恢复、学生授权顶栏 / 搜索裁剪、五学科概览删项与 Englab Hero 复活；Router 保持冻结 1116 行，资源代际为 `20260731v7969StudentUiP0`。用户要求的全课程与部分课程两套师生账号必须在 FE-021 固定提交后另建唯一 DATA 任务，通过正式 API 创建，不得把数据库运行态混入前端提交。
