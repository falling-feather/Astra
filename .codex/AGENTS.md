...
QA-014 / V7.7.23、V7.7.24 的 BLOCKED / NOT-RUN 现场与 V7.7.26 产品 RETURN 证据保持不变。V7.7.29 使用 detached `q28a@2d6a065` 完成当时既定 A03a 矩阵；其后从保留的 V7.7.28 现场补充回传了可重复的 Zoom 移动→桌面保持打开 P2。V7.7.30 已集成。V7.7.31 在 clean detached `qz31@944855e` 完成身份、10 项定向合同和文档 / Git 门禁，但完整 npm 遇 `spawnSync git EPERM`，唯一官方启动又因指定 `D:/Python/python.exe` 无法满足 `alembic==1.18.5` 锁依赖而在 data / SQLite / 9001 / Browser 前失败；最终 BLOCKED / NOT-RUN、P0—P3=N/A。V7.7.32 唯一续验同一 Zoom 切片；clean detached `qz32@88de17c6aa8e2a9e589d11d05d8f2415f47f6b88` 已创建，产品 `30f4dbd` 为祖先、`.venv` 不存在、9001=0，固定使用已核验的根 `.venv` Python 3.12.12，不得重试 V7.7.31 的失败命令。旧证据与 TEMP 均只读保留；旧 `89e8`、`c91f`、`b56e`、`qz31` 与受保护 `a02c` 永久禁入。
...
2. 项目对接模式已经启用。CONTENT、FE、BE、QA 已按标准标题恢复；QA 只读，写入组只在收到主开发的精确任务 / 版本 / 文件包后写各自物理目录。CONTENT-004 / V7.8.3、DATA-003 / V7.9.19 与 BE-013 / V7.8.4 均已完成并暂停。QA-013 / V7.8.6 已 RETURN FE-013 并释放 Browser / 9001；qe13 与唯一 TEMP 只读保留。FE-012 的 fz30 已提交 `30f7f189` 并关闭写入；QA-014 / V7.7.31 已 BLOCKED / NOT-RUN，qz31 与唯一 TEMP 只读冻结；V7.7.32 的 clean detached `qz32@88de17c` 已由 V7.9.33 回填，完整 `[ASSIGN]` 前不得运行。FE-013 / V7.8.7 已由 ft87 提交 `15c0e831`、独立复审 PASS 并进入集成线，ft87 clean 且写入关闭；真实运行复验仍须新 QA 包。q28a、c04e、da03、be13、fe13、qe13、qz31、qz32、ft87、fz30 与外部证据各守任务边界。原长期任务处于 systemError 时，只允许主开发登记的边界明确临时执行者接续同一任务编号，不创建竞争任务标题；不得复用旧 d6b6 / f27a / b56e。
...
- `主开发` 及其任务分支使用 `Vx.y.z GROUP type(scope): 中文说明（TASK-ID）`；版本由 02 唯一分配。`V7.6.6`、`V7.6.7`、`V7.6.11`、`V7.6.12` 均为已取消的空预留，不得复用；`V7.6.15—16` 只属于 `PM-015`，`V7.7.0` 只属于 `CONTENT-003`，`V7.7.1—2`、`V7.7.6`、`V7.7.8`、`V7.7.10`、`V7.7.12`、`V7.7.17`、`V7.7.19`、`V7.7.22`、`V7.7.27` 与 `V7.7.30` 只属于 `FE-012`，`V7.7.3` 只属于 `FE-017` 的视觉规格提案而非产品实现，`V7.7.4` 只属于 `TOOLS-001`，`V7.7.25` 只属于 `TOOLS-002`；`V7.7.5`、`V7.7.7`、`V7.7.9`、`V7.7.11`、`V7.7.16`、`V7.7.18`、`V7.7.20`、`V7.7.21`、`V7.7.23`、`V7.7.24`、`V7.7.26`、`V7.7.28`、`V7.7.29`、`V7.7.31` 与 `V7.7.32` 只属于 `QA-014 A03a`。V7.7.30 只修复 Zoom 保持打开的移动→桌面稳定同步；V7.7.31 因指定 Python 锁依赖阻断未形成产品判断，V7.7.32 只续验该修复和必要非回归。`V7.8.0` 已由 `BE-007` 使用，`V7.8.2` 已由 `BE-011` 使用，`V7.8.3` 唯一分配给 `CONTENT-004`，`V7.8.4` 唯一分配给 `BE-013`，`V7.8.5` 唯一分配给 `FE-013`，`V7.8.6` 唯一分配给 `QA-013`，`V7.8.7` 唯一分配给 `FE-013` 的挂接状态竞态返修，均不得复用。`V7.8.8` 尚未分配，只能在 V7.8.7 形成精确集成候选后重新查重并由主开发发放。`BE-012` 只属历史预提及、从未发卡，永久不分配。
- 增量版本对账：`V7.7.13—14`、`V7.9.2—3`、`V7.9.5—6`、`V7.9.8—11` 与 `V7.9.13—18`、`V7.9.20—33` 属于 PM 控制面；`V7.9.9` 只固化用户离线协作阻断规则，`V7.9.10` 只登记 V7.7.22 的 FE 写入包，`V7.9.11` 只冻结 DATA-002 单课程补样，`V7.9.13` 只登记 V7.7.23 的 QA 运行包，`V7.9.14` 只登记 TOOLS-002 写入包与 V7.7.26 QA 续验，`V7.9.15` 只登记 V7.7.26 的精确 QA 工作树与运行包，`V7.9.16` 只登记 V7.7.28 的精确 QA 工作树与修正版运行包，`V7.9.17` 只登记 V7.7.29 的全新 runtime 续验，`V7.9.18` 只登记 CONTENT-004 写入包，`V7.9.20` 只登记 BE-013 / DATA-003 并行任务卡，`V7.9.21` 只回填两组独立 worktree 与派单前门禁，`V7.9.22` 只登记 FE-013 / V7.8.5 写入包，`V7.9.23` 只回填 FE-013 独立 worktree 实态，`V7.9.24` 只登记 QA-013 / V7.8.6 运行包，`V7.9.25` 只回填 QA-013 独立工作树实态，`V7.9.26` 只登记 FE-012 / V7.7.30 返修包，`V7.9.27` 只回填 Zoom FE 工作树，`V7.9.28` 只登记 FE-013 / V7.8.7 返修包，`V7.9.29` 只回填教师挂接返修工作树，`V7.9.30` 只登记 V7.7.31 Zoom QA 运行包，`V7.9.31` 只回填该 QA 工作树，`V7.9.32` 只记录阻断并登记 V7.7.32 续验，`V7.9.33` 只回填 qz32 实态。`V7.7.15` 属于 `CONTENT-001`，`V7.9.4` 唯一分配给 `UI-003`。
