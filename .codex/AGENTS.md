...
BE-013 / V7.8.4 与 DATA-003 / V7.9.19 已由 V7.9.20 建卡，并由 V7.9.21 回填独立树：BE 使用 `C:/Users/niu-h/.codex/worktrees/be13/工科实验室` / `codex/be-v7.8-teacher-evidence-facts`，只在现有教师 evidence 读取链接入 activity / schema / event / exact leaf 有限投影；DATA 使用 `C:/Users/niu-h/.codex/worktrees/da03/工科实验室` / `codex/data-v7.9-teacher-evidence-producers`，只对齐 manifest / initializer 的 physics 完整、control-flow 浅层及四课 generic fallback producer profile。两树基线均为 `ddc186d` 且 clean；`BE-012` 只有历史预提及、未正式发卡，永久不分配。
...
2. 项目对接模式已经启用。CONTENT、FE、BE、QA 已按标准标题恢复；QA 只读，写入组只在收到主开发的精确任务 / 版本 / 文件包后写各自物理目录。QA-014 / V7.7.29 与 CONTENT-004 / V7.8.3 均已完成并暂停；q28a、c04e 与外部证据只保留审计。BE-013 与 DATA-003 的独立 worktree 已登记，只有对应 owner 在完整 `[ASSIGN]` 后可写各自 allowlist；不得跨树或交叉实现。原长期任务处于 systemError 时，只允许主开发登记的边界明确临时执行者接续同一任务编号，不创建竞争任务标题；不得复用旧 d6b6 / b56e。
