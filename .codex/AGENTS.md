...
1. 当前项目对接模式保持停用，CONTENT、FE、QA、BE、UI、DATA 长期工作组继续解散；只恢复“星序 Astra｜主开发｜总控”在 `主开发` 根工作区对 `FE-021 / V7.9.69` 的单写。`main` 只接收已收束的稳定版本，`houduan` 保留下一轮后端开发起点。
2. 其他未完成专业任务和工作树继续只读保全。V7.8.9 / fr89、FE-017 / f017、UX-001 / ux64 与 UI-003 / u358 均不得因本轮恢复而写入、复制、集成或清理；QA-014、QA-015 也未恢复。FE-021 只消费 `主开发@5dbea93`，其完成不替代这些任务的独立复审或 QA 结论。
...
- 单任务恢复增量：`V7.9.67` 只记录暂停与停组；`V7.9.68` 唯一分配给 `V7.9.68 FE feat(student-flow): 收束入班与课程发现体验（FE-020）`；`V7.9.69` 唯一分配给 `V7.9.69 FE fix(student-ui): 收束学生工作台与授权发现（FE-021）`。两项只由主开发在根工作区写入，不恢复专业工作组或消费冻结候选。
