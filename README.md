...
- 当前产品代码基线为 `bace1b4`：`V8.4.6` 通用课程闭环已经收口，原 `V8.1.5 / UI-015` 登录前作品导览候选也已正式整合。本轮文档提交完成后，本地 `main` 与 `主开发` 以快进方式保持同一控制点；其他历史分支只作审计锚点。
- 登录前导览、Cookie Session、教师申请、课程创建与审核、学生选课、多教师共享草稿、不可变发布、三种完成结果和三角色工作台均已进入当前实现。
- 三个学习空间合计 127 项正式活动：工科试验室 90、代码空间 18、未来星系 19；课程可以引用这些活动，但不改变既有实验本体，也不划分精品与普通等级。
- Alembic 当前唯一 head 为 `20260825_0058`。V8.4.6 全量验收收集 704 项后端测试并运行至 100% 无失败；整合 UI-015 后，当前基线的 274 个受跟踪 JavaScript 文件和 77 项前端合同全部通过。隔离 runner、生产部署和真实课堂实证仍不在当前完成口径内。
- 当前作品定位为毕业设计 / 多媒体竞赛本地展示候选：下一轮优先强化管理驾驶舱、课程版本演进和可解释学习证据，不以新增实验数量或生产级安全细节抬高完成度。
...
| [`doc/02-更新规划.md`](doc/02-更新规划.md) | 已完成周期、下一周期候选方向、优先级与长期暂缓事项 |
