...
- 当前已提交代码基线为 `主开发@b886829 / V8.6.6`。2026-09-05 起正在基于该版本审查与改进 Web 和后端设计；本轮工作区改动尚未发布。
...
- Alembic 当前唯一 head 为 `20260825_0058`。最新工作区验证结果见 [当前任务](doc/02-更新规划.md#3-当前任务状态)，历史验收见 [发布历史](doc/03-发布历史.md)。隔离 runner、生产部署和真实课堂实证仍不在当前完成口径内。
- 当前作品服务毕业设计、大作业与展示类竞赛；先打磨 Web 交互和后端架构，微信小程序暂不设计。重构依据、3D 试点和实施顺序见 [架构审查与重构路线](doc/07-子文档/71-Web与后端架构审查与重构路线.md)。
