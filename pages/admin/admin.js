(function () {
    ...
    const ADMIN_ASSET_VERSION = '20260828v864RoleWorkbenchHarmonyP0';
    ...
    function roleWorkbench(command) { const invoke = () => window.AstraRoleWorkbenchBridge[command]({ role: 'admin', root: state.root, getBaseUrl: () => state.apiBase, isActive: () => state.active && state.user && state.user.role === 'admin', refreshIcons }); if (window.AstraRoleWorkbenchBridge) return Promise.resolve(invoke()); if (command !== 'refresh') return Promise.resolve(false); return import('../../shared/js/role-workbench-bridge.js?v=20260828v864RoleWorkbenchHarmonyP0').then(invoke).catch(() => false); }
    function initAdmin() {
