...
>
> **当前基线**：`main@V7.5.11` 是公开稳定线；`主开发` 当前集成线包含 V7.7.4 控制面、V7.7.2 / FE-012 首批共享证据实现、V7.7.6 的发布态 / authority / 44px 返修、V7.7.8 的移动浮层避让、V7.7.10 的预测确认与折叠焦点返修、V7.7.12 的移动端力学裁切返修，以及 V7.7.17 的 Escape 路由逸出返修。V7.6.5 的 Alembic 0051、版本化完成规则、幂等学习事实、活动 / 续学投影、教师聚合和追加式纠错已通过 QA-014 A02；当前后端另提供教师逐学生证据安全摘要分页，以及管理员课程状态受约束 PATCH、影响计数与审计回读，均复用既有模型且不新增迁移。QA-014 A03a / V7.7.18 已确认 Escape、双视口力学内宽、Canvas / DPR、Zoom、44px、发布态、学习证据与资源释放通过；唯一 P2 是 authoritative hidden 冷启动正确失败关闭并归一到 `#physics` 时仍写入一次预期未知模块 warning。V7.7.19 当前待集成候选只在健康 registry 成功确认目标缺键后增加权威发布态分类并收窄该诊断边界，普通未知、registry 基础设施故障与畸形模块继续失败关闭。当前交付目标是 `astra-local.ps1 + SQLite + 127.0.0.1:9001` 的课程作业 / 设计大赛展示版，优先课程互动与学生—教师—管理员协同；域名、公网 R6、真实 MySQL、隔离 runner、压力测试、真实课堂试点和 AI 能力保留为后置工程。
>
...
- Future 的 6 方向 / 18 activity key 只登记到 catalog；本批没有 Future 页面、Bridge 或互动运行接线。V7.7.18 已确认 V7.7.17 Escape 返修及既有双视口、发布态、证据和资源门禁通过，只因 authoritative hidden 冷启动的预期拒绝 warning 而 RETURN；V7.7.19 仍是待独立复审与集成的最小候选，Future A03b 继续等待后续实现。
