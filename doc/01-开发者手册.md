...
>
> **当前基线**：`main@V7.5.11` 是公开稳定线；`主开发` 当前集成线包含 V7.7.4 控制面、V7.7.2 / FE-012 首批共享证据实现、V7.7.6 / FE-012 A03a 原范围返修和 V7.7.8 / FE-012 浮层避让返修。V7.6.5 的 Alembic 0051、版本化完成规则、幂等学习事实、活动 / 续学投影、教师聚合和追加式纠错已通过 QA-014 A02；V7.7.0—1 冻结内容 / 路由与 FE-012 选择性重放契约，V7.7.2 只接入两个非 Future 代表活动、共享证据基础设施和最小角色回读。V7.7.5 / A03a 首轮因 locked 深链与工科 44px 触达退回；V7.7.6 已集成定向返修。V7.7.7 独立复验确认 locked、班级 authority、44px 几何、教学、权限与账本通过，但精确 390×844 下固定评分卡和 FAB 仍遮挡放大按钮，A03a 因单一 P2 再次退回。V7.7.8 已把力学评分卡移入实验文档流、重排移动 dock 与侧栏模态层并补齐 ripple 减少动效，本地两视口命中验证通过，仍等待独立 QA，不能写成 FE-012、未来星系或三星系整体完成。当前交付目标是 `astra-local.ps1 + SQLite + 127.0.0.1:9001` 的课程作业 / 设计大赛展示版，优先课程互动与学生—教师—管理员协同；域名、公网 R6、真实 MySQL、隔离 runner、压力测试、真实课堂试点和 AI 能力保留为后置工程。
>
