...
>
> **当前基线**：业务代码沿用 2026-09-08 收口；正式前端为 `qianduan/`，FastAPI 为业务权威。2026-09-16 已进入持续开发：资源与模板、独立教学分叉、同步送审及冻结发布已完成本机功能联调；版本化检查点、作业重交与评分留档已接入；实验运行和后续查询/任务按 02 推进。
>
> **最后一次更新时间**：2026-09-17
>
> **更新者**：主开发组
...
权威接口注册位于 [router.py](../backend/app/api/router.py)，DTO 在 [schemas](../backend/app/schemas/)，数据库模型在 [models](../backend/app/models/)。当前基础教学保留 `/api` 与 `/api/v1`；`/api/v2` 已提供资源、课程草稿、派生、同步、候选审核、发布、媒体、学习上下文和结果历史接口；实验运行对接及后续治理按 02 继续实施。
...
既有运行已固定课程、单元、用户、运行和规则坐标；统一学习上下文与不可变发布绑定仍由 D 批次接入。学习者命令序号与服务端事实序号是两种游标：服务端派生事件会增加事实序号，但不占用下一次学习者命令序号。幂等重放保持原事实位置，不能把两种序号合成一个计数器。
...
实体表与外键以模型为准；当前唯一迁移 head 是 `20260916_0064`。0061 增加资源版本、独立课程家族、单元来源、修订/修改批次/候选/审核项和幂等发布回执表；旧课程逐门建立独立根，旧发布的候选引用保持空，不补造审核。0062 分离课程设置草稿与通用操作回执，0063 保存不可变媒体文件及课程取用关系。0064 增加学习上下文、作业尝试、评分历史和结果认定；旧证据与投影保留来源，新的发布完成统计读取统一结果索引。实验运行的接线仍在推进，不能把旧投影直接当作新版完成状态。

### 6.4 本轮课程工作流

2026-09-16 的当前工作区已建立 v2 课程草稿、独立教学分叉、公共内容同步预览、候选审核、冻结发布与历史恢复接口；正式页面已完成三角色真实 API 联调，开发提交与验证范围见 [03 本阶段记录](03-发布历史.md#104-课程创作同步审核与冻结发布)。这不表示第 13 章全部目标已经完成。任务状态以 [02 当前任务状态](02-更新规划.md#3-当前任务状态) 为准。
...
- 草稿设置、通用回执与文件存储分别由 0062、0063 迁移承接；已有新数据时拒绝有损降级。尚未运行本轮真实 MySQL 迁移与并发验收。

结果沿用与补做现已接通检查点、作业和成绩回读；实验运行、跨课程关联与统一汇总继续按 D—E 批次实施。
...
0064 能恢复旧提交与审计中可确认的分数；未知旧发布、满分及已脱敏反馈继续标为未知。出现新历史事实后拒绝有损回退。学习结果与学生自己的历史回读已经验证；声明参数运行已由下一节接入；退出课程后的历史目录入口及真实 MySQL/并发验收仍有后续工作，不能据本节宣称整个 D—F 阶段完成。
...
[ActivitySession](../qianduan/src/domain/activity-session.ts)串行发送并保留原请求重试；离页释放监听器与未发送内存队列，不把观察或答案持久化至普通浏览器存储。[CoursePlayer](../qianduan/src/portal/course-player.ts)对课程中的原实验入口执行完整页面导航，不再在课程页内嵌套 iframe；导航地址带有本次学习上下文和返回位置，实验完成后可用浏览器后退返回课程。原求解器不承担教学授权或评分。原力学重力滑杆的步长已由 50 改为 10，以避免默认 980 被浏览器归整成 1000；原保护基线不变，有独立的一致性回归与精确变更记录。

2026-09-17 用户主动暂停。上述主链路已验证，暂停前的最后页面复看和其余阶段仍未结束，实际范围见 [03 暂停快照](03-发布历史.md#106-实验操作版本绑定与暂停快照)，恢复安排只认 02。
...
2026-09-08 已完成真实建课、审核发布、学生提交、教师批改、重启后回读成绩、笔记与检查点完成回读。具体证据、测试计数和部署提交只在 [03 本阶段记录](03-发布历史.md#8-2026-09-07-正式门户接入与前后端封装) 保存；它们不是后续任一修改的自动验收结果。2026-09-16 新课程工作流的定向验证与浏览器证据见 [03 第 10.4 节](03-发布历史.md#104-课程创作同步审核与冻结发布)。
...
已接通课程家族、同源同步、候选审核与文件服务；检查点与作业的跨版本认定、补做和评分历史已接通。声明参数的实验运行已接入；退出课程后的历史目录、校内课程发现、统一跨课程路径及后续统计/任务尚未完成。公网业务后端、真实课堂效果、正式隔离执行与完整移动端精调未作为已完成能力。
...
本章按项目负责人要求展开完整目标架构与七组设计图。资源目录/模板预览与部分存储模型已落实，现状见前文，其余仍按 02 待实施任务推进。第 1—12 章及其实现子文档继续描述可核实的当前系统；本章展示完整目标：资源、谱系、同步和候选发布已进入开发提交，学习结果认定与查询/任务等部分仍在推进；图本身不表示全部上线。业务规则、正式任务和验收条件只在 [02 完整实施方案](02-更新规划.md#2-完整实施方案) 与 [当前任务状态](02-更新规划.md#3-当前任务状态) 维护。
