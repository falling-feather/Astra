...
>
> **当前基线**：业务代码沿用 2026-09-08 收口；正式前端为 `qianduan/`，FastAPI 为业务权威。本文的 2026-09-16 整理没有新增业务功能或课程。
>
> **最后一次更新时间**：2026-09-16
>
> **更新者**：主开发组
...
权威接口注册位于 [router.py](../backend/app/api/router.py)，DTO 在 [schemas](../backend/app/schemas/)，数据库模型在 [models](../backend/app/models/)。当前同时保留 `/api` 和 `/api/v1`；`/api/v2` 是目标设计。
...
实体表与外键以模型为准；当前唯一迁移 head 是 `20260908_0060`。旧事件/进度与 evidence/runtime/projection 并存仍是技术债，后续应迁移到统一语义，而非建立另一份完成真值。
...
目前没有“登记一次就自动接入”的统一插件机制。纯前端子站可通过入口和资源适配承接，完整教学接入仍需明确课程/单元/发布身份、权限和记录能力。适用学段、难度与先修关系也不能都塞进星系名称。
