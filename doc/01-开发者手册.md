...
>
> **当前基线**：`main@V7.5.11` 是公开稳定线；`主开发` 当前集成线包含 V7.7.4 控制面、V7.7.2 / FE-012 首批共享证据实现、V7.7.6 的发布态 / authority / 44px 返修、V7.7.8 的移动浮层避让、V7.7.10 的预测确认与折叠焦点返修，以及 V7.7.12 的移动端力学裁切返修。V7.6.5 的 Alembic 0051、版本化完成规则、幂等学习事实、活动 / 续学投影、教师聚合和追加式纠错已通过 QA-014 A02；当前后端另提供教师逐学生证据安全摘要分页，以及管理员课程状态受约束 PATCH、影响计数与审计回读，均复用既有模型且不新增迁移。QA-014 A03a / V7.7.16 已确认双视口力学内宽、Canvas / DPR、Zoom、44px、发布态和学习证据通过；唯一 P1 是次级 FAB 获焦后 Escape 同时触发 FAB 折叠与模块关闭，导致从 `#physics/mechanics` 逸出到 `#physics` 且焦点落到 `body`，故 A03a RETURN。当前交付目标是 `astra-local.ps1 + SQLite + 127.0.0.1:9001` 的课程作业 / 设计大赛展示版，优先课程互动与学生—教师—管理员协同；域名、公网 R6、真实 MySQL、隔离 runner、压力测试、真实课堂试点和 AI 能力保留为后置工程。
>
...
- Future 的 6 方向 / 18 activity key 只登记到 catalog；本批没有 Future 页面、Bridge 或互动运行接线。V7.7.16 已确认移动端力学裁切修复通过，但因折叠 dock 的 Escape 路由逸出 P1 而 RETURN；Future A03b 继续等待后续实现。
...
它在启动 Uvicorn 前通过 `create_app()` 的 ASGI HTTP 边界调用现有认证、学校/班级、课程/单元、发布计划、学习证据、作业/提交和审计 API，建立固定的 `admin/teacher/student`、14 门课程与 42 个单元。初始化器只接受本机 development + SQLite + `127.0.0.1` 环境；首次生成的强密码仅在终端显示一次，既有账号重跑时隐藏输入校验，不保存、不回显到报告或日志。所有写入先 GET/规范化比较，漂移、非预期 409、非本机数据源或初始化失败均保持失败关闭语义；该候选的动态漂移/路径/凭据门禁与第三次独立复审完成前，不标记为退出完成。初始化器不直接访问模型、迁移、数据库表或 SQL；`runner_unavailable` 是代码提交的诚实结果。
