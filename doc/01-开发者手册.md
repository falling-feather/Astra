...
>
> **当前基线**：`main@V7.5.11` 是公开稳定线；`主开发@V7.6.10` 包含 V7.6.5 的 Alembic 0051、版本化完成规则、幂等学习事实、活动 / 续学投影、教师聚合和追加式纠错，并已完成 V7.6.8/9 展示版范围重定与仓库瘦身。V7.6.10 只同步已确认的产品构型、角色边界和 AI 后置路线，没有改变运行代码。V7.6.5 已通过 QA-014 A02；FE-012 的三星系客户端接入仍是隔离工作树中的未提交候选，不能写成稳定实现。当前交付目标是 `astra-local.ps1 + SQLite + 127.0.0.1:9001` 的课程作业 / 设计大赛展示版，优先代表课程互动与学生—教师—管理员协同；域名、公网 R6、真实 MySQL、隔离 runner、压力测试、真实课堂试点和 AI 能力保留为后置工程。
>
...
- 当前活动集成分支为 `主开发`，由主开发单写并按 02 分配 V7.6.x；`main@V7.5.11` 保持公开稳定，`houduan` 保留下轮后端起点，`review` 冻结。前端、后端和 QA 长期组当前均为 HOLD；只有 02 登记唯一任务、隔离工作树、允许路径和版本令牌后才能恢复，门禁通过后由主开发集成。
