...
>
> **当前基线**：`main@V7.5.11` 是公开稳定线；`主开发` 当前集成线包含 V7.7.2 / FE-012 首批共享证据实现及其 A03a 返修。V7.7.20 的真实 9001 / SQLite / 外部 Edge 复验证明学生课程单元列表省略 hidden，现有集成线因而无法区分 hidden 与 missing，普通未知 warning 仍回显 slug，Zoom 打开态跨桌面/移动 resize 也会短暂沿用旧位图尺寸；A03a 最终以 P0=0、P1=0、P2=2、P3=1 RETURN。V7.6.5 的 Alembic 0051、版本化完成规则、幂等学习事实、活动 / 续学投影、教师聚合和追加式纠错已通过 QA-014 A02；V7.8.0 另提供教师逐学生证据安全摘要分页，以及管理员课程状态受约束 PATCH、影响计数与审计回读，均复用既有模型且不新增迁移。V7.8.2 已为学生已知 deep-link 集成精确单元访问处置，只返回 open / hidden / locked / missing 的最小二字段结果，不扩张课程目录。V7.7.22 当前未提交候选只在 `/units` 精确零匹配后消费该处置，以固定诊断收束 hidden / locked / missing，并让 Zoom 打开态按当前父容器与 DPR 立即重建；仍待主开发复审、集成与 QA。V7.9.4 已把管理台收束为六领域、严格首页 DTO、课程状态可信回读、全局写锁与两个 admin-only owner；第七轮独立代码 / 合同复审 P0—P3=0，真实浏览器和完整三角色演示联调仍待后续精确 QA。当前交付目标是 `astra-local.ps1 + SQLite + 127.0.0.1:9001` 的课程作业 / 设计大赛展示版，优先课程互动与学生—教师—管理员协同；域名、公网 R6、真实 MySQL、隔离 runner、压力测试、真实课堂试点和 AI 能力保留为后置工程。
>
...
- Future 的 6 方向 / 18 activity key 只登记到 catalog；本批没有 Future 页面、Bridge 或互动运行接线。V7.7.20 已确认 V7.7.19 的 hidden 分类受真实学生单元列表过滤边界阻断；BE-011 后端处置已集成，V7.7.22 目前仅为未提交 FE-012 接线候选，完整 FE-012、集成后 QA 与 Future A03b 继续等待后续验证。
