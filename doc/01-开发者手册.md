...
>
> **当前基线**：`main@V7.5.11` 是公开稳定线；`主开发` 当前集成线包含 V7.7.4 控制面、V7.7.2 / FE-012 首批共享证据实现、V7.7.6 的发布态 / authority / 44px 返修、V7.7.8 的移动浮层避让和 V7.7.10 的预测确认与折叠焦点返修。V7.6.5 的 Alembic 0051、版本化完成规则、幂等学习事实、活动 / 续学投影、教师聚合和追加式纠错已通过 QA-014 A02；V7.7.10 已实现预测命令精确回执、跨标签身份、共享队列可恢复背压以及 dock / 导出菜单焦点闭环，两条独立提交审查均为 P0—P3=0。QA-014 A03a / V7.7.11 已完成真实 9001、SQLite、双视口、Tab、console 与角色权限复验，并因精确 390×844 下力学内容内部仍约 896px 宽、暂停与放大中心被根级裁切而 RETURN；V7.7.12 当前是待主开发集成候选，集成后才分配下一轮 QA。当前交付目标是 `astra-local.ps1 + SQLite + 127.0.0.1:9001` 的课程作业 / 设计大赛展示版，优先课程互动与学生—教师—管理员协同；域名、公网 R6、真实 MySQL、隔离 runner、压力测试、真实课堂试点和 AI 能力保留为后置工程。
>
...
- Future 的 6 方向 / 18 activity key 只登记到 catalog；本批没有 Future 页面、Bridge 或互动运行接线。V7.7.10 已进入 `主开发` 集成线，QA-014 A03a / V7.7.11 已因精确 390×844 力学裁切 RETURN，V7.7.12 当前是待集成候选且集成后才分配下一轮 QA；Future A03b 继续等待后续实现。
