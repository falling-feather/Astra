...
>
> **当前基线**：`main@V7.5.11` 是公开稳定线；`主开发` 当前集成线包含 V7.7.2 / FE-012 首批共享证据实现及其 A03a 返修。V7.7.20 的真实 9001 / SQLite / 外部 Edge 复验证明学生课程单元列表省略 hidden，旧集成线因而无法区分 hidden 与 missing，普通未知 warning 仍回显 slug，Zoom 打开态跨桌面/移动 resize 也会短暂沿用旧位图尺寸；A03a 最终以 P0=0、P1=0、P2=2、P3=1 RETURN。V7.6.5 的 Alembic 0051、版本化完成规则、幂等学习事实、活动 / 续学投影、教师聚合和追加式纠错已通过 QA-014 A02；V7.8.0 另提供教师逐学生证据安全摘要分页，以及管理员课程状态受约束 PATCH、影响计数与审计回读，均复用既有模型且不新增迁移。V7.8.2 已为学生已知 deep-link 集成精确单元访问处置，只返回 open / hidden / locked / missing 的最小二字段结果，不扩张课程目录。V7.7.22 已在三轮独立复审 P0—P3=0 后集成：只在 `/units` 精确零匹配后消费该处置，以固定诊断收束 hidden / locked / missing，并让 Zoom 打开态按当前父容器与 DPR 立即重建；真实 QA-014 A03a 仍待执行。V7.9.4 已把管理台收束为六领域、严格首页 DTO、课程状态可信回读、全局写锁与两个 admin-only owner；第七轮独立代码 / 合同复审 P0—P3=0，真实浏览器和完整三角色演示联调仍待后续精确 QA。当前交付目标是 `astra-local.ps1 + SQLite + 127.0.0.1:9001` 的课程作业 / 设计大赛展示版，优先课程互动与学生—教师—管理员协同；域名、公网 R6、真实 MySQL、隔离 runner、压力测试、真实课堂试点和 AI 能力保留为后置工程。
>
> **当前任务入口**：V7.6—V8.0 展示闭环、FE-012 接入、教师 / 管理工作流和展示后 AI / 发布任务只在 [`02-项目规划.md`](02-项目规划.md) 维护；本文不复制任务状态。
>
> **具体规划法则**：
>
...
学生 AI 助教、教师 AI 教学助手、AI 维护 / 管理助手已经作为展示后条件路线登记在 [`02-项目规划.md`](02-项目规划.md)，但当前代码没有 provider、AI 对话入口或 AI 写入链，`shared/js/product-capabilities.js` 中 `ai-tutor` 仍为 `unavailable`。规则推荐、统计摘要和未来星系的 AI 课程名称都不是 AI 能力证据。以后接入时，AI 只能提供有来源、可拒绝的建议；学习完成、评分、课程发布、权限和治理变更仍须由人确认，并通过既有领域 API 与审计链执行。
...
│   ├── 01-开发者文档.md        # 当前实现规范入口
│   ├── 01-开发者手册.md        # 本详细实现卷
│   ├── 02-项目规划.md          # 唯一任务、版本、认领和状态控制面
│   ├── 02-更新规划.md          # 旧文件名兼容导航
│   ├── 03-开发历史.md          # V7.4.12 起提交与验收
│   ├── 03-发布历史.md          # V7.4.11 及以前历史
│   ├── 04-部署指南.md          # 服务器部署文档
...
- Future 的 6 方向 / 18 activity key 只登记到 catalog；本批没有 Future 页面、Bridge 或互动运行接线。V7.7.20 已确认 V7.7.19 的 hidden 分类受真实学生单元列表过滤边界阻断；BE-011 后端处置与 V7.7.22 前端接线均已集成，QA-014 A03a、完整 FE-012 与 Future A03b 继续等待后续验证。
...
| `app.services.learning_evidence` | 完成规则、append-only 事实、追加式行政纠错、投影/恢复/聚合，以及教师逐学生证据发现。明细查询先复用 class teacher/admin 与 active `CourseClass` 边界，再限定 active student membership；按 `occurred_at DESC, id DESC` 稳定分页，只按事件类型固定叶路径映射 learner / trusted-assessment 事实为最多 12 个有界标量和 `truncated`。所有用户可控字符串（包括 `source_ref/ref/kind/choice/operation`）只返回类型/长度且置 `truncated=true`，当前没有可回显原值的 literal enum；未知 nested leaf 不递归展开，ORM、原始 evidence、幂等键、请求哈希、凭据与完整源码均不出边界 |
...
| GET | `/api/learning-evidence/classes/{class_id}/courses/{course_id}/events` | 本班 teacher 或全局 admin 按必填 `subject_user_id` 和可选 `activity_key/event_type` 发现 0051 事实；`limit=1—100`、`offset=0—100000`，稳定返回 `total/limit/offset/next_offset`、内部纠错 ID、纠错关联和有界 `evidence_summary`。摘要只读取固定叶路径，所有用户可控字符串只给 `value_type/value_size` 并置 `truncated=true`，直接源码、JWT、`sk_live`、API key、PEM 与任意未知 nested raw leaf 均不返回原值或片段 |
...
- 浏览器报告 `ok=true` 后才可运行 `python tools/quality/sqlite-qa-ledger.py --database <same-isolated-db> --output <proof-dir>/sqlite-ledger.json`；helper 使用 SQLite `mode=ro`，核对全部表计数、当前 `20260727_0051` Alembic 版本、审计链头和 `admin-authority` 单例锁，并以排他临时文件、fsync 和原子替换写报告，拒绝输出覆盖数据库或其硬链/符号链接。当前结论仍只证明一次性 SQLite 与本机真实 Edge，不替代 MySQL、staging 或 R6 目标环境发布证据。
...
- 当前活动集成分支为 `主开发`，由主开发单写并按 02 分配版本；`main@V7.5.11` 保持公开稳定，`houduan` 保留下轮后端起点，`review` 冻结。项目对接已恢复：CONTENT 只读、FE 使用登记的隔离工作树、QA 独立只读，BE 在主开发证明接口缺口前停用；门禁通过后统一由主开发集成。
