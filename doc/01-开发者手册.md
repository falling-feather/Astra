...
- Future 的 6 方向 / 18 activity key 只登记到 catalog；本批没有 Future 页面、Bridge 或互动运行接线。V7.7.16 已确认移动端力学裁切修复通过，但因折叠 dock 的 Escape 路由逸出 P1 而 RETURN；V7.7.17 仍是待独立复审、集成与 QA 的最小候选，Future A03b 继续等待后续实现。
