...
0064 能恢复旧提交与审计中可确认的分数；未知旧发布、满分及已脱敏反馈继续标为未知。出现新历史事实后拒绝有损回退。学习结果与学生自己的历史回读已经验证；旧实验运行中的上下文、退出课程后的历史目录入口及真实 MySQL/并发验收仍有后续工作，不能据本节宣称整个 D—F 阶段完成。
...
已接通课程家族、同源同步、候选审核与文件服务；检查点与作业的跨版本认定、补做和评分历史已接通。实验运行接入、统一跨课程学习路径及后续统计/任务尚未完成。公网业务后端、真实课堂效果、正式隔离执行与完整移动端精调未作为已完成能力。
