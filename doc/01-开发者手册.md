...
>
> **当前基线**：`main@V7.5.11` 是公开稳定线；`主开发` 当前集成线包含 V7.7.4 控制面与 V7.7.2 / FE-012 首批共享证据实现。V7.6.5 的 Alembic 0051、版本化完成规则、幂等学习事实、活动 / 续学投影、教师聚合和追加式纠错已通过 QA-014 A02；V7.7.0—1 冻结内容 / 路由与 FE-012 选择性重放契约，V7.7.2 只接入两个非 Future 代表活动、共享证据基础设施和最小角色回读。V7.7.5 / A03a 已因 locked 深链与工科 44px 触达退回；V7.7.6 FE 候选已实现定向返修，但仍待主开发集成和 QA 复验，不能写成 FE-012、未来星系或三星系整体完成。当前交付目标是 `astra-local.ps1 + SQLite + 127.0.0.1:9001` 的课程作业 / 设计大赛展示版，优先课程互动与学生—教师—管理员协同；域名、公网 R6、真实 MySQL、隔离 runner、压力测试、真实课堂试点和 AI 能力保留为后置工程。
>
...
- V7.7.6 FE 候选中，学生课程单元只信任 `effective_release_state`：open 才生成可执行链接，locked 显示锁定状态且没有 `href`，hidden / 未知状态均不渲染。班级下拉在 `refreshClassScope()` 的空班提前返回前调用 `AstraEngineeringLabPublicationContext.switchClass()`：空值立即清除发布班级，非空值重新读取本人班级集；切班、销毁或 401 通过 scope generation / `AbortSignal` 同时取消旧导航和在途发布查询，且只按本次 `prepare()` 返回集校验目标班级。学生直接访问 `physics.mechanics` 时，`ModuleSelector` 在加载实验脚本、Canvas、Zoom 和 evidence owner 前调用发布上下文 `resolve()`。open 继续原教学闭环；locked 显示说明与安全返回，hidden 返回物理实验列表，异常状态失败关闭。复核已运行 owner 时必须先完成实验 owner cleanup，再清 evidence；pending 期间切换普通模块、关闭或 reset 会同步撤销 pending 与门禁节点，迟到结果不得复活旧门禁。力学清空、暂停、放大与四个 range 输入保持至少 44px 命中盒、4px 视觉轨道、原生方向键和可见焦点。该候选尚未获得主开发集成或 QA PASS。
