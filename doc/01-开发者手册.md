...
>
> **当前基线**：`main@V7.5.11` 是公开稳定线；`主开发@V7.7.1` 是本轮恢复基线，包含 V7.6.5 的 Alembic 0051、版本化完成规则、幂等学习事实、活动 / 续学投影、教师聚合和追加式纠错，并由 V7.7.0—1 冻结内容 / 路由与 FE-012 选择性重放契约。V7.6.5 已通过 QA-014 A02；V7.7.2 在登记的 FE 隔离工作树形成首个共享证据实现候选，只接入两个非 Future 代表活动和最小角色回读，不能写成 FE-012 或三星系稳定完成。当前交付目标是 `astra-local.ps1 + SQLite + 127.0.0.1:9001` 的课程作业 / 设计大赛展示版，优先课程互动与学生—教师—管理员协同；域名、公网 R6、真实 MySQL、隔离 runner、压力测试、真实课堂试点和 AI 能力保留为后置工程。
>
...
- `codex/fe-v7.7-evidence` 上的 V7.7.2 是 FE-012 首批实现候选：只包含共享证据 core/catalog/status、两个非 Future 代表活动、角色 recovery / aggregate 回读和 ModuleSelector 清理事务修复；Future 运行接线、其余课程、真实 Edge / SQLite A03 与主线状态仍未完成。
