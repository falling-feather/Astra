...
>
> **当前基线**：业务代码沿用 2026-09-08 收口；正式前端为 `qianduan/`，FastAPI 为业务权威。2026-09-16 已进入持续开发：资源与模板、独立教学分叉、同步送审及冻结发布已完成本机功能联调；换版结果认定与后续流程按 02 推进。
>
> **最后一次更新时间**：2026-09-16
>
> **更新者**：主开发组
...
权威接口注册位于 [router.py](../backend/app/api/router.py)，DTO 在 [schemas](../backend/app/schemas/)，数据库模型在 [models](../backend/app/models/)。当前基础教学保留 `/api` 与 `/api/v1`；`/api/v2` 已提供资源、课程草稿、派生、同步、候选审核、发布与媒体接口；学习结果认定等后续治理按 02 继续实施。
...
实体表与外键以模型为准；当前唯一迁移 head 是 `20260916_0063`。0061 增加资源版本、独立课程家族、单元来源、修订/修改批次/候选/审核项和幂等发布回执表；旧课程逐门建立独立根，旧发布的候选引用保持空，不补造审核。0062 分离课程设置草稿与通用操作回执，0063 保存不可变媒体文件及课程取用关系。新业务入口见下节。旧事件/进度与 evidence/runtime/projection 并存仍是技术债，后续应迁移到统一语义，而非建立另一份完成真值。
...
此处不把结果策略的保存等同于跨版本认定已完成。学习上下文、旧结果认定、补做记录和统一统计仍由后续 D—E 批次完成。
...
已接通课程家族、同源同步、候选审核与文件服务；尚未完成跨版本补做与旧结果认定、完整评分历史、统一跨课程学习路径和后续统计/任务。公网业务后端、真实课堂效果、正式隔离执行与完整移动端精调未作为已完成能力。
