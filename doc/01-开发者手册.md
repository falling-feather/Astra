...
>
> **当前基线**：`main@V7.5.11` 是公开稳定线；`主开发` 当前集成线包含 FE-012 的共享证据、精确单元处置与教学闭环返修。V7.7.29 曾完成既定 A03a 矩阵；其后同一产品树的补充移动→桌面保持打开探针稳定复现 Zoom P3：最终 943px host 仍使用过渡期 863px backing store 并被自动放大。V7.7.30 候选让 PhysicsSim 既有父容器 ResizeObserver 把稳定尺寸交给 Zoom 窄入口，继续由唯一 `resizeForZoom()` writer 同步 CSS、DPR 位图、`W/H` 与 render，并限制自动 base scale 不超过 1；不新增 Observer、RAF、全局监听或 Canvas writer，仍待独立复审与真实浏览器续验。V7.6.5 的 Alembic 0051、版本化完成规则、幂等学习事实、活动 / 续学投影、教师聚合和追加式纠错已通过 QA-014 A02；V7.8.0 另提供教师逐学生证据安全摘要分页，以及管理员课程状态受约束 PATCH、影响计数与审计回读，均复用既有模型且不新增迁移。V7.8.2 已为学生已知 deep-link 集成精确单元访问处置，只返回 open / hidden / locked / missing 的最小二字段结果，不扩张课程目录。V7.9.4 已把管理台收束为六领域、严格首页 DTO、课程状态可信回读、全局写锁与两个 admin-only owner；第七轮独立代码 / 合同复审 P0—P3=0，真实浏览器和完整三角色演示联调仍待后续精确 QA。当前交付目标是 `astra-local.ps1 + SQLite + 127.0.0.1:9001` 的课程作业 / 设计大赛展示版，优先课程互动与学生—教师—管理员协同；域名、公网 R6、真实 MySQL、隔离 runner、压力测试、真实课堂试点和 AI 能力保留为后置工程。
>
