...
[ActivitySession](../qianduan/src/domain/activity-session.ts)串行发送并保留原请求重试；离页释放监听器与未发送内存队列，不把观察或答案持久化至普通浏览器存储。[CoursePlayer](../qianduan/src/portal/course-player.ts)将模板滑杆及指定原实验控件接到同一协议；原求解器不承担教学授权或评分。原力学重力滑杆的步长已由 50 改为 10，以避免默认 980 被浏览器归整成 1000；原保护基线不变，有独立的一致性回归与精确变更记录。
