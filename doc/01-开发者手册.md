...
>
> **当前基线**：`主开发@V8.0.36` 是当前单线收束基线。平台已具备 Cookie Session 三角色外壳、课程/班级/作业与治理 API、SQLite/Alembic 数据层、学习证据写入/服务端完成投影/教师回读、学生与教师高频工作台、工科试验室 88 个实验、代码空间 18 个活动、未来星系 18 个活动，以及 `engineering.load-path`、`physics.mechanics` 两门作品级旗舰课。V8.0.26 曾由 QA 在同 revision 完成 20/20 cold、10/10 reload 与双课账本/教师回读保护线；V8.0.30 因移动回执和工程同帧构图两项展示 P1 退回，V8.0.32—V8.0.34 已完成窄修和统一代际，但最终浏览器复验在工作组解散前尚未执行。V8.0.35—V8.0.36 只完成文档、Git 现场与展示优先路线收束，不修改产品运行字节。因此当前可称“代码与合同集成候选”，不能称 V8.0.34 最终 QA PASS、production、正式 OJ 或真实课堂实证完成。
>
> **当前任务入口**：侧栏中的后端组、架构组、前端组、内容组、QA 组五个长期对接任务已归档，协作代理树仅保留 `/root`；全部非主工作树也已建立可恢复归档后退出。当前只保留用户与“星序 Astra｜主开发｜总控”单线对接；V8.0.34 未验状态、展示优先路线和后续恢复顺序只在 [`02-更新规划.md`](02-更新规划.md) 维护。本文记录当前真实实现，不复制任务状态。
>
> **具体规划法则**：
>
...
20. [V8.0.36 全局功能、业务闭环与展示边界](#20-v8036-全局功能业务闭环与展示边界)
...
2. 七个脏现场均已转为具名 stash，`u358@3734a2f` 的唯一提交仍由原分支保存；全部 66 条本地分支均保留，恢复索引见更新规划第 9 节。
...
flowchart TB
    ...
    SCOPE --> ENGLAB["工科试验室<br/>5 学科 / 88 实验"]
    SCOPE --> CODE["代码空间<br/>6 组 / 18 活动"]
    SCOPE --> FUTURE["未来星系<br/>6 方向 / 18 活动"]
...
| --- | --- | --- | --- |
| 工科试验室 | 数学 20、物理 20、化学 17、算法 12、生物 19，共 88 个可视化实验；包含 Canvas、图表、模拟、3D 与响应式实验页 | 广度很强，但 87 个普通实验的教学步骤、证据和解释深度不等同于旗舰力学课 | 作为“内容规模与可扩展性”背景；现场重点演示 `physics.mechanics`，其余只选少量视觉代表作快速扫览 |
...
| 未来星系 | 地球与宇宙、工程、数据、信息、材料、人文 6 方向 18 活动，包含 Canvas 和按需 Three.js 互动、课程发布裁剪与模型边界 | 目录和基础互动完整，但只有 `engineering.load-path` 达到受控观察、纠正、解释和权威证据的旗舰深度 | 作为跨学科品牌主视觉；现场深演工程受力路径，其余方向采用短卡片、关键动画和课程问题展示，不强行接入完整证据内核 |
