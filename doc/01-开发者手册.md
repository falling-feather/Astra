...
>
> **当前基线**：`main@V7.5.11` 是公开稳定线；`主开发` 当前集成线包含 V7.7.4 控制面、V7.7.2 / FE-012 首批共享证据实现和 V7.7.6 / FE-012 A03a 原范围返修。V7.6.5 的 Alembic 0051、版本化完成规则、幂等学习事实、活动 / 续学投影、教师聚合和追加式纠错已通过 QA-014 A02；V7.7.0—1 冻结内容 / 路由与 FE-012 选择性重放契约，V7.7.2 只接入两个非 Future 代表活动、共享证据基础设施和最小角色回读。V7.7.5 / A03a 首轮因 locked 深链与工科 44px 触达退回；V7.7.6 已集成定向返修。V7.7.7 独立复验确认 locked、班级 authority、44px 几何、教学、权限与账本通过，但精确 390×844 下固定评分卡和 FAB 仍遮挡放大按钮，A03a 因单一 P2 再次退回。V7.7.8 候选只调整力学页公共浮层布局并已完成本地两视口命中验证，仍等待主开发集成和独立 QA，不能写成 FE-012、未来星系或三星系整体完成。当前交付目标是 `astra-local.ps1 + SQLite + 127.0.0.1:9001` 的课程作业 / 设计大赛展示版，优先课程互动与学生—教师—管理员协同；域名、公网 R6、真实 MySQL、隔离 runner、压力测试、真实课堂试点和 AI 能力保留为后置工程。
>
...
- V7.7.6 中，学生课程单元只信任 `effective_release_state`：open 才生成可执行链接，locked 显示锁定状态且没有 `href`，hidden / 未知状态均不渲染。班级下拉在 `refreshClassScope()` 的空班提前返回前调用 `AstraEngineeringLabPublicationContext.switchClass()`：空值立即清除发布班级，非空值重新读取本人班级集；切班、销毁或 401 通过 scope generation / `AbortSignal` 同时取消旧导航和在途发布查询，且只按本次 `prepare()` 返回集校验目标班级。学生直接访问 `physics.mechanics` 时，`ModuleSelector` 在加载实验脚本、Canvas、Zoom 和 evidence owner 前调用发布上下文 `resolve()`。open 继续原教学闭环；locked 显示说明与安全返回，hidden 返回物理实验列表，异常状态失败关闭。复核已运行 owner 时必须先完成实验 owner cleanup，再清 evidence；pending 期间切换普通模块、关闭或 reset 会同步撤销 pending 与门禁节点，迟到结果不得复活旧门禁。力学清空、暂停、放大与四个 range 输入的元素几何保持至少 44px、4px 视觉轨道、原生方向键和可见焦点。V7.7.8 候选由评分 owner 把力学评分卡挂到当前实验 `.demo-layout` 前的文档流，并在移动端以 safe-area 感知的底部横向 dock 布置导览、收藏、返回顶部、导出和主 FAB；该页 scrim 不再成为全屏命中层。打开模块侧栏后，FAB、四个 dock 操作和导出菜单低于 backdrop / sidebar / toggle 的模态层；减少动效规则也覆盖 ripple 伪元素，五个 dock 操作在收起、展开及动画中均保持至少 44px。回归必须逐一检查评分卡、侧栏 toggle / 首末项、七个核心控件与五个 dock 控件的矩形交集和 `elementFromPoint` 网格，同时覆盖评分卡可见 / 关闭、滚动和减少动效，不能用提高核心控件 z-index、命中透传或缩小控件替代空间避让。
- Future 的 6 方向 / 18 activity key 只登记到 catalog；本批没有 Future 页面、Bridge 或互动运行接线。V7.7.8 候选仅返修上述单一移动端浮层 P2，仍待主开发集成和独立 QA；Future A03b 继续等待后续实现。
