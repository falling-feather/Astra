...
>
> **当前基线**：`main@V7.5.11` 是公开稳定线；`主开发@V7.6.5` 已加入 Alembic 0051、版本化完成规则、幂等学习事实、活动 / 续学投影、教师聚合和追加式纠错，并通过 QA-014 A02。FE-012 的三星系客户端接入仍是隔离工作树中的未提交候选，不能写成稳定实现。当前交付目标是 `astra-local.ps1 + SQLite + 127.0.0.1:9001` 的课程作业 / 设计大赛展示版，优先代表课程互动与学生—教师—管理员协同；域名、公网 R6、真实 MySQL、隔离 runner、压力测试和真实课堂试点保留为后置工程。
>
