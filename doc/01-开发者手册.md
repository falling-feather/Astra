...
>
> **当前基线**：`main@V7.5.11` 是公开稳定线；`主开发` 当前集成线包含 V7.7.4 控制面、V7.7.2 / FE-012 首批共享证据实现、V7.7.6 / FE-012 A03a 原范围返修和 V7.7.8 / FE-012 浮层避让返修。V7.6.5 的 Alembic 0051、版本化完成规则、幂等学习事实、活动 / 续学投影、教师聚合和追加式纠错已通过 QA-014 A02；V7.7.8 已把力学评分卡移入实验文档流、重排移动 dock 与侧栏模态层并补齐 ripple 减少动效。V7.7.9 独立 QA 确认两视口遮挡、44px、locked 深链、权限、SQLite 0051 和 console 通过，但“记录预测”没有形成 `predicted/completed` 权威事实（P1），折叠 dock 的不可见收藏按钮仍进入 Tab 顺序（P2），因此 A03a 再次退回原 FE-012。V7.7.10 FE 候选已实现预测命令的精确回执跟踪和 dock / 导出菜单焦点闭环，仍待主开发集成与 QA-014 复验。当前交付目标是 `astra-local.ps1 + SQLite + 127.0.0.1:9001` 的课程作业 / 设计大赛展示版，优先课程互动与学生—教师—管理员协同；域名、公网 R6、真实 MySQL、隔离 runner、压力测试、真实课堂试点和 AI 能力保留为后置工程。
>
...
- V7.7.6 中，学生课程单元只信任 `effective_release_state`：open 才生成可执行链接，locked 显示锁定状态且没有 `href`，hidden / 未知状态均不渲染。班级下拉在 `refreshClassScope()` 的空班提前返回前调用 `AstraEngineeringLabPublicationContext.switchClass()`：空值立即清除发布班级，非空值重新读取本人班级集；切班、销毁或 401 通过 scope generation / `AbortSignal` 同时取消旧导航和在途发布查询，且只按本次 `prepare()` 返回集校验目标班级。学生直接访问 `physics.mechanics` 时，`ModuleSelector` 在加载实验脚本、Canvas、Zoom 和 evidence owner 前调用发布上下文 `resolve()`。open 继续原教学闭环；locked 显示说明与安全返回，hidden 返回物理实验列表，异常状态失败关闭。复核已运行 owner 时必须先完成实验 owner cleanup，再清 evidence；pending 期间切换普通模块、关闭或 reset 会同步撤销 pending 与门禁节点，迟到结果不得复活旧门禁。力学清空、暂停、放大与四个 range 输入的元素几何保持至少 44px、4px 视觉轨道、原生方向键和可见焦点。V7.7.8 由评分 owner 把力学评分卡挂到当前实验 `.demo-layout` 前的文档流，并在移动端以 safe-area 感知的底部横向 dock 布置导览、收藏、返回顶部、导出和主 FAB；该页 scrim 不再成为全屏命中层。打开模块侧栏后，FAB、四个 dock 操作和导出菜单低于 backdrop / sidebar / toggle 的模态层；减少动效规则覆盖主 FAB、导览、收藏与返回顶部的 ripple 伪元素，五个 dock 操作在收起、展开及动画中均保持至少 44px。V7.7.10 候选把 `started` 的可操作门槛收紧为 `confirmed/reconciled/queued`，并以本次 `client_event_id + event_type` 绑定预测等命令状态。在途锁串行禁用整组命令，但 queued 等终态释放锁后仍保留最近命令身份；队列跨标签严格 DTO 同步保留 `event_type`，所以 peer exact confirmed 可继续把该命令从本地待同步更新为确认。untracked attempted / corrected 也只保存当前 authority 代际内的最近 pending 身份，exact peer confirmed 单调升级，迟到 local-pending 和错误 ID / 类型 / scope 均不能回退状态。mounted domain owner 以 16 条页面缓冲和单个在途请求串行处理真实操作；queued / local-pending 事件以 ID、类型、authority 代际与 scope 保留到最多 256 条 peer identity。容量满时暂停发请求并给出可恢复提示，exact peer terminal 由统一释放路径自动续排，错误 ID / 类型 / scope 不得释放容量；authority 变化与 destroy 清空身份表。domain 状态使用独立槽位，不能覆盖在途 tracked 命令。authority 清除、身份重配与 destroy 会同时使旧 tracked / untracked record、初始化和投影 recovery 失效；撤权或重配立即清空旧投影，任何 recovery 都在写 state / DOM 前校验本次实际 `AbortSignal` 与上下文，预期迟到取消不写 console warn。已确认事件与投影刷新错误分层，后续 recovery 失败只提示投影暂不可刷新，不能把 predicted / explained / peer 确认降级成未保存。cancelled / manual-intervention / 真正写入异常保持失败。中央 FAB owner 在折叠或菜单关闭时保存并撤销次级控件的 Tab / ARIA / inert 状态，展开或打开时精确恢复；body observer 只监听相关 `childList/subtree` 结构，导出菜单 class 使用独立、可替换并可销毁的 observer，晚创建控件、快速 toggle、焦点回主 FAB及 hide/show 的 timer 生命周期由动态合同固定。回归必须同时检查矩形、`elementFromPoint`、Tab 顺序和权威预测确认，不能用 CSS 隐藏或静态源码替代运行证据。
...
- Future 的 6 方向 / 18 activity key 只登记到 catalog；本批没有 Future 页面、Bridge 或互动运行接线。V7.7.10 仍为等待主开发集成与 QA-014 复验的 FE 候选；Future A03b 继续等待后续实现。
