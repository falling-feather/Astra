...
- V7.7.6 中，学生课程单元只信任 `effective_release_state`：open 才生成可执行链接，locked 显示锁定状态且没有 `href`，hidden / 未知状态均不渲染。班级下拉在 `refreshClassScope()` 的空班提前返回前调用 `AstraEngineeringLabPublicationContext.switchClass()`：空值立即清除发布班级，非空值重新读取本人班级集；切班、销毁或 401 通过 scope generation / `AbortSignal` 同时取消旧导航和在途发布查询，且只按本次 `prepare()` 返回集校验目标班级。学生直接访问 `physics.mechanics` 时，`ModuleSelector` 在加载实验脚本、Canvas、Zoom 和 evidence owner 前调用发布上下文 `resolve()`。open 继续原教学闭环；locked 显示说明与安全返回，hidden 返回物理实验列表，异常状态失败关闭。复核已运行 owner 时必须先完成实验 owner cleanup，再清 evidence；pending 期间切换普通模块、关闭或 reset 会同步撤销 pending 与门禁节点，迟到结果不得复活旧门禁。力学清空、暂停、放大与四个 range 输入保持至少 44px 命中盒、4px 视觉轨道、原生方向键和可见焦点。该实现已进入主开发集成线，仍待 V7.7.7 独立 QA 复验。
- Future 的 6 方向 / 18 activity key 只登记到 catalog；本批没有 Future 页面、Bridge 或互动运行接线。V7.7.5 / A03a 对 V7.7.2 集成基线的 locked 与 44px 结论仍为 FAIL；Future A03b 仍等待后续实现。
