...
1. **当前稳定基线**：主线已包含 `V8.0.12 / f831b03` 最小后端、`V8.0.13—V8.0.14` 工作台/旗舰课、`V8.0.17 / 8726c84` Engineering/busy 返修、`V8.0.21 / 1b49757` publication 单飞和 `V8.0.23 / 23c30de` 同路由事件窄修；V8.0.12 已由 QA 独立跑完 639 passed、8 skipped、1 known xfailed、0 failed。`V8.0.22 / 156324d` 的 10-context 复验为 cold 5/10 PASS、5/10 FAIL，P1 RETURN；当前正在形成 V8.0.24 统一终验候选，不把未复验字节称为发布版。
...
| 关键浏览器验收与返修 | `V8.0.15—V8.0.24` | V815 建立三缺陷失败基线；V817 关闭 Engineering/busy；V818/V820/V822 Physics 首载递进 RETURN；V821 provider 单飞已由并发探针证明，V823 再关闭迟到同路由 hashchange 误杀，V824 统一资源代际后以 10-context 复验并抽查桌面/390×844、键盘、reduced-motion、教师回读、console/pageerror/Network | QA-022 对 V824 给出可追溯 PASS 或再次退回，不用实现组自测替代，也不扩张为全量六课/生产验收 |
...
- **版本分配者**：主开发
- **当前版本轨道**：`V7.9.98` 参赛展示版重组 → `V8.0.12—V8.0.24` 可观测体验收口
- **集成分支**：`主开发`
- **当前基线**：后端 `V8.0.12 / f831b03`、旗舰课 `V8.0.14 / d284b9e`、角色工作台 `V8.0.13 / 8cc6b9d`、入口/反馈 `V8.0.17 / 8726c84`、provider 单飞 `V8.0.21 / 1b49757` 与同路由事件窄修 `V8.0.23 / 23c30de` 已进入主线。`V8.0.22 / 156324d` 的 QA 复验为 P0=0、P1=1，证明迟到同路由 hashchange 仍会无条件中止有效 flight；PM-022 正在形成 V8.0.24 统一资源代际，最终 Browser 结论只绑定新的 clean revision
- **提交责任组字段**：启用
...
| 旗舰课程组 | FE | 星序 Astra｜旗舰课程组｜长期对接 | FE-029 动画完成；FE-032 provider 单飞已由 V822 并发探针证明；FE-033 仅修同路由迟到 hashchange 误杀，真实离页仍取消 | 保留来源 worktree | `fe29@4fb963a`、`fe30@812e119` | 暂停 |
| QA组 | QA | 星序 Astra｜QA组｜长期对接 | V8.0.12 PASS；V815/V818/V820/V822 留下递进失败证据，V822 cold 5/10 PASS、5/10 FAIL；保留 fresh 数据/证据等待 V8.0.24 原位复验 | 只读/隔离 worktree | `codex/qa/qa-022-showcase` / `qa22@156324d` | 活跃 |

当前派单状态：`UI-005 / V8.0.17` 已关闭 Engineering/busy；`FE-032 / V8.0.21` 单飞已通过 V822 三路并发真实探针，但 V822 仍被迟到同路由 hashchange 无条件中止。`FE-033 / V8.0.23` 已以 clean 候选 `812e119` 修正并集成为 `23c30de`。`PM-022` 当前形成 V8.0.24 资源代际、文档、全量门禁和 QA 复验接线；UI/FE 不再扩张。六课迁移、深度分析、演示数据系统、用户研究与生产发布继续暂缓。
...
- [ ] PM-022｜状态：进行中｜目标版本：V8.0.18、V8.0.20、V8.0.22、V8.0.24｜模块：展示返修统一资源代际与终验接线｜主责：PM｜协作：UI、FE、QA｜任务：V818/V820/V822 三轮原位复验均诚实 RETURN；V822 已证明 provider 单飞有效，并把剩余 P1 锁定为同路由迟到 hashchange 无条件取消 exact flight。继续集成 FE-033，保留 V819 ModuleSelector、V817 学生、V813 教师、V814 旗舰课，并用 `20260812v824PublicationRouteP0` 统一 app-session/registries/Router/main/Service Worker，把新 provider 字节传播到 loader；同步精确静态合同与 01/02/03/08，把同一 clean revision 交 QA-022 再次原位复验｜前置：V8.0.22 QA cold 5/10 PASS、5/10 FAIL，P0=0/P1=1；FE-033 来源 `812e119` 已集成为 `23c30de`｜关联文档：本文、[`01-开发者文档.md`](01-开发者文档.md)、[`03-开发历史.md`](03-开发历史.md)、[`08-前端页面实现索引.md`](08-前端页面实现索引.md)｜关联版本：V8.0.18、V8.0.20、V8.0.22、V8.0.24；V819/V817/V813/V814/V824 分层资源代际｜允许写入：根工作区 Git、入口/registry/Router/main/SW、资源代际精确合同和共享 01/02/03/08；不得改 UI/FE 业务字节、API、后端或 route 算法｜验证：FE-033 来源/集成 blob 对账，完整 `npm test`、BE-018 定向、文档/项目对接/Git 门禁通过；QA-022 在保留 fresh 数据上以至少 10 个全新 context 复验 canonical Physics cold/reload、无重复 started/POST，并抽查 Engineering/busy、scope 与关键矩阵
...
- [ ] QA-022｜状态：进行中｜目标版本：V8.0.12 验收门禁、V815/V818/V820/V822 失败基线、V8.0.24 展示复验｜模块：参赛展示版独立质量门禁｜主责：QA｜协作：PM、UI、FE｜任务：V8.0.12 后端 PASS；V815、V818、V820、V822 形成可追溯递进失败链。V822 的 10 个全新 context 为 cold 5/10 PASS、5/10 FAIL，reload 5/5 PASS；同 key 三路探针 5/5 单飞正确，剩余 P1 锁定为迟到同路由 hashchange。PM-022 形成 V824 后，复用同一 fresh 数据与证据根，重启精确运行快照，以至少 10 个全新 context 复验首次 load、普通 reload、ledger/started/POST、scope、console/pageerror/Network，并抽查已关闭问题和关键矩阵｜前置：PM-022 V824 clean revision、静态/全量门禁与 HTTP 字节一致性通过｜关联文档：本文第 1.3、7 节、[`22-V8最终完成路线与验收蓝图.md`](02-子文档/22-V8最终完成路线与验收蓝图.md)｜关联版本：V815=`3b01ab2`、V818=`aa24fbf`、V820=`582a636`、V822=`156324d` RETURN；目标 V824 由 PM-022 形成｜允许写入：默认只读，不修改产品/后端/共享文档；证据只写仓库外目录｜验证：每个结论绑定精确 revision、视口、请求与截图；External/IAB 均不可用时只可按记录使用 Playwright Edge headless 并明确降级，不称 external Edge PASS；P0/P1 再次退回，不用实现组自测替代
...
flowchart LR
    ...
    INT24 --> QA22F["QA-022 同 revision 再验"]
    QA22F --> FINAL["同一 revision 展示候选"]
...
| FE | `FE-029 / V8.0.14`、`FE-030/031/032/033 / V8.0.16/19/21/23` | 工程/力学动画；FE-032/033 仅补 provider 单飞、caller 取消与同路由迟到事件边界 | 入口/SW/registry、师生工作台、后端、共享 01/02/03 | QA-022、PM |
| QA | `QA-022` | V8.0.12 PASS；V815/V818/V820/V822 递进失败基线；V8.0.24 只读复验 10× cold/reload 与关键矩阵 | 不得修改产品、后端、共享文档或历史 evidence | PM；失败退回原组 |
| PM | `PM-021 / V7.9.101、V8.0.15`、`PM-022 / V8.0.18、20、22、24` | 根工作区、共享文档、Goal、版本、Git、资源代际和串行集成 | 不在专业组 worktree 内代写候选；不覆盖冻结资产 | 用户、全部小组 |

V8.0.12、UI-004、FE-029、FE-030/031/032/033 与 UI-005 已分别通过既定门禁并串行进入主线；专业组候选没有自行修改共享 cache generation。V815/V818/V820/V822 均由 QA 真实旅程递进退回，V822 已证明单飞正确并把唯一 P1 定位到迟到同路由事件。PM-022 现只把 V819 ModuleSelector、V817 学生、既有 V813 教师/V814 旗舰课和 V824 主壳接到入口/registries/main/SW，并以 V824 传播新 provider 字节。QA-022 只验同一 V824 revision。六课迁移、教师深度驾驶舱、演示数据系统、用户研究、生产部署和性能重构不得借返修进入候选。
...
| 类别 | 标识 | 当前事实 | 本周期动作 |
| --- | --- | --- | --- |
| 集成工作区 | 根工作区 / `主开发` | V8.0.12—V8.0.23 已进入产品线；V815/V818/V820/`V8.0.22 / 156324d` Browser RETURN，FE-033=`23c30de` 已集成；V8.0.24 统一接线进行中 | PM 独占共享 01/02/03/08、资源代际、版本与集成 |
...
| 参赛展示版来源/验收树 | `ui04`、`fe29`、`ui05`、`fe30`、`qa22` | UI-004=`38b502a`、FE-029=`4fb963a`、UI-005=`25519b3`、FE-033=`812e119` 均 clean；QA=`156324d` clean 且保留同一 fresh data/evidence 与四轮 RETURN 证据 | 来源分支保留；V8.0.24 静态门禁通过后 fast-forward QA 树，重启同一外部数据目录做原位复验，不覆盖历史证据 |
