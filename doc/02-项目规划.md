...
- **版本分配者**：主开发
- **当前版本轨道**：`V7.9.98` 参赛展示版重组 → `V8.0.12—V8.0.26` 可观测体验收口 → `V8.0.27—V8.0.35` 效果表达与内容充实
- **集成分支**：`主开发`
- **当前基线**：`V8.0.26 / 2a34976` 是 QA-022 已验稳定底座。V8.0.30=`39b58d6` 的 QA-023 功能账本、Physics、恢复与教师回读稳定，但因移动回执和 Engineering 同帧构图两项赛事展示 P1 退回；UI-007 来源 `fde573f` 已等价集成为 V8.0.32=`b730a62`，FE-036 来源 `476cdf6` 已等价集成为 V8.0.33=`fc49324`。当前生成 V8.0.34 统一资源代际并只交原 QA-023 定向复验，不扩张 API、后端、课程事实或运行时架构
- **提交责任组字段**：启用
...
| UI体验组 | UI | 星序 Astra｜UI体验组｜长期对接 | 独占师生工作台可见布局与界面状态；UI-007 已在不改 JS/DOM/API 的前提下收束移动首屏回执，后续继续保留为同一长期责任线 | 长期 worktree | `codex/ui/ui-007-mobile-receipt` / `ui06@fde573f` | 活跃 |
| 旗舰课程组 | FE | 星序 Astra｜旗舰课程组｜长期对接 | 独占旗舰课程表现层与教学动画；FE-036 已用 Future CSS 收束 Engineering 同帧构图，禁止改求解、物理、证据和完成 owner | 长期 worktree | `codex/fe/fe-036-engineering-same-frame` / `fe35@476cdf6` | 活跃 |
| QA组 | QA | 星序 Astra｜QA组｜长期对接 | 独立保全 V830 证据并只读验收统一 V834 的移动回执、Engineering 三态同帧及稳定保护项；不由主控代写 Browser runner 或产品修复 | 长期只读 worktree | `codex/qa/qa-022-showcase` / `qa22@10eb17d`，V8.0.34 后 fast-forward | 活跃 |

当前派单状态：V830 的 QA-023 两项 P1 已分别退回原 UI、FE 长期组；UI-007/V832 与 FE-036/V833 已在 `ui06`、`fe35` 独立提交并串行等价集成，主控没有代写专业字节，也没有另建临时开发组。QA-023 已完成 V830 自动化/证据收束并在原 `qa22` 明确 WAIT V834。当前只做 V834 统一代际和原 QA 长期组同 revision 复验；六课迁移、深度分析、演示数据系统、用户研究、生产发布和底层重构继续暂缓。
...
- [ ] PM-023｜状态：进行中｜目标版本：V8.0.27、V8.0.30—V8.0.35｜模块：效果表达与内容充实阶段控制｜主责：PM｜协作：UI、FE、QA｜任务：维护效果表达优先的控制面和三个长期工作树责任线；V830 QA 退回后把移动首屏回执原位交 UI-007，把 Engineering 同帧构图原位交 FE-036，主控只负责版本令牌、补丁等价集成、V834 资源代际与共享文档；QA-023 在同一 V834 revision 通过后再用 V835 完成文档/Git/Goal 审计｜前置：V8.0.26=`2a34976` QA PASS；V830=`39b58d6` P0=0/P1=2；UI-007/FE-036 已独立交付｜关联文档：本文、[`01-开发者文档.md`](01-开发者文档.md)、[`03-开发历史.md`](03-开发历史.md)、[`08-前端页面实现索引.md`](08-前端页面实现索引.md)｜关联版本：V827、V830、V831；V832=`b730a62`、V833=`fc49324`；V834 统一候选与 V835 闭环待完成｜允许写入：根工作区共享 01/02/03/08、Git、入口与静态资源代际；不得在专业组范围内代写 UI/课程实现，不改 API、后端、数据库、课程事实或证据 owner｜验证：任务/版本/长期树可追溯；候选 blob 与来源 patch-id 一致；完整前端合同、必要后端定向、文档/项目对接、资源代际、`git diff --check` 与同 revision QA 通过
...
- [ ] QA-023｜状态：进行中｜目标版本：V8.0.34 统一候选验收｜模块：展示可读性与稳定基线回归｜主责：QA｜协作：PM、UI、FE｜任务：V830 已形成完整 RETURN 证据并停止扩张；在原 `qa22` fast-forward 到 V834 后，只定向复验真实学生 success、教师 empty/双课 success 的 390×844 回执可见性，Engineering prediction/C/D 的 1440×1000 同帧和 390 同 context 结果，并保护 Physics、busy/reload、双课账本与教师回读不回归｜前置：UI-007/V832、FE-036/V833 已集成且 PM-023 形成 V834；HTTP 静态资源与本地 blob/代际一致｜关联文档：本文第 1.3、7 节、[`03-开发历史.md`](03-开发历史.md)｜关联版本：V830 RETURN；V834 待验；QA 默认不提交产品代码｜允许写入：默认只读，产品、后端和共享文档禁止；证据只写仓库外新目录｜验证：修复项按冻结 bbox/同帧门禁通过；console/pageerror/关键 Network/HTTP≥400 为 0；Physics/Engineering 事件类型与 started 数不增殖、客户端 completed=0、刷新后服务端投影与教师回读一致；完整前端和 BE-018 定向通过；Browser 后端必须诚实记录

- [x] UI-007｜状态：已完成｜目标版本：V8.0.32｜模块：390 首屏教学叙事闭合｜主责：UI｜协作：QA、PM｜任务：只用既有 student/teacher focus stage 移动 CSS，把 action 与 receipt 放入同一紧凑状态行，使五段叙事落在可见 dock 上方；不删语义、不隐藏回执、不用 fixed overlay｜前置：V830 QA P1｜关联文档：本文第 4、6—7、9 节、[`03-开发历史.md`](03-开发历史.md)、[`08-前端页面实现索引.md`](08-前端页面实现索引.md)｜关联版本：来源 `fde573f`（父 `10eb17d`），主线等价集成 `b730a62`；精确标题 `V8.0.32 UI fix(role-mobile): 让师生首屏回执进入可视范围（UI-007）`｜允许写入：精确 2 CSS + 3 合同；JS/DOM/API/shared/backend/cache/docs 零改｜验证：Edge 151 等价生产 DOM 的学生/教师 empty+success 中 action/receipt bottom 均小于 dock top 778，primary=1、CTA≥46、overflow=false；完整 `npm test` 237 JS/59 contracts、diff-check 通过；真实统一页留 QA-023/V834

- [x] FE-036｜状态：已完成｜目标版本：V8.0.33｜模块：Engineering 桌面同帧观察构图｜主责：FE｜协作：QA、PM｜任务：仅用 desktop Future CSS 让真实 `.fg-stage` sticky，并把同一 load-path 教学流改为紧凑双列/全宽证据行；移动端保持原单列，不用缩放、fixed 假层或复制数值｜前置：V830 QA P1｜关联文档：本文第 4、6—7、9 节、[`03-开发历史.md`](03-开发历史.md)、[`08-前端页面实现索引.md`](08-前端页面实现索引.md)｜关联版本：来源 `476cdf6`（父 `10eb17d`），主线等价集成 `fc49324`；精确标题 `V8.0.33 FE fix(load-path-layout): 让工程观察与数值在桌面同帧呈现（FE-036）`｜允许写入：`pages/frontier/frontier.css` 与 1 个精确合同；JS/solver/state/evidence/API/shared/backend/cache/docs 零改｜验证：prediction 1440×1000 实测 Canvas/table/status/coach 同帧且 overflow=false；正文/表格/步骤保持 10—11px、动作≥44；完整 `npm test` 237 JS/59 contracts、diff-check 通过。C/D 候选侧未冒充 PASS，明确交 QA-023/V834 真实同 revision 复验
...
| 类别 | 标识 | 当前事实 | 本周期动作 |
| --- | --- | --- | --- |
| 集成工作区 | 根工作区 / `主开发` | V830=`39b58d6` 经 QA-023 以两项赛事展示 P1 退回；V832=`b730a62` 与 V833=`fc49324` 已按来源 patch-id 串行等价集成，当前生成 V834 | PM 独占共享 01/02/03/08、版本、资源代际与串行集成，不代写 UI/FE 产品字节 |
...
| 参赛展示版来源/验收树 | `ui04`、`fe29`、`ui05`、`fe30`、`qa22` | 旧 UI/FE 候选均 clean；`qa22@10eb17d` clean，V830 RETURN evidence、自动化摘要、fresh data 与 9001 保留，当前明确 WAIT V834 | 旧来源分支与仓库外 evidence 保留；QA 只在 PM 下发 V834 后 fast-forward 定向复验 |
| 展示深化长期树 | `ui06`、`fe35` | `ui06@fde573f` 与 `fe35@476cdf6` 均 clean；UI-007/FE-036 来源已分别等价集成为 `b730a62/fc49324`，stable patch-id 一致 | 两棵长期责任树继续保留，不清理、不复用为其他组；后续原范围失败仍回到对应长期组 |

剩余树都有明确保留理由。本周期不再清理脏树或含独有 patch 的树；后续仍禁止 `--force`、递归删除和 `git branch -D`。UI、FE、QA 的后续任务优先继续复用登记的 `ui06/fe35/qa22` 长期工作树；只有责任域新增且无法隔离时才按规范登记新树，禁止为一次窄修重复召集临时工作组。
