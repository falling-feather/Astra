...
| 效果表达与内容充实 | `V8.0.27—V8.0.31` | V827 重开控制面；V828 深化师生工作台的角色叙事、信息层次和可观察反馈；V829 为两门旗舰课补充目标、变量、观察、结果解释与收束表达；V830 统一静态代际并集成；V831 记录同 revision 展示验收与文档/Git 闭环 | 不改变既有 API、课程事实、证据 owner、状态机、求解器或物理积分器；桌面与 390×844 的关键画面、键盘、reduced-motion、console/pageerror/Network、刷新恢复、教师回读和账本无漂移均通过 QA-023 |
...
- **集成分支**：`主开发`
- **当前基线**：后端 `V8.0.12 / f831b03`、旗舰课 `V8.0.14 / d284b9e`、角色工作台 `V8.0.13 / 8cc6b9d`、入口/反馈 `V8.0.17 / 8726c84`、provider 单飞 `V8.0.21 / 1b49757`、同路由事件窄修 `V8.0.23 / 23c30de` 与初始化后挂载 `V8.0.25 / 4624f5d` 已进入主线。`V8.0.26 / 2a34976` 已通过 QA-022 同 revision 终验：20/20 cold、10/10 reload、0 回落、双课账本不漂移、师生关键矩阵与独立自动化全绿；P0/P1/P2=0/0/0
- **提交责任组字段**：启用
...
| UI体验组 | UI | 星序 Astra｜UI体验组｜长期对接 | 在 V8.0.26 既有工作台 owner、scope 和真实主动作上深化角色叙事、信息层次、内容提示与可观察反馈；禁止新增 API 或重写共享运行时 | 新任务隔离 worktree | `ui06` 待 V8.0.27 控制提交后创建 | 活跃 |
| 旗舰课程组 | FE | 星序 Astra｜旗舰课程组｜长期对接 | 在 FE-029 既有工程/力学动画和课程状态机上补目标、变量、观察线索、结果解释与收束反馈；禁止改求解、物理、证据和完成 owner | 新任务隔离 worktree | `fe35` 待 V8.0.27 控制提交后创建 | 活跃 |
| QA组 | QA | 星序 Astra｜QA组｜长期对接 | 以 V8.0.26 PASS 为稳定基线，先形成展示可读性检查表，再只读验收统一候选的桌面/390、键盘、reduced-motion、刷新恢复、教师回读和账本无漂移 | 只读/隔离 worktree | `codex/qa/qa-022-showcase` / `qa22@2a34976`，集成后 fast-forward | 活跃 |

当前派单状态：`PM-022 / QA-022` 已在 V8.0.26 关闭稳定性返修。用户于 2026-08-12 恢复 Goal，并明确“效果表达与呈现优先、充实内容与丰富体系、原则上不大改既有内容”；因此新建 `PM-023 / UI-006 / FE-035 / QA-023`，先完成 V8.0.27 控制提交，再从该 clean 基线创建 UI/FE 新树并行实施。六课迁移、深度分析、演示数据系统、用户研究、生产发布和底层重构继续暂缓。
...
- [ ] UI-006｜状态：进行中｜目标版本：V8.0.28｜模块：师生工作台展示叙事与内容层次｜主责：UI｜协作：FE、QA、PM｜任务：在现有学生/教师工作台真实 scope、主动作、busy/error/empty/success/disabled 和 API owner 上，补足“当前班课—本节目标—预计产出—下一动作—完成回执”的可读层次；强化桌面与 390×844 的首屏焦点、短时状态反馈、中文微文案和低干扰动效，使评委与师生无需理解内部 schema 即可判断当前任务与下一步｜前置：PM-023 / V8.0.27 clean 基线；UI-004/UI-005 已集成且 QA-022 PASS｜关联文档：本文第 1.1—1.3、2.2、7.2 节、[`05-UI规范模板.md`](05-UI规范模板.md)、[`08-前端页面实现索引.md`](08-前端页面实现索引.md)｜关联版本：预留 V8.0.28；提交标题须在候选就绪后由 PM 发放｜允许写入：`pages/student/student-workbench.js`、`pages/student/student.css`、`pages/teacher/teacher.js`、`pages/teacher/teacher-foundation.css`、`pages/teacher/teacher-workbench.css`、`pages/teacher/teacher-curriculum.css` 及本任务新增/更新的精确前端合同；共享 01/02/03、入口/cache、课程页、API、后端、`shared/` 禁止｜边界：只消费现有班课、课程、作业、发布计划、进度和证据投影，不新增统计结论、不伪造目标达成/误区、不改变 data owner、scope gate、写锁、键盘与权限语义；禁止大规模 DOM/框架重写｜验证：1440×1000 与 390×844 首屏主动作唯一、无横溢出、关键触点≥44px；五态、键盘、focus-visible、reduced-motion 可用；API/fetch/data-* owner 集合与 scope 语义不回归；定向与完整前端合同通过

- [ ] FE-035｜状态：进行中｜目标版本：V8.0.29｜模块：双旗舰课教学内容与过程表达深化｜主责：FE｜协作：UI、QA、PM｜任务：在 `engineering.load-path` 的 B/C/判断/D 与 `physics.mechanics` 的预测/.40/.80/比较/纠正/解释既有路径上，增加一眼可读的课程目标、固定变量/唯一变量、操作提示、观察清单、阶段解释、结果差异、模型边界和结课收束；让动画、表格与教学语言同步，支持现场讲解和录屏，不增加新课程或新证据类型｜前置：PM-023 / V8.0.27 clean 基线；FE-029/024/025 与 FE-034 已集成且 QA-022 PASS｜关联文档：本文第 1.1—1.3、2.2、3.2—3.3、7.2 节、[`27-V7.7三星系六门代表课程教学与证据冻结契约.md`](02-子文档/27-V7.7三星系六门代表课程教学与证据冻结契约.md)、[`26-V7.7未来星系页面视觉规格.md`](02-子文档/26-V7.7未来星系页面视觉规格.md)｜关联版本：预留 V8.0.29；提交标题须在候选就绪后由 PM 发放｜允许写入：`pages/engineering/bridge-truss.js`、`pages/engineering/engineering.css`、`pages/frontier/frontier.css`、`pages/physics/physics.js`、`pages/physics/physics.css` 及本任务新增/更新的精确前端合同；共享 01/02/03、入口/cache、师生工作台、API、后端、`shared/` 禁止｜边界：不得改桁架 exact solver、Canvas 单 writer、受控状态顺序、Physics 真实 loop/首峰、固定 H/g/r/vx/damping、authority/permit/receipt、离线语义或服务端 `completed` owner；新增展示计算只能消费既有 exact solve/真实 measurement，不进入 evidence｜验证：两课桌面/390 关键步骤可观察，动画/数值/文案同步；键盘与 reduced-motion 有等价输出；事件序列、账本、客户端 completed=0、固定量和唯一变量 e 不回归；定向与完整前端合同通过
...
| 层级 | 证明内容 | 当前状态 | V8 要求 |
| --- | --- | --- | --- |
| L1 静态/契约 | 语法、资源、接口形状、角色和生命周期边界 | V8.0.26 主线 235 个受跟踪 JavaScript 文件语法和 56 组合同通过；V8.0.9 覆盖六方法/七态/四投影、manual 锁存、因果 witness 和三类 fixture；V8.0.10 覆盖 raw-only 重算与 13 类报告不一致反例；V8.0.11 再覆盖双游标、21 组精确 identity 变体、敏感 recovery 输入、不可变 sidecar、server position reservation 和 aggregate-cursor manual 边界 | 继续作为快速门禁；合同只证明列举断言可捕获，不冒充真实课程、IndexedDB、后端/数据库顺序、浏览器、网络时延、外部真实性或用户价值验收 |
...
| 类别 | 标识 | 当前事实 | 本周期动作 |
| --- | --- | --- | --- |
| 集成工作区 | 根工作区 / `主开发` | V8.0.12—V8.0.26 已进入产品线；V815/V818/V820/V822/V824 Browser RETURN，V826=`2a34976` QA PASS；当前以 V8.0.27 重开展示深化控制面 | PM 独占共享 01/02/03/08、版本、新树创建与串行集成 |
...
| 展示深化新树 | `ui06`、`fe35` | V8.0.27 控制提交前尚未创建 | 只从 V8.0.27 clean 基线创建；分别绑定 UI-006/FE-035，禁止交叉写入与自行分配版本 |
