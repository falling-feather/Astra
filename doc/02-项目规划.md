...
1. **当前稳定基线**：产品实现为 `V8.0.11 / 080028af`，当前控制与调度治理更新为 `V7.9.97`。V8.0.9 已把 `AstraLearningActivity` 六方法运行时、不可变 manifest、七运行态/四服务端投影态、失败关闭恢复、manual-intervention 锁存、迟到响应隔离、服务端完成边界和 raw-evidence provenance envelope 合入主线；V8.0.10 又把该 provenance 边界接入 QA-016 当前动态探针，由只接收 raw records 的独立 verifier 重算六项 canonical facts，再核对全部报告层和真实退出码，其 assurance 固定为 `consistency-only`。V8.0.11 关闭了 BE 实施前暴露的单序号/snapshot P1：`run.sequence` 保持 learner-only，服务端账本另用连续 `server_sequence`；首次发送冻结 command 与 post-event canonical domain snapshot 为不可变 sidecar；恢复输入按不可信数据过滤敏感 authority/token/projection 字段，v2 envelope 与 identity 任一未知、额外或仅 trim 后相等的字段都失败关闭；新事件不得占用旧 server position，已确认重放必须锁定原位置。多条 local-pending 若只有聚合 cursor 而无逐事件权威位置，会安全进入 manual。现有课程页仍通过 `LearningEvidenceActivity.record/refresh/context/destroy` 运行，后端 `/me/recovery` 仍只提供有界投影，真实 IndexedDB 事务、HTTP/数据库顺序和三类真实课程适配均未实现；因此“内核、合同和报告重算已存在”仍不等于“六门课程已迁移”或“产品已经精确续学”。BE-018 / V8.0.12 现为唯一写入任务；全量后端门禁已暴露旧代码评测合同把仓库 current head 永久写死为 0052，本控制点只放行将该断言改为动态唯一 head，两个显式升级到 0052 的历史迁移场景不得改写。FE-028 继续等待权威服务器半包。其余 17 项 Future 活动、六门金标准课、真实浏览器总验收、教师诊断和代码容量治理仍未完成。当前代码事实只能从 [`01-开发者文档.md`](01-开发者文档.md)、代码、配置、测试和可复核运行结果读取，专业组候选 SHA 不能替代主线集成 revision。
...
- **集成分支**：`主开发`
- **当前基线**：产品语义基线 `V8.0.11 / 080028af`；本次控制面 `V7.9.97`（父控制提交 `d22d52a`）。BE-018 的干净候选必须以本控制提交为实际 Git 父，同时完整包含 `080028af` 的产品语义；各长期任务只接受主开发派单中的精确 Git SHA
- **提交责任组字段**：启用
...
| 架构组 | ARCH | 星序 Astra｜架构组｜长期对接 | 目标架构、活动契约、遗留候选处置、模块边界和技术债门禁 | 长期 worktree | `codex/astra-v8-arch-v811` / `0725` | 活跃（只读复核） |
| 前端组 | FE | 星序 Astra｜前端组｜长期对接 | 学生/教师路径、三个学习空间、活动适配器、导航、反馈和前端生命周期 | 长期 worktree | `codex/astra-v8-frontend-mechanics` / `10fa` | 活跃 |
| 后端组 | BE | 星序 Astra｜后端组｜长期对接 | 课程作用域、代码提交、证据服务、教师读模型、迁移和后端测试 | 长期 worktree | `codex/astra-v8-backend-v812` / `5779` | 活跃（实现恢复） |
...
当前派单状态：`ARCH-003 / V8.0.0`、`QA-016 / V8.0.1`、`CONTENT-005 / V7.9.76/V7.9.81`、`FE-023 / V8.0.2`、`BE-014 / V8.0.3`、`QA-018 / V7.9.78`、`FE-024 / V8.0.4`、`FE-025 / V8.0.5`、`BE-016 / V8.0.6`、`QA-019 / V8.0.7`、`QA-020 / V8.0.8`、`ARCH-004 / V8.0.9`、`QA-021 / V8.0.10` 及 `ARCH-005 / V8.0.11` 已进入主线。ARCH-005 的前两版候选分别因恢复侧信道可绕过敏感字段门禁/新事件可复用旧 server position，以及嵌套 identity 未按 frozen v2 层精确比较而被退回；最终候选 `690688f9082b` 经主开发与 QA 独立复审均为 PASS、P0/P1/P2=0，重放为 `080028afb88f`。当前 7 个产品文件、234/55 主线合同和 line ceiling 均通过，Browser/IndexedDB/BE/DB 明确 `NOT-RUN`。BE-018 的旧 `V8.0.11` 令牌保持撤回，dirty 预备树资产仍受保护；其 `V8.0.12` 令牌现恢复，只能按 `080028af` 的 v2 契约修订，并以当前 V7.9.97 控制点为实际父形成最终可审补丁。BE 定向合同与学习证据全套已通过；约 530.5 秒的全量后端仅剩旧代码评测测试写死 0052 current head 一项失败，现只授权动态唯一 head 修正，不能改写两个有意的 0052 历史升级目标。FE-028 继续等待。V8.0.11 集成不等于 Browser、外部真实性、后端精确恢复或六课适配完成。
...
| --- | --- | --- | --- | --- |
| ARCH | `ARCH-005 / V8.0.11`（已完成） | 当前只读复核已集成双游标、不可变 sidecar、纯恢复 helper、边界配置/合同与 24；不再追加候选写入 | backend、课程页、IndexedDB/cache owner、主壳、共享 01/02/03、遗留 worktree | `BE-018`、`FE-028`、PM |
...
| BE | `BE-018 / V8.0.12`（进行中） | 在原 dirty 预备树保全前提下适配 `080028af` v2 合同；只额外修正代码评测合同的一处动态 current-head 断言；最终补丁必须重放到干净 V7.9.97 控制父线并重跑门禁 | 0052 历史升级目标、single-sequence DTO/迁移、前端/运行时、遗留 worktree、共享 01/02/03；不得把服务器半包外推为客户端 pending 完整性 | QA、ARCH、FE、PM |
...
QA-021 已按 `V8.0.10 / b8691e4` 进入主线并回到只读；它只检验和接入 provenance，没有修改产品，也没有提供 Browser 或外部真实性证明。ARCH-005 已按 `V8.0.11 / 080028af` 经三轮严格复审进入主线并回到只读；它只冻结纯 JS 恢复消费合同，没有实现页面、IndexedDB、后端或数据库。当前唯一写入令牌属于 `BE-018 / V8.0.12`，FE/CONTENT/ARCH/QA 继续只读，禁止一个长期 worktree 同时承接两个写入任务。BE 当前 10 modified + 2 untracked 现场必须原样保全，禁止 reset/clean/rebase；可在该树适配 v2 契约和单一 current-head 回归，但最终可审补丁必须重放到以 V7.9.97 控制提交为实际父、并包含 `080028af` 产品语义的干净环境。BE-018 只生成服务器可证明部分，必须拒绝 learner 写入乱序并独立分配 server cursor，不能替客户端声明 IndexedDB pending 集完整或直接宣称精确续学。V7.9.93 已因实施期出现旧 service 约 `1,924→2,924` 行的候选膨胀而收紧模块边界：新活动端口、运行组写入和恢复 helper 必须迁入专用 owner，旧 service 只保留既有公共职责/兼容复用；V7.9.94 进一步要求清理搬移残留和大量 `_private` 反向依赖，再验收净差异；V7.9.97 只把全量后端发现的一处 0052 current-head 硬编码纳入 BE-018，两个历史 0052 升级目标保持不动。`FE-028` 必须等待 BE-018 集成后再汇合真实页面、单事务 outbox 与完整 pending。`FE-026`、`BE-015` 此刻不并行启动，避免审查容量分散。QA-019 未运行的当前 Browser 与真实网络矩阵仍归 QA-017。OPS-005 只在恢复目标 MySQL/发布范围时启动；BE-017 必须等待真实规模/MySQL 或等价可追溯证据。CONTENT-005 前两片只提供课程合同，剩余两课另行分配版本。
...
| L2 后端/集成 | 数据模型、权限、幂等、迁移、投影和报告联合语义 | QA-018 已分离历史 0051 与当前 head；QA-019 对账动态 head、六事件、规则/教师 witness 和同 ID 重放；V8.0.10 正式 baseline/gate 为 2/1、22 raw→6 facts、临时 SQLite verified-removed；V8.0.11 冻结 v2 双游标恢复 DTO、exactness 和 server position 的消费形状，但未运行后端/数据库 | `BE-018` 仍须提供权威服务器半包并拒绝乱序 learner 写入，`FE-028` 仍须证明 IndexedDB sidecar 和本地 pending 完整；QA-019 的 ASGI 503 不外推真实 socket/响应丢失，V8.0.10 canonical facts 不外推外部来源真实性；迁移节点测试不得硬编码 current head |
...
1. **学生 Future**：登录→继续学习→预测→B/C/D 改变载荷与记录观察→错误判断→反馈→修正→解释→刷新；V8.0.7 已把当前产品状态机、六事件、服务端完成 owner、教师回读、同 ID 重放和安全回退做成动态门禁，但 Browser 为 `NOT-RUN`；V8.0.11 已完成双游标精确恢复消费合同，只有 `BE-018 + FE-028` 接入真实服务器半包、IndexedDB sidecar、完整本地 pending 和课程页后，QA-017 才能验证刷新回同一运行步骤。
...
4. **力学受控实验**：未运行 e=.40 时 e=.80 不可执行→标准两档→比较→修正→解释→完成；V8.0.5 已证明自由探索、乱序、重复和离线 pending 不改变受控规则计数；V8.0.11 仍只有合同，`BE-018/FE-028` 仍须把精确运行组恢复接到真实后端、IndexedDB 和页面。
...
| 类别 | 标识 | 当前事实 | 本周期动作 |
| --- | --- | --- | --- |
| 集成工作区 | 根工作区 / `主开发`（产品点 `080028af`） | V7.9.91 收束 ARCH-004；V7.9.92 并行发放 QA-021/BE-018；V7.9.93 收紧 BE 服务 owner；V7.9.94 收束 QA-021；V7.9.95 退回 BE 单序号候选；V7.9.96 收束 ARCH-005 三轮复审并恢复 BE-018；V7.9.97 精确扩围一处动态迁移头回归并移动候选实际父线 | PM 独占共享 01/02/03 与后续集成 |
