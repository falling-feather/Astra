...
- **集成分支**：`主开发`
- **当前基线**：产品语义基线 `V8.0.12 / f831b03`；参赛展示版控制基线 `V7.9.100 / 820a522`。新 UI、FE、QA 工作树从 `6bb8a36` 创建并 fast-forward 到 `V7.9.99 / 86c2477` 后进入隔离实施；V8.0.12 只改变后端域，与两个前端写入域不重叠，各组继续只接受主开发派单中的精确 Git SHA
- **提交责任组字段**：启用
...
当前派单状态：旧五组停止接收新任务；`BE-018 / V8.0.12` 已独立复验并集成，不再由旧 BE 组继续扩张。`PM-020` 已完成团队与概念派单，当前由 `PM-021` 串行收口；`UI-004 / V8.0.13`、`FE-029 / V8.0.14` 继续并行，`QA-022` 等候候选后执行第二阶段。UI 只写师生工作台范围，FE 只写工程/力学课程范围，QA 默认只读；共享 `index.html`、`sw.js`、`shared/css`、`shared/js` 和 `01/02/03` 继续由主开发独占，跨域接线在集成阶段串行完成。
...
- [ ] PM-021｜状态：进行中｜目标版本：V7.9.101、V8.0.15｜模块：参赛展示版串行集成与资源代际｜主责：PM｜协作：UI、FE、QA｜任务：先把 QA-022 独立通过且字节不漂移的 BE-018 集成主线并同步稳定文档；再逐个复审、移植 UI-004 与 FE-029，最后只在根工作区升级角色、Future、Physics、入口和 Service Worker 的缓存/资源代际及精确静态合同，把同一集成 revision 交 QA-022 做浏览器终验｜前置：PM-020；BE-018 已完成第一阶段，UI/FE 候选尚在实现｜允许写入：根工作区 Git、`index.html`、`sw.js`、`shared/js/page-registry.js`、`shared/js/experiment-registry.js`、`shared/js/main.js`、`shared/js/frontier-learning.js`、与资源代际精确绑定的必要合同、共享 `01/02/03/08`；不得在根工作区替专业组改写学生/教师/课程业务实现，不得借接线扩张共享架构｜关联文档：本文、[`01-开发者文档.md`](01-开发者文档.md)、[`03-开发历史.md`](03-开发历史.md)、[`08-前端页面实现索引.md`](08-前端页面实现索引.md)｜关联版本：V7.9.101（V8.0.12 验收/集成记录）、V8.0.15（候选合并与统一资源代际）｜验证：每次移植无冲突或经原组重验；来源/集成 blob 或 stable patch 可解释；完整 `npm test`、必要后端回归、文档与项目对接门禁通过；最终同一 revision 交 QA-022 做桌面/390×844、键盘、reduced-motion、console/pageerror/Network、刷新恢复和教师回读
...
- [ ] UI-004｜状态：进行中｜目标版本：V8.0.13｜模块：师生高频工作台作品化｜主责：UI｜协作：UX、FE、QA｜任务：在不新建深度教师分析能力的前提下，统一学生与教师工作台的首屏层级、主动作、课程上下文、加载/空/错/成功状态、操作反馈、字号、间距、色彩、焦点和移动端布局；学生突出“继续学习”，教师突出当前班课与下一操作；保留现有权限、数据 owner 和业务动作｜前置：PM-020；复用现有 Future Galaxy 深蓝观测台视觉基准，并以已原图检查的[`师生双角色工作台概念`](image/astra-role-workspaces-desktop-concept.png)冻结“当前上下文→唯一主动作→状态回执”层级｜关联文档：本文第 1.3、2.4、3.5 节、[`05-UI规范模板.md`](05-UI规范模板.md)、[`26-V7.7未来星系页面视觉规格.md`](02-子文档/26-V7.7未来星系页面视觉规格.md)｜关联版本：V8.0.13｜允许写入：`pages/student/student.css`、`pages/student/student.js`、`pages/student/student-workbench.js`、`pages/teacher/teacher-foundation.css`、`pages/teacher/teacher-workbench.css`、`pages/teacher/teacher-curriculum.css`、`pages/teacher/teacher.js`、`tools/tests/teacher-style-layer-contract.cjs`；最后一项只允许同步三层教师 CSS 的精确 SHA-256 与 V8.0.13 语义，不得删除、跳过或放宽注册顺序、唯一 owner、禁止内联或文件边界断言；禁止写 `index.html`、`sw.js`、`shared/`、课程页、后端和共享 `01/02/03`｜验证：桌面与 390×844 首屏无溢出，学生/教师五秒内识别主动作；加载、空、错、成功和 disabled 状态可辨；键盘焦点与 reduced-motion 可用；原有 API/课程作用域合同和 `npm test` 通过

- [ ] FE-029｜状态：进行中｜目标版本：V8.0.14｜模块：两门旗舰课程交互动画与多媒体表达｜主责：FE｜协作：CONTENT、UI、QA｜任务：把 `engineering.load-path` 与 `physics.mechanics` 从功能可用提升为作品级教学体验：工程课以载荷变化、杆件受力/风险和 B→C→D 观察序列形成连续可视反馈；力学课只改变 e=0.40→e=0.80，以真实运动阶段、首峰高度、能量耗散、比较与纠正形成清晰状态动画，摩擦及其余控制量保持锁定；动画必须表达变量、过程或反馈，禁止装饰性动效遮挡教学信息，不改变既有证据事件、完成 owner、permit/receipt 和课程作用域｜前置：PM-020、FE-024、FE-025；消费 CONTENT-005 已冻结的两门合同；以已原图检查的[`旗舰力学主舞台概念`](image/astra-flagship-mechanics-concept.png)冻结构图、步骤轨、状态语言和移动层级，但图中可编辑摩擦/质量只是构图示意，明确不得进入实现｜关联文档：本文第 1.3、3.2—3.3 节、[`27-V7.7三星系六门代表课程教学与证据冻结契约.md`](02-子文档/27-V7.7三星系六门代表课程教学与证据冻结契约.md)、[`26-V7.7未来星系页面视觉规格.md`](02-子文档/26-V7.7未来星系页面视觉规格.md)｜关联版本：V8.0.14｜允许写入：`pages/engineering/bridge-truss.js`、`pages/engineering/engineering.css`、`pages/physics/physics.js`、`pages/physics/physics.css`、`pages/frontier/frontier.css` 及两门课专属新资产；禁止写 `index.html`、`sw.js`、`shared/`、师生工作台、后端和共享 `01/02/03`｜验证：两门课预测→操作→观察→反馈→纠正→解释主路径可操作；动画与数值/状态同步，桌面/390×844/键盘/reduced-motion 可用；旧顺序、证据、权限、离线重放和完成规则合同不回归；`npm test` 通过

- [ ] QA-022｜状态：进行中｜目标版本：V8.0.12 验收门禁、V8.0.15 展示验收｜模块：参赛展示版独立质量门禁｜主责：QA｜协作：PM、UI、FE｜任务：第一阶段已 PASS：对 `40bf80a` 完成完整后端、迁移/DDL、BE-018 定向合同和 patch 边界复验；当前等待第二阶段，届时建立主线、UI-004、FE-029 的桌面/390×844 视觉与交互基线，验收学生/教师主动作、两门旗舰课、键盘、reduced-motion、console、pageerror、关键 Network、刷新恢复和教师回读；不扩张为六课、生产网络、真实 MySQL或用户研究验收｜前置：PM-020；第二阶段等待 UI/FE 精确候选与 PM-021 同一集成 revision｜关联文档：本文第 1.3、7 节、[`22-V8最终完成路线与验收蓝图.md`](02-子文档/22-V8最终完成路线与验收蓝图.md)｜关联版本：V8.0.15 统一候选只由 PM-021 形成，QA 默认不提交产品改动｜允许写入：默认只读；未获版本令牌不得修改产品、后端、共享文档或历史 evidence｜验证：第一阶段候选 P0/P1/P2=0/0/0；第二阶段每个结论绑定精确 revision、视口、命令和截图；P0/P1 退回原组，未执行项明确标记 NOT-RUN；不得用实现组自测替代独立结论
...
| PM | `PM-021 / V7.9.101、V8.0.15` | 根工作区、共享文档、Goal、版本、Git、概念资产、跨域接线和最终集成 | 不在专业组 worktree 内代写候选；不覆盖冻结资产 | 用户、全部小组 |

V8.0.12 已通过 QA-022 第一阶段并以字节一致补丁进入主线，不再接受实现扩张。UI-004 与 FE-029 从同一控制基线并行，写入目录不重叠；两组不得自行修改共享 cache generation 或入口文件。需要 `index.html`、`sw.js`、共享 loader 或文档接线时，先在交付包列明，由 PM-021 在两个候选完成后串行更新；QA-022 只验同一最终 revision。六课迁移、教师深度驾驶舱、演示数据系统、用户研究、生产部署和性能重构在本轮不得借“顺手优化”进入候选。
...
| 类别 | 标识 | 当前事实 | 本周期动作 |
| --- | --- | --- | --- |
| 集成工作区 | 根工作区 / `主开发` | `V8.0.12 / f831b03`；后端服务器半包已进入产品线，PM-021 接管后续串行集成 | PM 独占共享 01/02/03/08、资源代际、版本与集成 |
