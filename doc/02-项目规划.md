...
1. **当前稳定基线**：产品实现为 `V8.0.11 / 080028af`，当前控制与调度治理更新为 `V7.9.96`。V8.0.9 已把 `AstraLearningActivity` 六方法运行时、不可变 manifest、七运行态/四服务端投影态、失败关闭恢复、manual-intervention 锁存、迟到响应隔离、服务端完成边界和 raw-evidence provenance envelope 合入主线；V8.0.10 又把该 provenance 边界接入 QA-016 当前动态探针，由只接收 raw records 的独立 verifier 重算六项 canonical facts，再核对全部报告层和真实退出码，其 assurance 固定为 `consistency-only`。V8.0.11 关闭了 BE 实施前暴露的单序号/snapshot P1：`run.sequence` 保持 learner-only，服务端账本另用连续 `server_sequence`；首次发送冻结 command 与 post-event canonical domain snapshot 为不可变 sidecar；恢复输入按不可信数据过滤敏感 authority/token/projection 字段，v2 envelope 与 identity 任一未知、额外或仅 trim 后相等的字段都失败关闭；新事件不得占用旧 server position，已确认重放必须锁定原位置。多条 local-pending 若只有聚合 cursor 而无逐事件权威位置，会安全进入 manual。现有课程页仍通过 `LearningEvidenceActivity.record/refresh/context/destroy` 运行，后端 `/me/recovery` 仍只提供有界投影，真实 IndexedDB 事务、HTTP/数据库顺序和三类真实课程适配均未实现；因此“内核、合同和报告重算已存在”仍不等于“六门课程已迁移”或“产品已经精确续学”。BE-018 / V8.0.12 现恢复为唯一写入任务，FE-028 继续等待其权威服务器半包。其余 17 项 Future 活动、六门金标准课、真实浏览器总验收、教师诊断和代码容量治理仍未完成。当前代码事实只能从 [`01-开发者文档.md`](01-开发者文档.md)、代码、配置、测试和可复核运行结果读取，专业组候选 SHA 不能替代主线集成 revision。
...
| 阶段 | 版本范围 | 交付重点 | 退出条件 |
| --- | --- | --- | --- |
| 控制面与测试治理恢复 | `V7.9.72—V7.9.75、V7.9.77—V7.9.80、V7.9.82—V7.9.96` | 重写 02、重启长期目标、建立新团队与 worktree 实态、集中分配版本，在产品集成后诚实同步外部验收阻塞，清除迁移测试对过期 current head 的硬编码，记录性能候选边界，拆分候选开发/Browser 终验，并在 FE-024/025、BE-016、QA-019/020、ARCH-004/005 与 QA-021 集成后登记恢复、容量、模块化、浏览器、报告、证据来源及产品适配责任；实施期发现单 owner 膨胀或上下游合同语义冲突时，先收紧边界、撤回令牌并插入架构修复；稳定文档能力声明改变时同步更新其静态合同 | 文档/任务/版本/团队/Git 一致；旧候选均有处置状态；历史 revision 测试与当前 head 账本职责分离；候选与主线能力分开；已通过、待补证和目标态不混写 |
...
- **集成分支**：`主开发`
- **当前基线**：产品实现 `V8.0.11 / 080028af`；本次控制面 `V7.9.96`（父控制提交 `e070509`，父产品/集成提交 `080028af`）；各长期任务只接受主开发派单中的精确 Git SHA
- **提交责任组字段**：启用
...
| BE | `BE-018 / V8.0.12`（进行中） | 在原 dirty 预备树保全前提下适配 `080028af` v2 合同；最终补丁必须重放到干净当前产品父线并重跑门禁 | single-sequence DTO/迁移、前端/运行时、遗留 worktree、共享 01/02/03；不得把服务器半包外推为客户端 pending 完整性 | QA、ARCH、FE、PM |
...
QA-021 已按 `V8.0.10 / b8691e4` 进入主线并回到只读；它只检验和接入 provenance，没有修改产品，也没有提供 Browser 或外部真实性证明。ARCH-005 已按 `V8.0.11 / 080028af` 经三轮严格复审进入主线并回到只读；它只冻结纯 JS 恢复消费合同，没有实现页面、IndexedDB、后端或数据库。当前唯一写入令牌属于 `BE-018 / V8.0.12`，FE/CONTENT/ARCH/QA 继续只读，禁止一个长期 worktree 同时承接两个写入任务。BE 的 8 modified + 2 untracked 现场必须原样保全，禁止 reset/clean/rebase；可在该树适配 v2 契约，但最终可审补丁必须重放到以 `080028af` 为父的干净环境。BE-018 只生成服务器可证明部分，必须拒绝 learner 写入乱序并独立分配 server cursor，不能替客户端声明 IndexedDB pending 集完整或直接宣称精确续学。V7.9.93 已因实施期出现旧 service 约 `1,924→2,924` 行的候选膨胀而收紧模块边界：新活动端口、运行组写入和恢复 helper 必须迁入专用 owner，旧 service 只保留既有公共职责/兼容复用；V7.9.94 进一步要求清理搬移残留和大量 `_private` 反向依赖，再验收净差异。`FE-028` 必须等待 BE-018 集成后再汇合真实页面、单事务 outbox 与完整 pending。`FE-026`、`BE-015` 此刻不并行启动，避免审查容量分散。QA-019 未运行的当前 Browser 与真实网络矩阵仍归 QA-017。OPS-005 只在恢复目标 MySQL/发布范围时启动；BE-017 必须等待真实规模/MySQL 或等价可追溯证据。CONTENT-005 前两片只提供课程合同，剩余两课另行分配版本。
...
| 类别 | 标识 | 当前事实 | 本周期动作 |
| --- | --- | --- | --- |
| 集成工作区 | 根工作区 / `主开发`（产品点 `080028af`） | V7.9.91 收束 ARCH-004；V7.9.92 并行发放 QA-021/BE-018；V7.9.93 收紧 BE 服务 owner；V7.9.94 收束 QA-021；V7.9.95 退回 BE 单序号候选；V7.9.96 收束 ARCH-005 三轮复审并恢复 BE-018 | PM 独占共享 01/02/03 与后续集成 |
