...
- **集成分支**：`主开发`
- **当前基线**：后端语义底座 `V8.0.12 / f831b03`、旗舰课程集成 `V8.0.14 / d284b9e`、师生工作台集成 `V8.0.13 / 8cc6b9d`；V8.0.15 正在根工作区统一 V813/V814 动态资源与 V815 主壳代际。UI、FE 候选均以 `86c2477` 为精确父线并经来源/集成 blob 与 stable patch-id 对账；QA-022 只对即将形成的统一 revision 给出最终 Browser 结论
- **提交责任组字段**：启用
...
| UI体验组 | UI | 星序 Astra｜UI体验组｜长期对接 | UI-004 已交付并独立复审；只在统一候选暴露原范围 P0/P1 时恢复 | 保留 worktree | `codex/ui/ui-004-role-workspaces@38b502a` / `ui04` | 已完成/待命 |
| 旗舰课程组 | FE | 星序 Astra｜旗舰课程组｜长期对接 | FE-029 已交付并完成 P2 窄修；只在统一候选暴露原范围 P0/P1 时恢复 | 保留 worktree | `codex/fe/fe-029-flagship-motion@4fb963a` / `fe29` | 已完成/待命 |
| QA组 | QA | 星序 Astra｜QA组｜长期对接 | V8.0.12 独立复验、视觉基线、桌面/390×844 与关键师生旅程验收 | 只读/隔离 worktree | `codex/qa/qa-022-showcase` / `qa22` | 活跃 |

当前派单状态：`BE-018 / V8.0.12`、`UI-004 / V8.0.13`、`FE-029 / V8.0.14` 均已独立复审并以字节等价补丁进入主线；UI/FE 组转为待命，不继续扩张实现。`PM-021` 正在串行完成 V8.0.15 入口、Router、registry、共享展示语言与 Service Worker 接线；`QA-022` 已完成静态候选复审，等待统一 clean revision 后启动第二阶段真实浏览器终验。六课迁移、深度分析、演示数据系统、用户研究与生产发布继续暂缓。
...
- [ ] PM-021｜状态：进行中｜目标版本：V7.9.101、V8.0.15｜模块：参赛展示版串行集成与资源代际｜主责：PM｜协作：UI、FE、QA｜任务：已把 QA-022 独立通过的 BE-018、UI-004、FE-029 逐个以字节等价补丁集成主线；当前只在根工作区升级角色、Future、Physics、入口和 Service Worker 的缓存/资源代际，清理共享学生进度面板的研发术语并同步精确静态合同，再把同一 clean revision 交 QA-022 做浏览器终验｜前置：PM-020；BE/UI/FE 候选复审 P0/P1/P2 均为 0，专业组已转待命｜允许写入：根工作区 Git、`index.html`、`sw.js`、`shared/js/page-registry.js`、`shared/js/experiment-registry.js`、`shared/js/router.js`、`shared/js/main.js`、`shared/js/frontier-learning.js`、`shared/js/student-learning-evidence.js`、与资源代际/展示语言精确绑定的必要合同、共享 `01/02/03/08`；`router.js` 只更新 module-selector/Future 查询代际，学生证据 owner 只改用户可见标签，均不得改变 API、恢复算法或领域语义；不得在根工作区替专业组改写学生/教师/课程业务实现，不得借接线扩张共享架构｜关联文档：本文、[`01-开发者文档.md`](01-开发者文档.md)、[`03-开发历史.md`](03-开发历史.md)、[`08-前端页面实现索引.md`](08-前端页面实现索引.md)｜关联版本：V7.9.101（V8.0.12 验收/集成记录）、V8.0.15（候选合并与统一资源代际）｜验证：来源/集成 blob 与 stable patch-id 已分别对账；完整 `npm test`、必要后端回归、文档与项目对接门禁通过；最终同一 revision 交 QA-022 做桌面/390×844、键盘、reduced-motion、console/pageerror/Network、刷新恢复和教师回读
...
- [ ] QA-022｜状态：进行中｜目标版本：V8.0.12 验收门禁、V8.0.15 展示验收｜模块：参赛展示版独立质量门禁｜主责：QA｜协作：PM、UI、FE｜任务：第一阶段已 PASS：对 `40bf80a` 完成完整后端、迁移/DDL、BE-018 定向合同和 patch 边界复验；UI-004 已独立静态复审 P0/P1/P2=0，FE-029 核心语义已独立复审且最终 P2 窄修通过主开发复核。第二阶段在 PM-021 形成统一 clean revision 后，用全新仓库外数据目录和 fresh student 验收学生/教师主动作、两门旗舰课、键盘、reduced-motion、console、pageerror、关键 Network、刷新安全回退和教师回读；不扩张为六课、生产网络、真实 MySQL 或用户研究验收｜前置：PM-020；等待 V8.0.15 资源代际、全量合同和文档门禁完成后 fast-forward `qa22` 到同一 revision｜关联文档：本文第 1.3、7 节、[`22-V8最终完成路线与验收蓝图.md`](02-子文档/22-V8最终完成路线与验收蓝图.md)｜关联版本：V8.0.15 统一候选只由 PM-021 形成，QA 默认不提交产品改动｜允许写入：默认只读；未获版本令牌不得修改产品、后端、共享文档或历史 evidence｜验证：第一阶段候选 P0/P1/P2=0/0/0；第二阶段每个结论绑定精确 revision、视口、命令和截图；P0/P1 退回原组，未执行项明确标记 NOT-RUN；不得用实现组自测替代独立结论
...
V8.0.12、UI-004 与 FE-029 已分别通过既定门禁并以字节等价补丁进入主线；专业组候选没有自行修改共享 cache generation。PM-021 只把 V813 角色资源、V814 旗舰课资源与 V815 主壳接到 `index.html`、Router、registries、main fallback 和 Service Worker，并以静态合同冻结每个实际加载 URL；QA-022 只验同一最终 revision。六课迁移、教师深度驾驶舱、演示数据系统、用户研究、生产部署和性能重构在本轮不得借“顺手优化”进入候选。
...
| 类别 | 标识 | 当前事实 | 本周期动作 |
| --- | --- | --- | --- |
| 集成工作区 | 根工作区 / `主开发` | `V8.0.12 / f831b03`、`V8.0.14 / d284b9e`、`V8.0.13 / 8cc6b9d` 已进入产品线；V8.0.15 统一接线进行中 | PM 独占共享 01/02/03/08、资源代际、版本与集成 |
...
| 新参赛展示版树 | `ui04`、`fe29`、`qa22` | UI=`38b502a` clean、FE=`4fb963a` clean、QA=`86c2477` clean；UI/FE 来源分支保留，QA 尚未 fast-forward 到统一候选 | UI/FE 转待命；V8.0.15 提交并通过静态门禁后只 fast-forward QA 树，使用 fresh 外部数据目录终验 |
