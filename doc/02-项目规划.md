...
| 关键浏览器验收与返修 | `V8.0.15—V8.0.26` | V815 建立三缺陷失败基线；V817 关闭 Engineering/busy；V818/V820/V822/V824 Physics 首载递进 RETURN；V824 跑满 20× cold 与 10× reload 后把剩余 P1 锁定为 owner init 与 evidence mount 反序，V825 只修 current-generation 初始化完成顺序，V826 统一资源代际后再跑同规模复验并抽查桌面/390×844、键盘、reduced-motion、教师回读、console/pageerror/Network | QA-022 对 V826 给出可追溯 PASS 或再次退回，不用实现组自测替代，也不扩张为全量六课/生产验收 |
...
- **版本分配者**：主开发
- **当前版本轨道**：`V7.9.98` 参赛展示版重组 → `V8.0.12—V8.0.26` 可观测体验收口
- **集成分支**：`主开发`
- **当前基线**：后端 `V8.0.12 / f831b03`、旗舰课 `V8.0.14 / d284b9e`、角色工作台 `V8.0.13 / 8cc6b9d`、入口/反馈 `V8.0.17 / 8726c84`、provider 单飞 `V8.0.21 / 1b49757`、同路由事件窄修 `V8.0.23 / 23c30de` 与初始化后挂载 `V8.0.25 / 4624f5d` 已进入主线。`V8.0.24 / 3622981` 的 QA 复验为 P0=0、P1=1：cold 7/20，reload 9/10，且唯一失败 reload 发生 ready→false 回落；PM-022 已完成 V8.0.26 统一资源代际与提交门禁，最终 Browser 结论只绑定新的 clean revision
- **提交责任组字段**：启用
...
| UI体验组 | UI | 星序 Astra｜UI体验组｜长期对接 | UI-004 作品化完成；UI-005 已修复 Future 工作台真实入口、师生 `aria-busy` 与学生加载伪空态，不扩张 API/shared | 保留来源 worktree | `ui04@38b502a`、`ui05@25519b3` | 暂停 |
| 旗舰课程组 | FE | 星序 Astra｜旗舰课程组｜长期对接 | FE-029 动画完成；FE-032 provider 单飞与 FE-033 route 边界已保留；FE-034 已把 evidence mount 严格排在 current-generation owner init 成功之后 | 保留来源 worktree | `fe29@4fb963a`、`fe30@622e373` | 暂停 |
| QA组 | QA | 星序 Astra｜QA组｜长期对接 | V8.0.12 PASS；V815/V818/V820/V822/V824 留下递进失败证据；V824 cold 7/20、reload 9/10，保留 fresh 数据/证据等待 V8.0.26 原位复验 | 只读/隔离 worktree | `codex/qa/qa-022-showcase` / `qa22@3622981` | 活跃 |

当前派单状态：`UI-005 / V8.0.17` 已关闭 Engineering/busy；`FE-032 / V8.0.21` 与 `FE-033 / V8.0.23` 分别关闭 provider 并发与同路由事件误杀，但 V824 20× cold/10× reload 又暴露 owner init 与 evidence mount 反序。`FE-034 / V8.0.25` 已以 clean 候选 `622e373` 修正并集成为 `4624f5d`。`PM-022` 已完成 V8.0.26 资源代际、文档、全量门禁和 QA 复验接线；UI/FE 不再扩张。六课迁移、深度分析、演示数据系统、用户研究与生产发布继续暂缓。
...
- [ ] PM-022｜状态：进行中｜目标版本：V8.0.18、V8.0.20、V8.0.22、V8.0.24、V8.0.26｜模块：展示返修统一资源代际与终验接线｜主责：PM｜协作：UI、FE、QA｜任务：V818/V820/V822/V824 四轮原位复验均诚实 RETURN；V824 20× cold 为 7/20 PASS、13/20 FAIL，前 10× reload 为 9/10 PASS、1/10 出现 ready→false，已把剩余 P1 锁定为异步 owner init 与 evidence mount 反序。集成 FE-034，以 `20260812v825PhysicsBindOrderP0` 加载 ModuleSelector，保留 V817 学生、V813 教师、V814 旗舰课，并用 `20260812v826PhysicsBindOrderP0` 统一 app-session/registries/Router/main/Service Worker；同步精确静态合同与 01/02/03/08，把同一 clean revision 交 QA-022 再次原位复验｜前置：V8.0.24 QA P0=0/P1=1；FE-034 来源 `622e373` 已集成为 `4624f5d`｜关联文档：本文、[`01-开发者文档.md`](01-开发者文档.md)、[`03-开发历史.md`](03-开发历史.md)、[`08-前端页面实现索引.md`](08-前端页面实现索引.md)｜关联版本：V8.0.18、V8.0.20、V8.0.22、V8.0.24、V8.0.26；V825/V817/V813/V814/V826 分层资源代际｜允许写入：根工作区 Git、入口/registry/Router/main/SW、资源代际精确合同和共享 01/02/03/08；不得改 UI/FE 业务字节、API、后端或课程事实｜验证：FE-034 来源/集成 blob 对账，完整 `npm test`、BE-018 定向、文档/项目对接/Git 门禁通过；QA-022 在保留 fresh 数据上复验至少 20× cold、10× reload、全程无 ready→false、无重复 started/POST，并抽查 Engineering/busy、scope 与关键矩阵
...
- [ ] QA-022｜状态：进行中｜目标版本：V8.0.12 验收门禁、V815/V818/V820/V822/V824 失败基线、V8.0.26 展示复验｜模块：参赛展示版独立质量门禁｜主责：QA｜协作：PM、UI、FE｜任务：V8.0.12 后端 PASS；V815、V818、V820、V822、V824 形成可追溯递进失败链。V824 跑满 20 个全新 cold context，仅 7/20 PASS；前 10 个 ordinary reload 为 9/10 PASS，失败 run 先 ready 后回落 false；全轮 requestfailed/pageerror/evidence POST=0，账本不变。PM-022 形成 V826 后，复用同一 fresh 数据与证据根，重启精确运行快照，再跑至少 20× cold、10× reload，显式采样全程无 ready→false，并复核 ledger/started/POST、scope、console/pageerror/Network 与已关闭问题｜前置：PM-022 V826 clean revision、V825 ModuleSelector/V826 shell HTTP 字节、静态/全量门禁一致｜关联文档：本文第 1.3、7 节、[`22-V8最终完成路线与验收蓝图.md`](02-子文档/22-V8最终完成路线与验收蓝图.md)｜关联版本：V815=`3b01ab2`、V818=`aa24fbf`、V820=`582a636`、V822=`156324d`、V824=`3622981` RETURN；目标 V826 由 PM-022 形成｜允许写入：默认只读，不修改产品/后端/共享文档；证据只写仓库外目录｜验证：每个结论绑定精确 revision、视口、请求与截图；External/IAB 均不可用时只可按记录使用 Playwright Edge headless 并明确降级，不称 external Edge PASS；P0/P1 再次退回，不用实现组自测替代
...
flowchart LR
    ...
    INT26 --> QA22G["QA-022 20× cold / 10× reload"]
    QA22G --> FINAL["同一 revision 展示候选"]
...
| --- | --- | --- | --- | --- |
| UI | `UI-004 / V8.0.13`、`UI-005 / V8.0.17` | 师生高频工作台；UI-005 仅补真实 Future 入口与 busy/loading 反馈 | `index.html`、`sw.js`、`shared/`、课程页、后端、共享 01/02/03 | QA-022、PM |
| FE | `FE-029 / V8.0.14`、`FE-030/031/032/033/034 / V8.0.16/19/21/23/25` | 工程/力学动画；FE-032/033 补 provider 单飞与 route 边界，FE-034 只补 owner init→evidence mount 顺序 | 入口/SW/registry、师生工作台、后端、共享 01/02/03 | QA-022、PM |
| QA | `QA-022` | V8.0.12 PASS；V815/V818/V820/V822/V824 递进失败基线；V8.0.26 只读复验 20× cold、10× reload 与关键矩阵 | 不得修改产品、后端、共享文档或历史 evidence | PM；失败退回原组 |
| PM | `PM-021 / V7.9.101、V8.0.15`、`PM-022 / V8.0.18、20、22、24、26` | 根工作区、共享文档、Goal、版本、Git、资源代际和串行集成 | 不在专业组 worktree 内代写候选；不覆盖冻结资产 | 用户、全部小组 |

V8.0.12、UI-004、FE-029、FE-030/031/032/033/034 与 UI-005 已分别通过既定实现门禁并串行进入主线；专业组候选没有自行修改共享 cache generation。V815/V818/V820/V822/V824 均由 QA 真实旅程递进退回；V824 的 20× cold/10× reload 已把唯一 P1 收束为 owner init 与 evidence mount 反序。PM-022 现只把 V825 ModuleSelector、V817 学生、既有 V813 教师/V814 旗舰课和 V826 主壳接到入口/registries/main/SW，并以 V826 传播共享证据子资源。QA-022 只验同一 V826 revision。六课迁移、教师深度驾驶舱、演示数据系统、用户研究、生产部署和性能重构不得借返修进入候选。
...
| 类别 | 标识 | 当前事实 | 本周期动作 |
| --- | --- | --- | --- |
| 集成工作区 | 根工作区 / `主开发` | V8.0.12—V8.0.25 已进入产品线；V815/V818/V820/V822/`V8.0.24 / 3622981` Browser RETURN，FE-034=`4624f5d` 已集成；V8.0.26 统一接线与门禁完成，待 QA 原位复验 | PM 独占共享 01/02/03/08、资源代际、版本与集成 |
...
| 参赛展示版来源/验收树 | `ui04`、`fe29`、`ui05`、`fe30`、`qa22` | UI-004=`38b502a`、FE-029=`4fb963a`、UI-005=`25519b3`、FE-034=`622e373` 均 clean；QA=`3622981` clean 且保留同一 fresh data/evidence 与五轮 RETURN 证据 | 来源分支保留；将 QA 树 fast-forward 到 V8.0.26 clean revision，重启同一外部数据目录做原位复验，不覆盖历史证据 |
