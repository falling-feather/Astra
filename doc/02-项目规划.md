...
1. **当前稳定基线**：主线已包含 `V8.0.12 / f831b03` 最小后端、`V8.0.13—V8.0.14` 工作台/旗舰课和 `V8.0.16—V8.0.17 / 104e526、8726c84` 两项返修；V8.0.12 已由 QA 独立跑完 639 passed、8 skipped、1 known xfailed、0 failed。`V8.0.15 / 3b01ab2` 的静态门禁通过但 Browser 因 1 个 P1、2 个 P2 被退回，当前正在形成 V8.0.18 统一终验候选，不把未复验字节称为发布版。
...
| 关键浏览器验收与返修 | `V8.0.15—V8.0.18` | V815 建立同 revision 失败基线；V816 修复 Physics 冷加载竞态，V817 修复 Engineering 工作台入口与 busy 反馈，V818 统一资源代际后复验桌面/390×844、键盘、reduced-motion、刷新、教师回读、console/pageerror/Network | QA-022 对 V818 给出可追溯 PASS 或再次退回，不用实现组自测替代，也不扩张为全量六课/生产验收 |
...
- **版本分配者**：主开发
- **当前版本轨道**：`V7.9.98` 参赛展示版重组 → `V8.0.12—V8.0.18` 可观测体验收口
- **集成分支**：`主开发`
- **当前基线**：后端语义底座 `V8.0.12 / f831b03`、旗舰课程 `V8.0.14 / d284b9e`、角色工作台 `V8.0.13 / 8cc6b9d`、Physics 冷加载窄修 `V8.0.16 / 104e526`、角色入口/反馈窄修 `V8.0.17 / 8726c84` 已进入主线。`V8.0.15 / 3b01ab2` 的静态门禁通过但 Browser 被 QA-022 以 1 个 P1、2 个 P2 退回；PM-022 正在形成 V8.0.18 统一资源代际，最终 Browser 结论只绑定新的 clean revision
- **提交责任组字段**：启用
...
| 旗舰课程组 | FE | 星序 Astra｜旗舰课程组｜长期对接 | FE-029 动画完成；FE-030 已修复 Physics canonical 冷加载暂态锁死，保持同 controller 单次有界恢复 | 保留来源 worktree | `fe29@4fb963a`、`fe30@2e62173` | 暂停 |
| QA组 | QA | 星序 Astra｜QA组｜长期对接 | 第一阶段 V8.0.12 PASS；第二阶段对 V8.0.15 给出 RETURN 并保留 fresh 数据/证据，等待 V8.0.18 原位复验三个缺陷及关键矩阵 | 只读/隔离 worktree | `codex/qa/qa-022-showcase` / `qa22@3b01ab2` | 活跃 |

当前派单状态：`PM-021 / V8.0.15` 已完成集成并把统一候选交 QA，但 QA-022 在真实旅程中给出 RETURN；`FE-030 / V8.0.16` 与 `UI-005 / V8.0.17` 已分别形成 clean 候选、完成全量合同并串行进入主线。`PM-022 / V8.0.18` 当前只负责资源代际、文档、全量门禁和 QA 复验接线；UI/FE 不再扩张。六课迁移、深度分析、演示数据系统、用户研究与生产发布继续暂缓。
...
- [ ] PM-022｜状态：进行中｜目标版本：V8.0.18｜模块：展示返修统一资源代际与终验接线｜主责：PM｜协作：UI、FE、QA｜任务：串行集成 FE-030/UI-005；以 `20260812v816PhysicsReloadP0` 加载 ModuleSelector、`20260812v817RoleFeedbackP0` 加载学生工作台、保留 V813 教师与 V814 旗舰课，并用 `20260812v818ShowcaseRecoveryP0` 统一 app-session/registries/Router/main/Service Worker；同步精确静态合同与 01/02/03/08，把同一 clean revision 交 QA-022 原位复验｜前置：V8.0.15 QA RETURN；FE-030 来源 `2e62173` 已集成为 `104e526`，UI-005 来源 `25519b3` 已集成为 `8726c84`｜关联文档：本文、[`01-开发者文档.md`](01-开发者文档.md)、[`03-开发历史.md`](03-开发历史.md)、[`08-前端页面实现索引.md`](08-前端页面实现索引.md)｜关联版本：V8.0.18；V816/V817/V818 三层资源代际｜允许写入：根工作区 Git、入口/registry/Router/main/SW、资源代际精确合同和共享 01/02/03/08；不得改 UI/FE 业务字节、API、后端或恢复算法｜验证：来源/集成 blob 对账，完整 `npm test`、BE-018 定向、文档/项目对接/Git 门禁通过；QA-022 在保留 fresh 数据上复验 canonical Physics 冷加载、Engineering 工作台入口、师生 busy/loading，并抽查既有关键矩阵
...
- [ ] QA-022｜状态：进行中｜目标版本：V8.0.12 验收门禁、V8.0.15 失败基线、V8.0.18 展示复验｜模块：参赛展示版独立质量门禁｜主责：QA｜协作：PM、UI、FE｜任务：第一阶段已 PASS：BE-018 完整后端、迁移/DDL、定向合同与 patch 边界通过。第二阶段已对 `3b01ab2` 建立 fresh student、桌面/390、双课、教师回读、五态、键盘、reduced-motion、console/pageerror/Network 证据；结论 P0=0、P1=1、P2=2，明确 RETURN。PM-022 形成 V8.0.18 后，保留原 fresh 数据并原位复验 canonical Physics 冷加载、Engineering 工作台入口、师生 busy/loading，同时抽查刷新无重复 started、scope、教师回读与既有关键矩阵｜前置：PM-022 clean revision、静态/全量门禁与 HTTP 字节一致性通过｜关联文档：本文第 1.3、7 节、[`22-V8最终完成路线与验收蓝图.md`](02-子文档/22-V8最终完成路线与验收蓝图.md)｜关联版本：V8.0.15=`3b01ab2` RETURN；目标 V8.0.18 由 PM-022 形成｜允许写入：默认只读，不修改产品/后端/共享文档；证据只写仓库外目录｜验证：每个结论绑定精确 revision、视口、请求与截图；External/IAB 均不可用时只可按记录使用 Playwright Edge headless 并明确降级，不称 external Edge PASS；P0/P1 再次退回，不用实现组自测替代
...
flowchart LR
    ...
    UI5 --> INT18
    INT18 --> QA22C["QA-022 同 revision 复验"]
    QA22C --> FINAL["同一 revision 展示候选"]
...
| FE | `FE-029 / V8.0.14`、`FE-030 / V8.0.16` | 工程/力学课程动画；FE-030 仅补 ModuleSelector 冷加载 evidence mount 生命周期 | 入口/SW/registry、师生工作台、后端、共享 01/02/03 | QA-022、PM |
| QA | `QA-022` | V8.0.12 PASS；V8.0.15 失败基线；V8.0.18 只读复验三个返修与既有关键矩阵 | 不得修改产品、后端、共享文档或历史 evidence | PM；失败退回原组 |
| PM | `PM-021 / V7.9.101、V8.0.15`、`PM-022 / V8.0.18` | 根工作区、共享文档、Goal、版本、Git、资源代际和串行集成 | 不在专业组 worktree 内代写候选；不覆盖冻结资产 | 用户、全部小组 |

V8.0.12、UI-004、FE-029、FE-030 与 UI-005 已分别通过既定门禁并串行进入主线；专业组候选没有自行修改共享 cache generation。PM-021 的 V815 已被 QA 真实旅程退回；PM-022 只把 V816 ModuleSelector、V817 学生工作台、既有 V813 教师/V814 旗舰课和 V818 主壳接到 `index.html`、Router、registries、main fallback 与 Service Worker，并以静态合同冻结实际加载 URL。QA-022 只验同一 V818 revision。六课迁移、教师深度驾驶舱、演示数据系统、用户研究、生产部署和性能重构不得借返修进入候选。
...
| 类别 | 标识 | 当前事实 | 本周期动作 |
| --- | --- | --- | --- |
| 集成工作区 | 根工作区 / `主开发` | V8.0.12—V8.0.17 已进入产品线；`V8.0.15 / 3b01ab2` Browser RETURN，FE-030=`104e526`、UI-005=`8726c84` 已集成；V8.0.18 统一接线进行中 | PM 独占共享 01/02/03/08、资源代际、版本与集成 |
...
| 参赛展示版来源/验收树 | `ui04`、`fe29`、`ui05`、`fe30`、`qa22` | UI-004=`38b502a`、FE-029=`4fb963a`、UI-005=`25519b3`、FE-030=`2e62173` 均 clean；QA=`3b01ab2` clean 且保留 fresh data/evidence 与 V815 RETURN 证据 | 来源分支保留；V8.0.18 静态门禁通过后 fast-forward QA 树，重启同一外部数据目录做原位复验，不覆盖历史证据 |
