...
- **集成分支**：`主开发`
- **当前基线**：产品语义基线 `V8.0.11 / 080028af`；控制面 `V7.9.97 / 83742ac`；待验收后端候选 `V8.0.12 / 40bf80a`。新 UI、FE、QA 工作树统一从本次 `V7.9.98` 控制提交创建，各组只接受主开发派单中的精确 Git SHA
- **提交责任组字段**：启用
...
| UI体验组 | UI | 星序 Astra｜UI体验组｜长期对接 | 学生/教师高频工作台、主动作、状态、视觉统一和响应式 | 长期 worktree | `codex/ui/ui-004-role-workspaces` / `ui04`（待创建） | 待创建 |
| 旗舰课程组 | FE | 星序 Astra｜旗舰课程组｜长期对接 | 工程/力学两门课的操作动画、过程反馈、多媒体表达和课程页适配 | 长期 worktree | `codex/fe/fe-029-flagship-motion` / `fe29`（待创建） | 待创建 |
| QA组 | QA | 星序 Astra｜QA组｜长期对接 | V8.0.12 独立复验、视觉基线、桌面/390×844 与关键师生旅程验收 | 只读/隔离 worktree | `codex/qa/qa-022-showcase` / `qa22`（待创建） | 待创建 |
...
- [ ] PM-020｜状态：进行中｜目标版本：V7.9.98｜模块：参赛展示版目标与团队重组｜主责：PM｜协作：UI、FE、QA｜任务：登记新 Goal；停用旧 ARCH/BE/CONTENT 组；安全退出已集成或补丁等价的旧 worktree；保全脏树、独有提交和 V8.0.12 候选；将当前范围收束为师生平台、两门旗舰课、视觉动画、最小后端和关键浏览器验收；建立并派发新 UI/FE/QA 隔离任务｜前置：用户于 2026-08-12 明确清除旧 Goal、解散旧组并启动新方向；根工作区 clean｜关联文档：本文、[`03-开发历史.md`](03-开发历史.md)｜关联版本：V7.9.98｜验证：Goal active；旧组生命周期、全部 worktree 和候选可解释；新任务写入范围不重叠；文档/项目对接验证和 `git diff --check` 通过
