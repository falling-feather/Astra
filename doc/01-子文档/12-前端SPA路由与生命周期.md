...
学生端不再把前端固定学科键与教师创建后生成的随机 `course_key` 直接比较。共享内容采用稳定身份 `galaxy_key + subject_key + activity_key`，课程实例、选课、发布和学习作用域继续使用后端真实 `course_id`。

| 学习空间 | 后端读取 | 前端呈现 | 当前边界 |
| --- | --- | --- | --- |
| 工科试验室 | 学生可见课程、课程单元、单元开放状态 | 只显示后端开放的学科与实验，首页进度分母和画廊数量同步变化，卡片可刷新直达 | 实验名称、目标、画布与交互仍由冻结的本地活动目录负责 |
| 代码空间 | 全部可见课程叠加当前行政班课程，再按学科读取发布单元 | 允许只开放部分课程；后端锁定原因转换为中文；未出现的课程不渲染 | 直接选课可以学习内容，没有明确班级时不建立正式证据作用域 |
| 未来星系 | 全部可见课程叠加当前行政班课程，再按方向读取发布单元 | 允许只开放部分方向；卡片状态、开放单元和深链统一受发布结果控制 | 直接选课可以浏览，班级证据仍要求明确班级归属 |
| 三角色工作台 | `/api/v1/workbench`、真实课程与单元 | 显示课程实例名称、教师、时间、名单、发布、任务与进度 | 工作台负责课程实例信息，不改写实验本体文案 |

行政班与课程选课是两个维度：同一学生可以有行政班，同时直接加入其他课程。若同一学科同时存在直接课程和当前行政班课程，当前行政班课程优先；多行政班没有明确选择时继续安全停留，不猜测学习作用域。目录投影完成后触发 `astra:student-catalogue-ready`，模块画廊和首页进度在同一轮重绘。

资源代际由主站 `patch=20260828v866StudentProjectionP0` 与代码空间 `759r3` 统一传播，避免旧脚本把新课程投影覆盖回静态全集。代码空间的后端投影位于 `codevis/shared/js/course-publication-adapter.js`，未来星系的后端投影位于 `shared/js/frontier-course-publication-adapter.js`；两者均与受保护的活动清单文件分离。该变更只调整目录、状态、数量与跳转，不修改任何既有实验结构、求解器、动画或操作方式。
