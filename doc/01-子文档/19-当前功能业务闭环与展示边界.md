...
> 最近更新：2026-08-25（V8.4.2 / BE-025）
> 更新者：DOC 组（主开发单线兼任）

集中保存 V8.0.34—V8.4.2 的集成状态、三角色业务闭环、学习空间完成层次与展示边界。
...
`DATA-009@5596112` 把课程闭环所需的关系模型接入稳定 Alembic 链，`BE-024@4fa147c` 接通教师创建、共同教师可见和信息修订提交，`FE-038@b27b886` 把这条能力整理为教师可见的四步创建向导。数据库基线为唯一 head `20260825_0056`。`BE-025@3fde9af` 已继续完成管理员审核后端；规划 P-03 仍等待 `BE-026` 学生准入，P-08 仍等待 `FE-039` 管理员可见页面后再迁移。
...
flowchart LR
    ...
    P --> A[管理员通过 BE-025 接口批准或退回]
    A --> R[批准后生成课程码与隐藏内部群组]
    R --> N[暂无已发布内容]
    A -. FE-039 尚未实现 .-> UI[管理员可见审核页面]
```

该图描述当前已经接通的教师创建和 BE-025 后端审核段，不替代 2 号文档中的完整 P-08。P-08 必须等 `FE-039` 管理员可见页面完成后才能迁移；首个不可变内容版本属于 V8.4.4，不再作为 P-08 的审批前置条件。
...
- BE-024 创建阶段仍不批准课程或生成课程码；BE-025 批准后才同步正式信息、共同教师、准入班级、课程码和内部群组。学生加入与名单接口仍属于 `BE-026`。
- 教师端页面已经完成四步向导、同一课程列表、待审状态、失败恢复和角色资源独立加载；管理员审核后端已经完成，管理员可见审核页仍属于 `FE-039`。
- 课程信息批准不会生成空白 `CourseRelease` 或 `CourseClassReleaseBinding`；没有真实单元和完成规则时，读取端明确返回“暂无已发布内容”。首个不可变版本留给 V8.4.4 的教师显式发布。
- 最终隔离暂存快照通过 261 个受跟踪 JavaScript 文件语法检查和 71 项前端契约；后端课程创建/关系专项 4 项通过，Alembic 唯一 head 为 `20260825_0056`。
- 浏览器实测覆盖创建者创建并提交、共同教师读取同一待审记录、桌面与 `390×844` 移动端布局，以及无新增 console warning/error。127 项正式活动未修改。

---

## V8.4.2 课程信息审核后端（BE-025 当前稳定实现）

本节接收原规划图 `P-06`。状态图已经按照 `BE-025@3fde9af` 的真实接口、事务和验证结果从“目标态”修正为“当前稳定实现”；[`02-更新规划.md`](../02-更新规划.md) 只保留完成结论和本节链接。完整课程创建时序图 `P-08` 仍等待 `FE-039` 管理员页面，不在本节提前迁移。

### 当前课程信息审核状态图（原 P-06）

- **状态**：当前稳定实现（后端）；管理员可见页面尚待 `FE-039`。
...
- 管理员还没有 FE-039 审核页面，当前能力只能通过已认证 API 和自动化测试使用。
- 学生尚不能输入课程码、申请加入或进入课程名单；这些属于 V8.4.3 / BE-026—FE-040。
- 教师尚不能用结构化编辑器建立真实课程单元和完成规则，也不能生成首个不可变 release；这些属于 V8.4.4。
- P-08 仍保留在 2 号文档，待 FE-039 完成真实可见创建—审核旅程后迁移。
