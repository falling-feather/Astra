...
> 最近更新：2026-08-25（V8.4.1 / DATA-009）
> 更新者：DOC 组（主开发单线兼任）

集中保存 V8.0.34—V8.4.1 的集成状态、三角色业务闭环、学习空间完成层次与展示边界。
...
- **尚未越级声明**：`course_cohort` 类型已经进入模型和约束；`V8.4.1 / DATA-009` 只建立课程关系模型，实际内部课程群组要到 `V8.4.2 / BE-025` 首次审核通过的事务中创建。
...
`DATA-009@5596112` 把课程闭环所需的关系模型接入稳定 Alembic 链，`BE-024@4fa147c` 接通教师创建、共同教师可见和信息修订提交。数据库基线为唯一 head `20260825_0056`。本节只记录已经落地的事实；规划 P-03 仍需等待 `BE-025—BE-026` 完成后再按真实接口与页面迁入。
...
- 本迁移没有创建任何 `ClassGroup(kind=course_cohort)` 实例。内部课程群组仍由 `V8.4.2 / BE-025` 在首次课程审核通过的同一事务中生成。
- BE-024 没有批准课程、生成课程码、创建内部课程群组、生成初始发布版本，也没有学生加入或名单接口；这些动作仍属于 `BE-025—BE-026`。
- 本阶段没有页面变化。下一步是 `V8.4.1 / FE-038` 教师创建课程分步向导。
- 最终隔离暂存快照完成 70 项课程、权限与身份回归，覆盖新创作接口、DATA-009、旧课程、课程分类、教师申请、协作者权限、发布计划、单元访问、学校班级和学生作业中心。
