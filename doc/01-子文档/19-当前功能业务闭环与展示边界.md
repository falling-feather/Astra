...
> 最近更新：2026-08-25（V8.4.4 / BE-027）
> 更新者：DOC 组（主开发单线兼任）

集中保存 V8.0.34—V8.4.4 的集成状态、三角色业务闭环、学习空间完成层次与展示边界。V8.4.2 已将“教师建课—管理员审核—生成课程码”整理为可见业务旅程；V8.4.3 已完成课程成员闭环；V8.4.4 的 BE-027 已建立多教师共享草稿、不可变发布历史和课程成员当前版本读取，结构化编辑器与三种完成预设仍在后续任务中。
...
- BE-027 已提供共享草稿、七类内容 schema、实验引用、显式发布和不可变版本的后端能力；教师可见结构化编辑器属于 FE-041，三种完成规则属于 BE-028。
...
- **状态**：当前稳定实现。
- **数据库基线**：Alembic `20260825_0057`，唯一稳定 head。
- **实际提交**：`BE-027@5a2d5f1`。
...
- **兼容边界**：binding 只指向课程隐藏 `course_cohort` 对应的 `CourseClass`，不绑定行政班；BE-028 尚未配置规则时，release 明确冻结 `not_configured` 完成规则快照，不伪造学习完成。
...
- FE-041 已在后续提交中完成；BE-028 的实验操作、检查点和作业批改三种完成预设尚未执行。当前 `not_configured` 快照只诚实记录“未配置”，不产生完成结果。
...
- 本任务没有新增数据库迁移，没有改动 127 项活动，没有接入自由拖拽、管理员逐内容审批、小程序、服务器部署或竞赛材料。`BE-028` 尚未完成，因此 release 中的 `not_configured` 规则快照仍不产生学习完成结果。
