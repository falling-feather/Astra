...
> 最近更新：2026-08-28（V8.6.3 / FE-044）
> 更新者：DOC 组（主开发单线兼任）

集中保存 V8.0.34—V8.6.3 的集成状态、三角色业务闭环、学习空间完成层次与展示边界。V8.4 已闭合课程业务，V8.5 把既有事实整理为四项可见展示增强，V8.6.1—V8.6.3 再依次完成主演示清理、课程健康只读聚合和管理员健康矩阵；上述版本均未修改实验本体。
...
专项自动化覆盖中文演示数据、旧库兼容升级、可见技术残留禁入、完成规则独立比较和资源代际；学生与教师真实页面已经验证账号区、结构化作答、批改反馈和发布预演。后续 `QA-034` 仍负责把本轮全部模块放到同一桌面/移动矩阵中终验。
...
以下问题曾是 V8.5 的可见维护边界；前三项已由 `UI-016` 收束，剩余回归范围继续只认 [2 号文档当前任务](../02-更新规划.md#3-下一轮正式任务原件)：
