...
> 最近更新：2026-09-08（正式门户接入；普通开发提交，未新增产品发布号）
> 更新者：BE组 / ARCH组（主开发单线）
...
## 全局审查结论与后续边界（ARCH-013，2026-09-08）

本次审查按真实入口追踪了登录、身份、学校/班级、课程资料、共享草稿、发布包、单元开放、提交批改、完成投影和前端资源生命周期。修复结果分别归入 BE-042—BE-044 与 FE-053；当前没有替换 FastAPI 或引入无业务依据的微服务拆分。

| 结构判断 | 当前处理 | 下一步 |
| --- | --- | --- |
| 前端展示与业务混杂 | 新门户的视图、数据服务、草稿派生和渲染资源分开；真实与演示适配器显式选择 | 扩展复杂课程编辑时按业务页继续拆分控制器，不回灌旧巨型页面 |
| 前后端 DTO 可漂移 | 真机联调修正教师申请字段与旧报告正文映射，保留明确错误 | 在 ARCH-012 的协议工作中引入 OpenAPI 来源的类型生成/契约差异门禁，减少手写重复 |
| 旧权限通道绕过新页面 | 默认关闭直接教师注册、自建学校和直接入班；仅本机旧夹具显式兼容 | 单校实例的组织绑定与旧数据清理仍需受控迁移，不能通过删库实现 |
| 草稿状态影响已发布学习 | 草稿成员与已发布状态解耦；稳定活动身份不重赋，恢复保留源单元 | DATA-011 拆开资源身份与单元实例，为同资源重复引用、课程家族与版本固定提供模型 |
| 发布与评价混用 | 不可变包、运行携带发布 ID、服务端检查点和批改、有效规则投影保持边界 | 候选内容审核、教师补做策略与旧结果认定仍待 BE-035—BE-038，不包装成现成功能 |
| 内容及资源能力不均 | 127 个目录身份完整保留，正文 Markdown 安全渲染；复杂旧内容保留 | 完整媒体文件服务、模板参数校验和复杂块编辑由资源/模板任务承接 |
| 查询和运维 | 发布正文去除逐单元读取，完成集合共用查询；列表已有分页入口 | 教师课程资料列表等旧读取仍需容量和 SQL 预算评测；真实 MySQL、公网后端、正式 OJ 与大规模负载不由本机结果替代 |

课程维度的具体改进顺序以 [02 第 2.14 节](../02-更新规划.md#214-交叉审查后的课程改进顺序) 为唯一计划原件。当前不因某项实验画面复杂就把它定为优先精品课，也不把共享一个实现的多个活动 slug 算成多个独立实验。
