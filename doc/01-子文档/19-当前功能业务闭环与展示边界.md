...
> 最近更新：2026-08-28（V8.5.4 / QA-033）
> 更新者：DOC 组（主开发单线兼任）

集中保存 V8.0.34—V8.5.4 的集成状态、三角色业务闭环、学习空间完成层次与展示边界。V8.4 已闭合教师申请、建课审核、课程成员、共享草稿、不可变发布、三种完成结果和三角色工作台；V8.5 在不增加数据库迁移、不修改实验本体的前提下，把这些既有事实整理为教学管理驾驶舱、课程版本时光机、发布影响预演和师生学习证据链。
...
以下问题不否定 V8.5 的功能验收，但属于当前稳定实现仍然可见的维护边界；后续是否处理只认 [2 号文档下一轮任务](../02-更新规划.md#3-下一轮正式任务原件)：

- 学生总览的恢复失败说明仍可能同时显示 `recovery_route_unavailable` 内部代码；用户可理解的中文解释已经存在，但代码本身不适合作为作品展示文案。
- 本地演示数据仍有英文占位反馈、原始 JSON 和“用户 #数字”式技术身份；它们来自真实提交和发布事实，不是伪造结果，但展示转换尚未统一。
- 发布影响预演会把单元完成方式纳入内容差异并阻止无变化重复发布；版本历史也会单独显示“完成规则发生变化”，但预演摘要目前只把这类变化计入“调整单元”。
