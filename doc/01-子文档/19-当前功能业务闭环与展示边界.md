...
> 最近更新：2026-09-05（V8.6.6 后的普通开发提交，未新增产品发布号）
> 更新者：BE组 / ARCH组（主开发单线）
...
计入条件同时包括：已批准课程、active 课程成员、最新 release 中精确的单元和活动身份、发布规则及其版本、当前 plan 的同规则绑定、active 的内部 `course_cohort` 范围、学校一致，以及 `completed / transferred` 结果。未发布草稿不影响已发布版本统计；仅正文变化且规则复用时保留完成，新规则不继承旧规则完成。
...
补充回归覆盖退课、同规则正文发布、规则换版、其他班级历史投影与 `transferred` 双端一致性。提交与验证见 [03 开发历史](../03-发布历史.md)。
