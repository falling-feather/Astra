...
- **状态**：当前稳定后端实现。
- **协作语义**：创建者和有效共同教师读取同一 `content_draft_revision`；旧修订必须由调用方重新读取并合并，服务端不会静默覆盖。
- **发布语义**：发布、页面版本、release units、内部群组 binding、审计和草稿滚动到下一修订处于同一事务。
...
sequenceDiagram
    ...
    T1->>API: GET draft
    API-->>T1: revision 7 与完整单元
    T2->>API: PATCH draft expected_revision=7
    API->>D: 原子替换并排序
    D-->>T2: revision 8
    T1->>API: PATCH draft expected_revision=7
    API-->>T1: 409，当前 revision 8
    T1->>API: 重新读取、合并并保存 revision 8
    API-->>T1: revision 9
    T2->>API: POST releases expected_revision=9
...
- FE-041 的教师可见结构化编辑器尚未完成；BE-028 的实验操作、检查点和作业批改三种完成预设尚未执行。当前 `not_configured` 快照只诚实记录“未配置”，不产生完成结果。
