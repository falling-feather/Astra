...
> **文档梗概**：作为 Astra 参赛展示版的唯一规划控制面，负责可观测师生交互、两门旗舰课程动画、最小后端真实性、浏览器验收、团队登记、任务、版本和风险；稳定实现以 [`01-开发者手册.md`](01-开发者手册.md) 为准，已发生提交以 [`03-发布历史.md`](03-发布历史.md) 为准。
>
> **具体规划法则**：
>
...
> 4. 2026-08-22 起项目进入暂停收束态：所有工作组已解散，主开发只完成文档归并、Git 对账和安全工作树清理；恢复开发前必须重新登记最小责任组和写入边界，禁止把冻结候选当成后台任务。
>
> **最后一次更新时间**：2026-08-22
>
> **更新者**：PM组
...
- **项目对接模式**：暂停（工作组已解散，保留 Git 与任务历史）
- **主控任务标题**：星序 Astra｜主开发｜总控
- **版本分配者**：主开发
- **当前版本轨道**：`V8.0.34` 展示候选暂停 → `V8.0.35` 文档归并与工作树收束；恢复后的产品版本重新由用户和主开发确认
- **集成分支**：`主开发`
- **当前基线**：`V8.0.34 / d1c4d24` 已集成 V832 移动回执、V833 Engineering 同帧布局和 V834 统一资源代际；主线门禁为 239 个 JavaScript 与 60/60 前端合同、BE-018 14/14。V8.0.34 尚未执行原计划 QA-023 同 revision 浏览器终验，因此最终展示结论仍停在“V830 两项 P1 已代码返修、待 V834 真实复验”，不能声明最终 PASS。
- **提交责任组字段**：启用
- **长期团队策略**：当前不保留任何活跃工作组或后台任务。UI、FE、QA 的已交付结果进入主线，工作树按安全门禁移除；脏树和独有提交树仅作为冻结 Git 资产，不对应活跃小组。恢复时从实际任务重新建立最小团队，不批量复活旧组。

| 小组 | 代号 | 长期任务标题 | 责任边界 | Git 模式 | 分支/工作树 | 生命周期 |
| --- | --- | --- | --- | --- | --- | --- |
| 主开发 | PM | 星序 Astra｜主开发｜总控 | 用户接口、收束记录、版本、共享文档与 Git 对账 | 集成工作区 | `主开发` / 根工作区 | 暂停前收束 |
...
当前派单状态：无。UI-007/V832、FE-036/V833 与 V834 统一代际均已进入主线；QA-023 的 V834 定向终验在真正启动前被中断。所有工作组已解散，后续不会自行后台推进。六课迁移、深度分析、演示数据系统、用户研究、生产发布和底层重构继续暂缓。
...
本节只负责当前 Git 资源对账，不继承旧任务过程。2026-08-22 按 PM-024 完成新一轮全面收束：本轮处置清单覆盖 24 棵非根工作树，其中 16 棵已安全退出，8 棵因未提交修改或独有提交继续保全；全部 66 条本地分支保持原样。

| 类别 | 标识 | 当前事实 | 本周期动作 |
| --- | --- | --- | --- |
| 集成工作区 | 根工作区 / `主开发@d1c4d24` | V832=`b730a62`、V833=`fc49324` 与 V834=`d1c4d24` 已集成；V834 的 QA-023 同 revision 终验尚未执行 | 只完成 PM-024/V835 文档与 Git 收束，不修改产品运行字节 |
| BE 脏现场 | `5779@f505353` | 11 个 tracked 修改 + 2 个 untracked 文件；HEAD 无主线外提交 | 冻结保全；不 reset、clean、rebase、强制移除或删除分支 |
| Future 脏候选 | `f017@4ea1643`、`f017a/f017b/f017c@948b692` | 依次为 `36+22`、`4+6`、`4+6`、`4+6` 个 tracked/untracked 文件；HEAD 均无主线外提交 | 冻结保全；不把旧差异复制进新任务，恢复时逐树决定吸收、归档或放弃 |
| 其他脏候选 | `fr89@197bada`、`ux64@12aeef9` | 分别为 `7+0`、`4+1` 个 tracked/untracked 文件；HEAD 均无主线外提交 | 冻结保全；不再对应活跃 FE/UX 工作组 |
| clean 独有提交树 | `u358@3734a2f` | 工作区 clean，但相对 `主开发` 有 1 条未集成提交 `V7.9.60 UI fix(admin): 修复管理入口与资源代际（UI-003）` | 保留工作树和分支；未做 merge、cherry-pick、rebase 或删除，等待用户决定是否重新验收吸收 |
| 本轮安全退出 | `6f2a`、`be13`、`c04e`、`da03`、`f27a`、`fe13`、`fe29`、`fe30`、`fe35`、`ft87`、`fz30`、`qa22`、`u356`、`ui04`、`ui05`、`ui06` | 16 棵均 clean，且提交已集成、补丁等价或完成标题已在主线形成对应事实 | 全部使用普通 `git worktree remove`，随后 `git worktree prune`；未使用 `--force`，对应分支一条未删 |
| 分支与工作组 | 66 条本地分支；协作树仅保留 `/root` | 工作组、子工作组与后台任务均已解散；Git 分支是历史资产，不等于活跃工作组 | 保留全部分支；恢复时从实际任务建立最小组，不批量复活旧树 |

剩余 8 棵树都有明确保留理由。本周期不再清理脏树或独有提交树；后续仍禁止 `--force`、递归删除和 `git branch -D`。恢复时先确定是否继续使用这些受保护现场；新任务只建立必要的最小工作组和工作树，禁止批量复活旧组或为同一窄修创建重复现场。
...
除 PM-024 收束外不再持续推进产品任务。用户明确恢复后，主开发先核对 8 棵受保护工作树、V834 未完成的 QA-023 和最新 Git 基线，再建立最小责任组；不会自动唤醒旧任务。
