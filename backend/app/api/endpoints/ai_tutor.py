...
def get_config(current_user: User = Depends(get_current_user)) -> AiTutorConfigRead:
    del current_user
...
def chat(payload: AiTutorChatCreate, current_user: User = Depends(get_current_user)) -> AiTutorChatRead:
    ...
    system = (
        ...
        f"当前学习身份：{current_user.role}。当前页面上下文：{context_line or '未提供'}。"
    )
