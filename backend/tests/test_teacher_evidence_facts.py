...
def test_demo_producer_profiles_and_teacher_projection_stay_aligned():
    physics = DEMO_EVIDENCE_PROFILE_BY_ACTIVITY["physics.mechanics"]
    assert physics.producer_mode == PRODUCER_MODE_FRONTEND_PRECISE
    assert physics.course_fact_status == COURSE_FACT_STATUS_PRECISE
    physics_expectations = (
        ...
        (
            "explained",
            1,
            {"artifact.kind", "artifact.value", "cursor.stage"},
            False,
        ),
    )
    for event_type, index, expected_keys, expected_truncated in physics_expectations:
        summary = _project(
            physics.activity_key,
            event_type,
            build_demo_evidence_payload(physics, event_type, index),
        )
        assert set(summary["facts"]) == expected_keys
        assert summary["truncated"] is expected_truncated

    control = DEMO_EVIDENCE_PROFILE_BY_ACTIVITY["control-flow.loop-boundary"]
    assert control.producer_mode == PRODUCER_MODE_FRONTEND_SHALLOW
    assert control.course_fact_status == COURSE_FACT_STATUS_SHALLOW
    control_expectations = (
        ("started", 1, set(), True),
        (
            "predicted",
            1,
            {"prediction.choice", "cursor.stage"},
            False,
        ),
        (
            "attempted",
            1,
            {"operation", "cursor.runner"},
            True,
        ),
        (
            "attempted",
            2,
            {"operation", "reported_correct", "cursor.runner"},
            False,
        ),
        (
            "attempted",
            3,
            {"operation", "cursor.judge"},
            False,
        ),
        (
            "corrected",
            1,
            {"correction.kind", "correction.result", "cursor.stage"},
            False,
        ),
        (
            "explained",
    ...
    for event_type, index, expected_keys, expected_truncated in control_expectations:
        summary = _project(
            control.activity_key,
            event_type,
            build_demo_evidence_payload(control, event_type, index),
        )
        assert set(summary["facts"]) == expected_keys
        assert summary["truncated"] is expected_truncated

    for profile in DEMO_EVIDENCE_PROFILE_BY_ACTIVITY.values():
        if profile.course_fact_status != COURSE_FACT_STATUS_UNAVAILABLE:
            continue
        assert profile.producer_mode == PRODUCER_MODE_GENERIC_FALLBACK
        for event_type in ("started", "predicted", "attempted", "corrected", "explained"):
            summary = _project(
                profile.activity_key,
                event_type,
                build_demo_evidence_payload(profile, event_type, 1),
            )
            assert summary == {"facts": {}, "truncated": True}
