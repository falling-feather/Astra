...
6. 相同 `client_event_id` 再次 POST 得到 201 accepted，再次重放得到 200 duplicate 且 SQLite 仅一行；
7. 后续事件继续走正式 `/api/learning-evidence/events`；
8. 对账 learner 事件、rule producer 的唯一 `completed`、projection、规则 witness、教师 class+course 事件回读和 aggregate；
9. 检查 `PRAGMA foreign_key_check`；
10. 默认只删除由本进程创建且再次验证过名称、父目录和非仓库重叠关系的 TEMP 目录。

`--keep-data` 仅用于明确的人工诊断；默认验收不得使用它。凭据随机生成且不进入报告。
...
- `structured-backend-result`：移除真实后端结果形状中的 rule completed owner、把 projection 改为 `in_progress` 并清除教师完成聚合；验证服务端 owner 缺失必定失败。
...
- `server_completion_owner`：learner 零 completed，rule producer 唯一 completed；
- `teacher_exact_scope_readback`：教师 class+course 原始事件与 aggregate 对账；
- `fresh_unseeded_sqlite`：仓库外新库、迁移版本、空初态、无演示 seed、FK 与清理；
