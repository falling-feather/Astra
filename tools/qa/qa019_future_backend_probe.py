...
def _configure_database(database_path: Path) -> tuple[str, str]:
    ...
    if len(heads) != 1:
        raise AssertionError(f"QA-019 requires one Alembic head, found {heads}")
    expected_head = heads[0]
    command.upgrade(config, "head")
    return database_url, expected_head
...
def _run(
    ...
    if alembic_revision != expected_alembic_head:
        raise AssertionError(
            f"database revision {alembic_revision!r} does not match current Alembic head "
    ...
    teacher_items = teacher_events["items"]
    receipt_event_ids = [receipt["event_id"] for receipt in receipts]
    teacher_event_ids = [item["event_id"] for item in teacher_items]
    if (
        len(teacher_event_ids) != 6
        or len(set(teacher_event_ids)) != 6
        or set(teacher_event_ids) != set(receipt_event_ids)
    ):
        raise AssertionError("teacher item event IDs do not match the six learner receipts")
    if any(item["subject_user_id"] != scope["student"]["id"] for item in teacher_items):
        raise AssertionError("teacher item escaped the requested subject scope")
    if any(item["producer_type"] != "learner" for item in teacher_items):
        raise AssertionError("teacher raw item unexpectedly exposes a non-learner producer")
    if any(item["activity_key"] != scope["activity_key"] for item in teacher_items):
        raise AssertionError("teacher item escaped engineering.load-path")
    ...
    return {
        ...
        "sqlite": {
            ...
            "rule_witness": {
                ...
                "learner_receipt_event_ids": receipt_event_ids,
            },
        ...
        "teacher_readback": {
            ...
            "event_ids": teacher_event_ids,
            "item_subject_user_ids": [item["subject_user_id"] for item in teacher_items],
            "event_types": [item["event_type"] for item in teacher_items],
            "producer_types": [item["producer_type"] for item in teacher_items],
            "activity_keys": [item["activity_key"] for item in teacher_items],
...
def main() -> int:
    ...
    try:
        ...
        database_url, expected_alembic_head = _configure_database(database_path)
        report = _run(payload, database_path, database_url, expected_alembic_head)
        exit_code = 0
