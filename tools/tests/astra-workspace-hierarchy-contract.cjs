...
assert.ok(bootRouterUrl, 'the workspace entry must load a versioned router');
assert.match(main, /const galaxy = window\.AstraPageRegistry[\s\S]*const showEnglab = galaxy === 'englab' && page !== 'home'/);
assert.ok(main.includes(`'./${bootRouterUrl}'`), 'workspace fallback must use the boot router generation');
assert.ok(serviceWorker.includes(`'./${bootRouterUrl}'`), 'workspace precache must use the boot router generation');
