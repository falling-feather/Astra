...
const courseAuthoringOwner = read('pages/teacher/teacher-course-authoring.js');
...
assert.ok(courseAuthoringOwner.includes('name="galaxy_key"'), 'course authoring must expose the galaxy classification');
assert.ok(teacher.includes('name="activity_key"'), 'unit authoring must expose the activity key');
assert.doesNotMatch(courseAuthoringOwner, /name="course_key"/, 'course keys are generated by the service and must not be teacher-entered');
assert.match(courseAuthoringOwner, /galaxy_key:\s*String\(draft\.galaxy_key/);
