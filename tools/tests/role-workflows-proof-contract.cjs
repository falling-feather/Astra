...
assert.match(source, /loadedScripts:[\s\S]*filter\(\(resourcePath\) => rolePaths\.includes\(resourcePath\)\)/);
...
assert.match(source, /expectedUnboundLearningEvidence[\s\S]*\/api\\\/learning-evidence\\\/classes\\\/\\d\+\\\/courses\\\/\\d\+\\\/aggregate/);
assert.match(source, /expectedUnboundLearningEvidence[\s\S]*\/api\/learning-evidence\/me\/recovery/);
assert.match(source, /teacherForm\([\s\S]*'assignment-class-policy'[\s\S]*当前班级作业与积分覆盖策略已保存/);
...
assert.match(source, /admin frozen business overview matches pending, disabled, organization, course and audit authority/);
assert.match(source, /data-admin-business-overview-state="ready"/);
assert.match(source, /pendingRelationshipPage[\s\S]*disabledAccountPage[\s\S]*overviewCourses[\s\S]*overviewAudit/);
assert.match(source, /businessOverview\.beforeDataMap/);
assert.match(source, /async function selectAdminSection/);
assert.match(source, /const ADMIN_DOMAINS = Object\.freeze\(\[[\s\S]*'overview'[\s\S]*'organizations'[\s\S]*'identity'[\s\S]*'classes'[\s\S]*'courses'[\s\S]*'audit'/);
assert.match(source, /selectAdminSection\(adminRuntime\.page, 'identity', '\[data-admin-panel="join-requests"\]'/);
assert.match(source, /selectAdminSection\(adminRuntime\.page, 'identity', '\[data-admin-panel="users"\]'/);
assert.match(source, /selectAdminSection\(adminRuntime\.page, 'audit', '\[data-admin-panel="audit-logs"\]'/);
assert.match(source, /selectAdminSection\(adminRuntime\.page, 'overview', '\[data-admin-overview\]'/);
assert.doesNotMatch(source, /selectAdminSection\([^)]*, 'operations'/);
assert.match(source, /panelId === 'classes' \? 'classes' : 'organizations'/);
assert.match(source, /async function createAdminCourseFixture/);
assert.match(source, /\/api\/courses\/\$\{courseId\}\/classes[\s\S]*\/api\/courses\/\$\{courseId\}\/units[\s\S]*\/api\/courses\/\$\{courseId\}\/units\/\$\{unitIds\[0\]\}\/assignments/);
assert.match(source, /expectedImpact:[\s\S]*attached_class_count:\s*1[\s\S]*course_unit_count:\s*2[\s\S]*assignment_count:\s*1/);
assert.match(source, /studentCourseStatusDenied\.status === 403 && teacherCourseStatusDenied\.status === 403/);
assert.match(source, /adminOwnerLoads\.every\(\(item\) => item\.authorityConfirmed\)/);
assert.match(source, /anonymous admin route must not preload admin owner scripts/);
assert.match(source, /admin business home must prioritize the real pending join request/);
assert.match(source, /async function adminGovernanceLayoutEvidence/);
assert.match(source, /width <= 820[\s\S]*scrollIntoView\(\{ block: 'center', inline: 'nearest', behavior: 'auto' \}\)[\s\S]*domainNavigation\.scrollIntoViewIfNeeded\(\)/);
assert.match(source, /1015,[\s\S]*898/);
assert.match(source, /390,[\s\S]*844/);
assert.match(source, /\['ArrowRight', 'organizations'\][\s\S]*\['ArrowLeft', 'overview'\][\s\S]*\['End', 'audit'\][\s\S]*\['Home', 'overview'\]/);
assert.match(source, /evidence\.width >= 44 && evidence\.height >= 44/);
assert.match(source, /element\.contains\(hit\)/);
assert.match(source, /emulateMedia\(\{ reducedMotion: 'reduce' \}\)/);
assert.match(source, /joinReviewPatches\.length === 0/);
assert.match(source, /joinReviewPatches\.length === 1/);
assert.match(source, /\/api\/classes\/\$\{classId\}\/members\/page\?role=student&status=active/);
assert.match(source, /pending_class_join_requests[\s\S]*class\.join\.request\.approve[\s\S]*resource_type=class_join_request[\s\S]*request_id=/);
assert.match(source, /async function exerciseAdminCourseGovernance/);
assert.match(source, /first course confirmation sent PATCH/);
assert.match(source, /course publish expected exactly one PATCH/);
assert.match(source, /expected_status:\s*'draft'[\s\S]*status:\s*'published'/);
assert.match(source, /course exact audit evidence mismatch/);
assert.match(source, /searchParams\.get\('action'\) === 'course\.status\.patch'[\s\S]*searchParams\.get\('resource_type'\) === 'course'[\s\S]*searchParams\.get\('resource_id'\)[\s\S]*searchParams\.get\('request_id'\)[\s\S]*searchParams\.get\('from'\)[\s\S]*searchParams\.get\('to'\)/);
assert.match(source, /course status governance changed Unit release plan/);
assert.match(source, /unknown-lock action expected one sent PATCH/);
assert.match(source, /locked course A must stay pinned when the current filter would otherwise hide it/);
assert.match(source, /course A lock must be auto-selected after mobile re-entry/);
assert.match(source, /selected locked course A must remain a read-only reconciliation entry/);
assert.match(source, /course B must remain disabled while course A owns the global lock/);
assert.match(source, /reopening locked course A with keyboard must send zero PATCH requests/);
assert.match(source, /read-only course reconciliation must not resend PATCH/);
assert.match(source, /course B must become selectable only after explicit reconciliation and unlock/);
assert.match(source, /admin top refresh must remain disabled while course unknown lock exists/);
assert.match(source, /admin API Base must remain disabled while course unknown lock exists/);
assert.match(source, /async function exerciseAdvancedLazyLoading/);
assert.match(source, /advanced diagnostics were prefetched/);
assert.match(source, /beforeEntry:\s*0/);
