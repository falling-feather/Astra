...
当前门户使用 TypeScript、原生 DOM、Vite 和 Three.js。设计与页面说明统一见 [01 第 5.4 节](../doc/01-开发者手册.md#54-正式门户的实现与数据适配)，进度与未来任务见 [02](../doc/02-更新规划.md)，不要在本目录另建一份实施计划。
...
实际运行先核对 [04 环境约束](../doc/04-部署指南.md#1-运行方式与环境)；仓库与 Windows 构建已统一使用 Node 24.20.0 / npm 11.19.0。从仓库根目录运行 `powershell -ExecutionPolicy Bypass -File .\astra-local.ps1`，脚本构建正式前端、迁移数据库并在 `http://127.0.0.1:9001/` 提供完整版本。首次创建演示账号可使用 `-InitializeDemoData`；这只操作指定的本机数据目录。
